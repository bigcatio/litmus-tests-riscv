...
inline static int final_cond(int _out_0_x1) {
  switch (_out_0_x1) {
  case 0:
    return 1;
  default:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x1=%d;\n",c,show ? '*' : ':',(int)o[out_0_x1_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("lr.w.aq.rl, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"lr.w.aq.rl");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      cond = final_ok(final_cond(_out_0_x1_i));
...
#else
#include "lr.w.aq.rl.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SINGLE_INST/lr.w.aq.rl.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV lr.w.aq.rl");
  fprintf(out,"%s\n","\"Test the instruction lr.w.aq.rl\"");
  fprintf(out,"%s\n","{");
  fprintf(out,"%s\n"," 0:x2=x;");
  fprintf(out,"%s\n","}");
  fprintf(out,"%s\n"," P0                 ;");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test lr.w.aq.rl Required\n");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "lr.w.aq.rl: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int lr_2E_w_2E_aq_2E_rl(int argc, char **argv, FILE *out) {
