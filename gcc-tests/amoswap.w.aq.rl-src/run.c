...
extern int amoswap_2E_w_2E_aq_2E_rl(int argc,char **argv,FILE *out);
...
static void end_report(int argc,char **argv,FILE *out) {
  ...
  fprintf(out,"%s\n","Command line: litmus7 -mach ./riscv.cfg -avail 4 -o gcc-tests/amoswap.w.aq.rl-src tests/non-mixed-size/SINGLE_INST/amoswap.w.aq.rl.litmus");
  fprintf(out,"%s\n","Parameters");
...
static void run(int argc,char **argv,FILE *out) {
  my_date(out);
  amoswap_2E_w_2E_aq_2E_rl(argc,argv,out);
