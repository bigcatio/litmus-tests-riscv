...
inline static int final_cond(int _out_0_x1,int _x) {
  switch (_out_0_x1) {
  case 0:
    switch (_x) {
    case 1:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x1=%d; [x]=%d;\n",c,show ? '*' : ':',(int)o[out_0_x1_f],(int)o[x_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("amoswap.w.aq.rl, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"amoswap.w.aq.rl");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("amoswap.w.aq.rl, global x unstabilized") ;
      }
      cond = final_ok(final_cond(_out_0_x1_i,_x_i));
...
#else
#include "amoswap.w.aq.rl.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SINGLE_INST/amoswap.w.aq.rl.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test amoswap.w.aq.rl Required\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","forall (0:x1=0 /\\ [x]=1)",cond ? "" : "NOT ");
  fprintf(out,"Hash=ba6f160115e2c7844292236b3ea070cc\n");
  count_t cond_true = p_false;
  count_t cond_false = p_true;
  fprintf(out,"Observation amoswap.w.aq.rl %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time amoswap.w.aq.rl %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "amoswap.w.aq.rl: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("amoswap.w.aq.rl, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int amoswap_2E_w_2E_aq_2E_rl(int argc, char **argv, FILE *out) {
