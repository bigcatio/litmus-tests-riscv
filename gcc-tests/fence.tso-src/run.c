...
extern int fence_2E_tso(int argc,char **argv,FILE *out);
...
static void end_report(int argc,char **argv,FILE *out) {
  ...
  fprintf(out,"%s\n","Command line: litmus7 -mach ./riscv.cfg -avail 4 -o gcc-tests/fence.tso-src tests/non-mixed-size/SINGLE_INST/fence.tso.litmus");
  fprintf(out,"%s\n","Parameters");
