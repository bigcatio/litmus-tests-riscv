...
inline static int final_cond(void) {
  return 1;
}
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"fence.tso");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SINGLE_INST/fence.tso.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV fence.tso");
  fprintf(out,"%s\n","\"Test the instruction fence.tso\"");
  fprintf(out,"%s\n","{");
  fprintf(out,"%s\n","}");
  fprintf(out,"%s\n"," P0        ;");
  fprintf(out,"%s\n"," fence.tso ;");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test fence.tso Required\n");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "fence.tso: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int fence_2E_tso(int argc, char **argv, FILE *out) {
