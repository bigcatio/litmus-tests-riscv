...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("ISA10, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA10, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA10, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA10");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x13],0(%[x8])\n"
"#END _litmus_P1\n"
:[x13] "=&r" (_a->out_1_x13[_i]),[x10] "=&r" (_a->out_1_x10[_i]),[x28] "=&r" (trashed_x28),[x19] "=&r" (trashed_x19),[x12] "=&r" (trashed_x12),[x11] "=&r" (trashed_x11),[x8] "=&r" (trashed_x8),[x7] "=&r" (trashed_x7)
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA10 Allowed\n");
  ...
  fprintf(out,"Hash=ba0290b5600ceb909bd892eac35f1d4c\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ISA10 %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA10 %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA10: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int ISA10(int argc, char **argv, FILE *out) {
