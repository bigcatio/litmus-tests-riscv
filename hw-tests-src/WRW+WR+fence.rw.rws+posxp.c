...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("WRW+WR+fence.rw.rws+posxp, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"WRW+WR+fence.rw.rws+posxp");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("WRW+WR+fence.rw.rws+posxp, global x unstabilized") ;
      }
...
#else
#include "WRW+WR+fence.rw.rws+posxp.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/ATOMICS/CO/WRW+WR+fence.rw.rws+posxp.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test WRW+WR+fence.rw.rws+posxp Allowed\n");
  ...
  fprintf(out,"Hash=85412de5e281b7b1e10b2b6d7d6716be\n");
  fprintf(out,"Cycle=Rfe Fence.rw.rwsRW WsePX PosWRXP Fre\n");
  fprintf(out,"Relax WRW+WR+fence.rw.rws+posxp %s %s\n",p_true > 0 ? "Ok" : "No","WsePX");
  fprintf(out,"Safe=Rfe Fre PosWR Fence.rw.rwsRW\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Ws Fr\n");
  fprintf(out,"Orig=Rfe Fence.rw.rwsRW WsePX PosWRXP Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0, 1] [2] ; (2,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation WRW+WR+fence.rw.rws+posxp %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time WRW+WR+fence.rw.rws+posxp %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "WRW+WR+fence.rw.rws+posxp: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("WRW+WR+fence.rw.rws+posxp, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int WRW_2B_WR_2B_fence_2E_rw_2E_rws_2B_posxp(int argc, char **argv, FILE *out) {
