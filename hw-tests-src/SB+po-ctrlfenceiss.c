...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("SB+po-ctrlfenceiss, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("SB+po-ctrlfenceiss, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"SB+po-ctrlfenceiss");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test SB+po-ctrlfenceiss Allowed\n");
  ...
  fprintf(out,"Hash=279b53fb3866f4de843e4526f3dd6a37\n");
  fprintf(out,"Cycle=Fre PodWR DpCtrlFenceIsR Fre PodWR DpCtrlFenceIsR\n");
  fprintf(out,"Relax SB+po-ctrlfenceiss %s %s\n",p_true > 0 ? "Ok" : "No","PodWR");
  fprintf(out,"Safe=Fre DpCtrlFenceIsR\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Fr Fr\n");
  fprintf(out,"Orig=PodWR DpCtrlFenceIsR Fre PodWR DpCtrlFenceIsR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1] [0] ; (0,1) (1,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation SB+po-ctrlfenceiss %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time SB+po-ctrlfenceiss %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "SB+po-ctrlfenceiss: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("SB+po-ctrlfenceiss, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int SB_2B_po_2D_ctrlfenceiss(int argc, char **argv, FILE *out) {
