...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("LB+fence.rw.rw+addr-fri-rfi-data, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("LB+fence.rw.rw+addr-fri-rfi-data, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("LB+fence.rw.rw+addr-fri-rfi-data, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"LB+fence.rw.rw+addr-fri-rfi-data");
  check_globals(_a);
  int _th_id = _b->th_id;
...
#else
#include "LB+fence.rw.rw+addr-fri-rfi-data.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LB+fence.rw.rw+addr-fri-rfi-data Allowed\n");
  ...
  fprintf(out,"Hash=23f742778a2e5f243b35544eff8eee82\n");
  fprintf(out,"Cycle=Rfi DpDatadW Rfe Fence.rw.rwdRW Rfe DpAddrdR Fri\n");
  fprintf(out,"Relax LB+fence.rw.rw+addr-fri-rfi-data %s %s\n",p_true > 0 ? "Ok" : "No","[Fri,Rfi]");
  fprintf(out,"Safe=Rfe Fence.rw.rwdRW DpAddrdR DpDatadW\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Rf\n");
  fprintf(out,"Orig=Fence.rw.rwdRW Rfe DpAddrdR Fri Rfi DpDatadW Rfe\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1, 0] ; \n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation LB+fence.rw.rw+addr-fri-rfi-data %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time LB+fence.rw.rw+addr-fri-rfi-data %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LB+fence.rw.rw+addr-fri-rfi-data: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("LB+fence.rw.rw+addr-fri-rfi-data, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int LB_2B_fence_2E_rw_2E_rw_2B_addr_2D_fri_2D_rfi_2D_data(int argc, char **argv, FILE *out) {
