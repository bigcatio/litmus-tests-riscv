...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("LB+data+ctrlfencei-[fr-ws], check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("LB+data+ctrlfencei-[fr-ws], check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"LB+data+ctrlfencei-[fr-ws]");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("LB+data+ctrlfencei-[fr-ws], global x unstabilized") ;
      }
...
#else
#include "LB+data+ctrlfencei-[fr-ws].h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/LB+data+ctrlfencei-[fr-ws].litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV LB+data+ctrlfencei-[fr-ws]");
  fprintf(out,"%s\n","\"DpDatadW Rfe DpCtrlFenceIdR FrLeave WsBack Rfe\"");
  fprintf(out,"%s\n","{");
  fprintf(out,"%s\n"," 0:x6=x; 0:x8=y;");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LB+data+ctrlfencei-[fr-ws] Allowed\n");
  ...
  fprintf(out,"Hash=26bac7440d8433056c4ee7d24c1fa5ea\n");
  fprintf(out,"Cycle=Rfe DpDatadW Rfe DpCtrlFenceIdR FrLeave WsBack\n");
  fprintf(out,"Relax LB+data+ctrlfencei-[fr-ws] %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"Safe=Rfe DpDatadW DpCtrlFenceIdR FrLeave WsBack\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Rf Ws\n");
  fprintf(out,"Orig=DpDatadW Rfe DpCtrlFenceIdR FrLeave WsBack Rfe\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0, 1] [2] ; \n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation LB+data+ctrlfencei-[fr-ws] %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time LB+data+ctrlfencei-[fr-ws] %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LB+data+ctrlfencei-[fr-ws]: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("LB+data+ctrlfencei-[fr-ws], sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int LB_2B_data_2B_ctrlfencei_2D__5B_fr_2D_ws_5D_(int argc, char **argv, FILE *out) {
