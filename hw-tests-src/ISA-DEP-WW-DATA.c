...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA-DEP-WW-DATA, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA-DEP-WW-DATA, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA-DEP-WW-DATA");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sw %[x9],0(%[x10])\n"
"#END _litmus_P0\n"
:[x9] "=&r" (trashed_x9),[x8] "=&r" (trashed_x8),[x7] "=&r" (trashed_x7),[x6] "=&r" (trashed_x6)
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("ISA-DEP-WW-DATA, global x unstabilized") ;
      }
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA-DEP-WW-DATA Allowed\n");
  ...
  fprintf(out,"Hash=5082724191ef0bb9e85eb5586f7824c2\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ISA-DEP-WW-DATA %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA-DEP-WW-DATA %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA-DEP-WW-DATA: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("ISA-DEP-WW-DATA, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int ISA_2D_DEP_2D_WW_2D_DATA(int argc, char **argv, FILE *out) {
