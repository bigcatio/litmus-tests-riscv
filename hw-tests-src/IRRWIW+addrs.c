...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("IRRWIW+addrs, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("IRRWIW+addrs, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"IRRWIW+addrs");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("IRRWIW+addrs, global x unstabilized") ;
      }
...
#else
#include "IRRWIW+addrs.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/IRRWIW+addrs.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV IRRWIW+addrs");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test IRRWIW+addrs Allowed\n");
  ...
  fprintf(out,"Hash=616545802c70709e221ba4265edbb817\n");
  fprintf(out,"Cycle=Rfe DpAddrdW Wse Rfe DpAddrdR Fre\n");
  fprintf(out,"Relax IRRWIW+addrs %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"Safe=Rfe Fre Wse DpAddrdW DpAddrdR\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Fr Rf Ws\n");
  fprintf(out,"Orig=Rfe DpAddrdR Fre Rfe DpAddrdW Wse\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[2, 3] [0, 1] ; (1,2)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation IRRWIW+addrs %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time IRRWIW+addrs %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "IRRWIW+addrs: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int IRRWIW_2B_addrs(int argc, char **argv, FILE *out) {
