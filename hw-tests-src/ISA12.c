...
inline static int final_cond(uint64_t _x) {
  switch (_x) {
  case 2:
    return 1;
  default:
...
static const int out_0_x28_f = 0 ;
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x28=%d; [x]=%"PRIu64";\n",c,show ? '*' : ':',(int)o[out_0_x28_f],(uint64_t)o[x_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA12, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA12");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sc.d %[x28],%[x7],0(%[x10])\n"
"#END _litmus_P0\n"
:[x28] "=&r" (_a->out_0_x28[_i]),[x7] "=&r" (trashed_x7),[x6] "=&r" (trashed_x6),[x5] "=&r" (trashed_x5)
:[x10] "r" (&_a->x[_i])
:"cc","memory"
);
    }
...
static void init(ctx_t *_a) {
  ...
  _a->seed = rand();
  _a->out_0_x28 = malloc_check(size_of_test*sizeof(*(_a->out_0_x28)));
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      int _out_0_x28_i = ctx.out_0_x28[_i];
      uint64_t _x_i = ctx.x[_i];
      outcome_t o;
      int cond;

      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("ISA12, global x unstabilized") ;
      }
      cond = final_ok(final_cond(_x_i));
      o[out_0_x28_f] = _out_0_x28_i;
      o[x_f] = _x_i;
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA12 Allowed\n");
  ...
  fprintf(out,"Hash=8fed07cd82e7d350f11afbafa8066f36\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ISA12 %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA12 %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA12: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int ISA12(int argc, char **argv, FILE *out) {
