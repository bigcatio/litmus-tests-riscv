...
inline static int final_cond(int _out_0_x7,int _out_0_x8,int _out_1_x5,int _out_1_x7,int _out_2_x8,int _out_2_x9,int _x) {
  switch (_out_1_x5) {
  case 0:
    switch (_out_1_x7) {
    case 2: case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x7=%d; 0:x8=%d; 1:x5=%d; 1:x7=%d; 2:x8=%d; 2:x9=%d; [x]=%d;\n",c,show ? '*' : ':',(int)o[out_0_x7_f],(int)o[out_0_x8_f],(int)o[out_1_x5_f],(int)o[out_1_x7_f],(int)o[out_2_x8_f],(int)o[out_2_x9_f],(int)o[x_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("WRR+2W+fence.rw.rws+fence.rw.rwspx+X, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"WRR+2W+fence.rw.rws+fence.rw.rwspx+X");
  check_globals(_a);
  int _th_id = _b->th_id;
...
:[x5] "r" ((int)(2)),[x6] "r" (&_a->x[_i]),[x7] "r" ((int)(3))
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("WRR+2W+fence.rw.rws+fence.rw.rwspx+X, global x unstabilized") ;
      }
      cond = final_ok(final_cond(_out_0_x7_i,_out_0_x8_i,_out_1_x5_i,_out_1_x7_i,_out_2_x8_i,_out_2_x9_i,_x_i));
...
#else
#include "WRR+2W+fence.rw.rws+fence.rw.rwspx+X.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/ATOMICS/CO/WRR+2W+fence.rw.rws+fence.rw.rwspx+X.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV WRR+2W+fence.rw.rws+fence.rw.rwspx+X");
  fprintf(out,"%s\n","\"RfeXP Fence.rw.rwsRR Fre Fence.rw.rwsWWPX WseXX\"");
  fprintf(out,"%s\n","{");
  fprintf(out,"%s\n"," 0:x5=x; 0:x6=1;");
  fprintf(out,"%s\n"," 1:x6=x;");
  ...
  fprintf(out,"%s\n","                  |             | sc.w x9,x7,0(x6) ;");
  fprintf(out,"%s\n","");
  fprintf(out,"%s\n","exists (not (2:x8=2 /\\ (2:x9=0 /\\ ([x]=3 /\\ (0:x8=1 /\\ (1:x5=0 /\\ (0:x7=0 /\\ (1:x7=0 \\/ 1:x7=2 \\/ 1:x7=3) \\/ 0:x7=2 /\\ (1:x7=3 \\/ 1:x7=2 \\/ 1:x7=0) \\/ 0:x7=3 /\\ (1:x7=0 \\/ 1:x7=2 \\/ 1:x7=3)) \\/ 1:x5=2 /\\ (1:x7=2 /\\ (0:x7=3 \\/ 0:x7=2 \\/ 0:x7=0) \\/ 1:x7=3 /\\ (0:x7=0 \\/ 0:x7=2 \\/ 0:x7=3)) \\/ 1:x5=3 /\\ 1:x7=3 /\\ (0:x7=0 \\/ 0:x7=2 \\/ 0:x7=3)) \\/ 0:x7=0 /\\ 0:x8=0 /\\ (1:x5=0 /\\ (1:x7=3 \\/ 1:x7=2 \\/ 1:x7=1 \\/ 1:x7=0) \\/ 1:x5=1 /\\ (1:x7=1 \\/ 1:x7=2 \\/ 1:x7=3) \\/ 1:x5=2 /\\ (1:x7=3 \\/ 1:x7=2) \\/ 1:x5=3 /\\ 1:x7=3)) \\/ 0:x7=3 /\\ 0:x8=0 /\\ [x]=1 /\\ (1:x5=0 /\\ (1:x7=3 \\/ 1:x7=2 \\/ 1:x7=1 \\/ 1:x7=0) \\/ 1:x5=2 /\\ (1:x7=1 \\/ 1:x7=2 \\/ 1:x7=3) \\/ 1:x5=3 /\\ (1:x7=3 \\/ 1:x7=1) \\/ 1:x5=1 /\\ 1:x7=1)) \\/ 2:x9=1 /\\ (0:x8=0 /\\ (0:x7=0 /\\ [x]=2 /\\ (1:x5=0 /\\ (1:x7=2 \\/ 1:x7=1 \\/ 1:x7=0) \\/ 1:x5=1 /\\ (1:x7=1 \\/ 1:x7=2) \\/ 1:x5=2 /\\ 1:x7=2) \\/ 0:x7=2 /\\ [x]=1 /\\ (1:x5=0 /\\ (1:x7=0 \\/ 1:x7=1 \\/ 1:x7=2) \\/ 1:x5=2 /\\ (1:x7=2 \\/ 1:x7=1) \\/ 1:x5=1 /\\ 1:x7=1)) \\/ 0:x8=1 /\\ [x]=2 /\\ (1:x5=0 /\\ (0:x7=0 /\\ (1:x7=0 \\/ 1:x7=2) \\/ 0:x7=2 /\\ (1:x7=2 \\/ 1:x7=0)) \\/ 1:x5=2 /\\ 1:x7=2 /\\ (0:x7=2 \\/ 0:x7=0)))) \\/ 0:x7=2 /\\ 0:x8=0 /\\ 2:x8=1 /\\ (2:x9=0 /\\ [x]=3 /\\ (1:x5=0 /\\ (1:x7=3 \\/ 1:x7=2 \\/ 1:x7=1 \\/ 1:x7=0) \\/ 1:x5=2 /\\ (1:x7=1 \\/ 1:x7=2 \\/ 1:x7=3) \\/ 1:x5=1 /\\ (1:x7=3 \\/ 1:x7=1) \\/ 1:x5=3 /\\ 1:x7=3) \\/ 2:x9=1 /\\ [x]=1 /\\ (1:x5=0 /\\ (1:x7=0 \\/ 1:x7=1 \\/ 1:x7=2) \\/ 1:x5=2 /\\ (1:x7=2 \\/ 1:x7=1) \\/ 1:x5=1 /\\ 1:x7=1))))");
  fprintf(out,"%s\n","Generated assembler");
  ass(out);
}
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test WRR+2W+fence.rw.rws+fence.rw.rwspx+X Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (not (2:x8=2 /\\ (2:x9=0 /\\ ([x]=3 /\\ (0:x8=1 /\\ (1:x5=0 /\\ (0:x7=0 /\\ (1:x7=0 \\/ 1:x7=2 \\/ 1:x7=3) \\/ 0:x7=2 /\\ (1:x7=3 \\/ 1:x7=2 \\/ 1:x7=0) \\/ 0:x7=3 /\\ (1:x7=0 \\/ 1:x7=2 \\/ 1:x7=3)) \\/ 1:x5=2 /\\ (1:x7=2 /\\ (0:x7=3 \\/ 0:x7=2 \\/ 0:x7=0) \\/ 1:x7=3 /\\ (0:x7=0 \\/ 0:x7=2 \\/ 0:x7=3)) \\/ 1:x5=3 /\\ 1:x7=3 /\\ (0:x7=0 \\/ 0:x7=2 \\/ 0:x7=3)) \\/ 0:x7=0 /\\ 0:x8=0 /\\ (1:x5=0 /\\ (1:x7=3 \\/ 1:x7=2 \\/ 1:x7=1 \\/ 1:x7=0) \\/ 1:x5=1 /\\ (1:x7=1 \\/ 1:x7=2 \\/ 1:x7=3) \\/ 1:x5=2 /\\ (1:x7=3 \\/ 1:x7=2) \\/ 1:x5=3 /\\ 1:x7=3)) \\/ 0:x7=3 /\\ 0:x8=0 /\\ [x]=1 /\\ (1:x5=0 /\\ (1:x7=3 \\/ 1:x7=2 \\/ 1:x7=1 \\/ 1:x7=0) \\/ 1:x5=2 /\\ (1:x7=1 \\/ 1:x7=2 \\/ 1:x7=3) \\/ 1:x5=3 /\\ (1:x7=3 \\/ 1:x7=1) \\/ 1:x5=1 /\\ 1:x7=1)) \\/ 2:x9=1 /\\ (0:x8=0 /\\ (0:x7=0 /\\ [x]=2 /\\ (1:x5=0 /\\ (1:x7=2 \\/ 1:x7=1 \\/ 1:x7=0) \\/ 1:x5=1 /\\ (1:x7=1 \\/ 1:x7=2) \\/ 1:x5=2 /\\ 1:x7=2) \\/ 0:x7=2 /\\ [x]=1 /\\ (1:x5=0 /\\ (1:x7=0 \\/ 1:x7=1 \\/ 1:x7=2) \\/ 1:x5=2 /\\ (1:x7=2 \\/ 1:x7=1) \\/ 1:x5=1 /\\ 1:x7=1)) \\/ 0:x8=1 /\\ [x]=2 /\\ (1:x5=0 /\\ (0:x7=0 /\\ (1:x7=0 \\/ 1:x7=2) \\/ 0:x7=2 /\\ (1:x7=2 \\/ 1:x7=0)) \\/ 1:x5=2 /\\ 1:x7=2 /\\ (0:x7=2 \\/ 0:x7=0)))) \\/ 0:x7=2 /\\ 0:x8=0 /\\ 2:x8=1 /\\ (2:x9=0 /\\ [x]=3 /\\ (1:x5=0 /\\ (1:x7=3 \\/ 1:x7=2 \\/ 1:x7=1 \\/ 1:x7=0) \\/ 1:x5=2 /\\ (1:x7=1 \\/ 1:x7=2 \\/ 1:x7=3) \\/ 1:x5=1 /\\ (1:x7=3 \\/ 1:x7=1) \\/ 1:x5=3 /\\ 1:x7=3) \\/ 2:x9=1 /\\ [x]=1 /\\ (1:x5=0 /\\ (1:x7=0 \\/ 1:x7=1 \\/ 1:x7=2) \\/ 1:x5=2 /\\ (1:x7=2 \\/ 1:x7=1) \\/ 1:x5=1 /\\ 1:x7=1))))",cond ? "" : "NOT ");
  fprintf(out,"Hash=9b7bec0fb844c3a33bde71eeb98745bc\n");
  fprintf(out,"Cycle=Fre Fence.rw.rwsWWPX WseXX RfeXP Fence.rw.rwsRR\n");
  fprintf(out,"Relax WRR+2W+fence.rw.rws+fence.rw.rwspx+X %s %s\n",p_true > 0 ? "Ok" : "No","WseXX");
  fprintf(out,"Safe=Rfe Fre Fence.rw.rwsWW Fence.rw.rwsRR\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Fr Ws\n");
  fprintf(out,"Orig=RfeXP Fence.rw.rwsRR Fre Fence.rw.rwsWWPX WseXX\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0, 1] [2] ; (1,2)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation WRR+2W+fence.rw.rws+fence.rw.rwspx+X %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time WRR+2W+fence.rw.rws+fence.rw.rwspx+X %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "WRR+2W+fence.rw.rws+fence.rw.rwspx+X: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("WRR+2W+fence.rw.rws+fence.rw.rwspx+X, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int WRR_2B_2W_2B_fence_2E_rw_2E_rws_2B_fence_2E_rw_2E_rwspx_2B_X(int argc, char **argv, FILE *out) {
