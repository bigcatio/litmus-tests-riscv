...
inline static int final_cond(int _out_0_x7,int _out_1_x7,int _c,int _d) {
  switch (_out_0_x7) {
  case 0:
    switch (_out_1_x7) {
    case 0:
...
static const int c_f = 2 ;
static const int d_f = 3 ;
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x7=%d; 1:x7=%d; [c]=%d; [d]=%d;\n",c,show ? '*' : ':',(int)o[out_0_x7_f],(int)o[out_1_x7_f],(int)o[c_f],(int)o[d_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && d[_i] != 0) fatal("LR-SC-NOT-FENCE, check_globals failed");
    if (rand_bit(&(_a->seed)) && c[_i] != 0) fatal("LR-SC-NOT-FENCE, check_globals failed");
    if (rand_bit(&(_a->seed)) && b[_i] != 0) fatal("LR-SC-NOT-FENCE, check_globals failed");
    if (rand_bit(&(_a->seed)) && a[_i] != 0) fatal("LR-SC-NOT-FENCE, check_globals failed");
  }
...
static void stabilize_globals(int _id, ctx_t *_a) {
  ...
  for ( ; ; ) {
    ...
    for (int _i = size_of_test-1 ; _i >= 0 && !_found ; _i--) {
      if (cpy_c[_id][_i] != cpy_c[_nxt_id][_i]) { _found = 1; }
      if (cpy_d[_id][_i] != cpy_d[_nxt_id][_i]) { _found = 1; }
    }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"LR-SC-NOT-FENCE");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lr.w.aqrl %[x12],0(%[x10])\n"
"#_litmus_P0_3\n\t"
"sc.w.aqrl %[x11],%[x5],0(%[x10])\n"
"#_litmus_P0_4\n\t"
"lw %[x7],0(%[x8])\n"
"#END _litmus_P0\n"
:[x7] "=&r" (_a->out_0_x7[_i]),[x12] "=&r" (trashed_x12),[x11] "=&r" (trashed_x11),[x5] "=&r" (trashed_x5)
:[x6] "r" (&_a->a[_i]),[x8] "r" (&_a->b[_i]),[x10] "r" (&_a->c[_i])
:"cc","memory"
);
    }
  }
...
"lw %[x7],0(%[x8])\n"
"#END _litmus_P1\n"
:[x7] "=&r" (_a->out_1_x7[_i]),[x12] "=&r" (trashed_x12),[x11] "=&r" (trashed_x11),[x5] "=&r" (trashed_x5)
:[x6] "r" (&_a->b[_i]),[x8] "r" (&_a->a[_i]),[x10] "r" (&_a->d[_i])
:"cc","memory"
);
    }
  }
...
static void init(ctx_t *_a) {
  ...
  _a->d = malloc_check(size_of_test*sizeof(*(_a->d)));
  _a->c = malloc_check(size_of_test*sizeof(*(_a->c)));
  ...
  for (int _p = N-1 ; _p >= 0 ; _p--) {
    _a->cpy_c[_p] = malloc_check(size_of_test*sizeof(*(_a->cpy_c[_p])));
    _a->cpy_d[_p] = malloc_check(size_of_test*sizeof(*(_a->cpy_d[_p])));
  }
...
static void finalize(ctx_t *_a) {
  ...
  po_free(_a->s_or);
  for (int _p = N-1 ; _p >= 0 ; _p--) {
    free((void *)_a->cpy_c[_p]);
    free((void *)_a->cpy_d[_p]);
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_c_i != ctx.cpy_c[_p][_i]) fatal("LR-SC-NOT-FENCE, global c unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_d_i != ctx.cpy_d[_p][_i]) fatal("LR-SC-NOT-FENCE, global d unstabilized") ;
      }
      cond = final_ok(final_cond(_out_0_x7_i,_out_1_x7_i,_c_i,_d_i));
...
#else
#include "LR-SC-NOT-FENCE.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/HAND/LR-SC-NOT-FENCE.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LR-SC-NOT-FENCE Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([c]=1 /\\ [d]=1 /\\ 0:x7=0 /\\ 1:x7=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=544838a0d1533238e14999b6643fd779\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation LR-SC-NOT-FENCE %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time LR-SC-NOT-FENCE %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LR-SC-NOT-FENCE: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("LR-SC-NOT-FENCE, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int LR_2D_SC_2D_NOT_2D_FENCE(int argc, char **argv, FILE *out) {
