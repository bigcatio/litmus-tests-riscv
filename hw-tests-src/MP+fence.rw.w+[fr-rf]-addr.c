...
inline static int final_cond(int _out_1_x5,int _out_1_x7,int _out_1_x9,int _y) {
  switch (_out_1_x5) {
  case 1:
    switch (_out_1_x7) {
    case 2:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x5=%d; 1:x7=%d; 1:x9=%d; [y]=%d;\n",c,show ? '*' : ':',(int)o[out_1_x5_f],(int)o[out_1_x7_f],(int)o[out_1_x9_f],(int)o[y_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("MP+fence.rw.w+[fr-rf]-addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("MP+fence.rw.w+[fr-rf]-addr, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"MP+fence.rw.w+[fr-rf]-addr");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x9],0(%[x11])\n"
"#END _litmus_P1\n"
:[x9] "=&r" (_a->out_1_x9[_i]),[x7] "=&r" (_a->out_1_x7[_i]),[x5] "=&r" (_a->out_1_x5[_i]),[x11] "=&r" (trashed_x11),[x8] "=&r" (trashed_x8)
:[x6] "r" (&_a->y[_i]),[x10] "r" (&_a->x[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("MP+fence.rw.w+[fr-rf]-addr, global y unstabilized") ;
      }
      cond = final_ok(final_cond(_out_1_x5_i,_out_1_x7_i,_out_1_x9_i,_y_i));
...
#else
#include "MP+fence.rw.w+[fr-rf]-addr.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/MP+fence.rw.w+[fr-rf]-addr.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV MP+fence.rw.w+[fr-rf]-addr");
  fprintf(out,"%s\n","\"Fence.rw.wdWW Rfe FrLeave RfBack DpAddrdR Fre\"");
  fprintf(out,"%s\n","{");
  ...
  fprintf(out,"%s\n","             | lw x9,0(x11)   |             ;");
  fprintf(out,"%s\n","");
  fprintf(out,"%s\n","exists ([y]=2 /\\ 1:x5=1 /\\ 1:x7=2 /\\ 1:x9=0)");
  fprintf(out,"%s\n","Generated assembler");
  ass(out);
}
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test MP+fence.rw.w+[fr-rf]-addr Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([y]=2 /\\ 1:x5=1 /\\ 1:x7=2 /\\ 1:x9=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=51432b19c146a835d05d619ac7038b49\n");
  fprintf(out,"Cycle=Rfe FrLeave RfBack DpAddrdR Fre Fence.rw.wdWW\n");
  fprintf(out,"Relax MP+fence.rw.w+[fr-rf]-addr %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"Safe=Rfe Fre Fence.rw.wdWW DpAddrdR FrLeave RfBack\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Fr Rf\n");
  fprintf(out,"Orig=Fence.rw.wdWW Rfe FrLeave RfBack DpAddrdR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[2, 0] [1] ; (1,2)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation MP+fence.rw.w+[fr-rf]-addr %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time MP+fence.rw.w+[fr-rf]-addr %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "MP+fence.rw.w+[fr-rf]-addr: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("MP+fence.rw.w+[fr-rf]-addr, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int MP_2B_fence_2E_rw_2E_w_2B__5B_fr_2D_rf_5D__2D_addr(int argc, char **argv, FILE *out) {
