...
inline static int final_cond(int _out_1_x5,int _out_1_x10,int _out_1_x11,int _z) {
  switch (_out_1_x5) {
  case 1:
    switch (_out_1_x10) {
    case 2:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x5=%d; 1:x10=%d; 1:x11=%d; [z]=%d;\n",c,show ? '*' : ':',(int)o[out_1_x5_f],(int)o[out_1_x10_f],(int)o[out_1_x11_f],(int)o[z_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("MP+fence.rw.rw+data-wsi-rfi-ctrl, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("MP+fence.rw.rw+data-wsi-rfi-ctrl, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("MP+fence.rw.rw+data-wsi-rfi-ctrl, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"MP+fence.rw.rw+data-wsi-rfi-ctrl");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x11],0(%[x12])\n"
"#END _litmus_P1\n"
:[x11] "=&r" (_a->out_1_x11[_i]),[x10] "=&r" (_a->out_1_x10[_i]),[x5] "=&r" (_a->out_1_x5[_i]),[x7] "=&r" (trashed_x7)
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_z_i != ctx.cpy_z[_p][_i]) fatal("MP+fence.rw.rw+data-wsi-rfi-ctrl, global z unstabilized") ;
      }
      cond = final_ok(final_cond(_out_1_x5_i,_out_1_x10_i,_out_1_x11_i,_z_i));
...
#else
#include "MP+fence.rw.rw+data-wsi-rfi-ctrl.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test MP+fence.rw.rw+data-wsi-rfi-ctrl Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([z]=2 /\\ 1:x5=1 /\\ 1:x10=2 /\\ 1:x11=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=8b68b3f1c991ed15ce6fb00f4d164988\n");
  fprintf(out,"Cycle=Rfi DpCtrldR Fre Fence.rw.rwdWW Rfe DpDatadW Wsi\n");
  fprintf(out,"Relax MP+fence.rw.rw+data-wsi-rfi-ctrl %s %s\n",p_true > 0 ? "Ok" : "No","[Wsi,Rfi]");
  fprintf(out,"Safe=Rfe Fre Fence.rw.rwdWW DpDatadW DpCtrldR\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Fr\n");
  fprintf(out,"Orig=Fence.rw.rwdWW Rfe DpDatadW Wsi Rfi DpCtrldR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0] [1] ; (1,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation MP+fence.rw.rw+data-wsi-rfi-ctrl %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time MP+fence.rw.rw+data-wsi-rfi-ctrl %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "MP+fence.rw.rw+data-wsi-rfi-ctrl: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("MP+fence.rw.rw+data-wsi-rfi-ctrl, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int MP_2B_fence_2E_rw_2E_rw_2B_data_2D_wsi_2D_rfi_2D_ctrl(int argc, char **argv, FILE *out) {
