...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("WRR+2W+fence.rw.rws, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("WRR+2W+fence.rw.rws, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"WRR+2W+fence.rw.rws");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("WRR+2W+fence.rw.rws, global x unstabilized") ;
      }
...
#else
#include "WRR+2W+fence.rw.rws.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/WRR+2W+fence.rw.rws.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV WRR+2W+fence.rw.rws");
  fprintf(out,"%s\n","\"Rfe Fence.rw.rwdRR Fre Fence.rw.rwdWW Wse\"");
  fprintf(out,"%s\n","{");
  fprintf(out,"%s\n"," 0:x5=2; 0:x6=x;");
  fprintf(out,"%s\n"," 1:x6=x; 1:x8=y;");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test WRR+2W+fence.rw.rws Allowed\n");
  ...
  fprintf(out,"Hash=f34440d0976faa1ddfa1dd42407c01fb\n");
  fprintf(out,"Cycle=Rfe Fence.rw.rwdRR Fre Fence.rw.rwdWW Wse\n");
  fprintf(out,"Relax WRR+2W+fence.rw.rws %s %s\n",p_true > 0 ? "Ok" : "No","");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "WRR+2W+fence.rw.rws: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("WRR+2W+fence.rw.rws, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int WRR_2B_2W_2B_fence_2E_rw_2E_rws(int argc, char **argv, FILE *out) {
