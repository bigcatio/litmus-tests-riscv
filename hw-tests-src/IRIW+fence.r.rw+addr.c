...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("IRIW+fence.r.rw+addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("IRIW+fence.r.rw+addr, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"IRIW+fence.r.rw+addr");
  check_globals(_a);
  int _th_id = _b->th_id;
...
#else
#include "IRIW+fence.r.rw+addr.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/IRIW+fence.r.rw+addr.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV IRIW+fence.r.rw+addr");
  fprintf(out,"%s\n","\"Rfe Fence.r.rwdRR Fre Rfe DpAddrdR Fre\"");
  fprintf(out,"%s\n","{");
  fprintf(out,"%s\n"," 0:x5=1; 0:x6=x;");
  fprintf(out,"%s\n"," 1:x6=x; 1:x8=y;");
  fprintf(out,"%s\n"," 2:x5=1; 2:x6=y;");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test IRIW+fence.r.rw+addr Allowed\n");
  ...
  fprintf(out,"Hash=7a7b55991b0cd31e99b03bc2caa883a9\n");
  fprintf(out,"Cycle=Rfe Fence.r.rwdRR Fre Rfe DpAddrdR Fre\n");
  fprintf(out,"Relax IRIW+fence.r.rw+addr %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"Safe=Rfe Fre Fence.r.rwdRR DpAddrdR\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Fr Rf Fr\n");
  fprintf(out,"Orig=Rfe Fence.r.rwdRR Fre Rfe DpAddrdR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[2, 3] [0, 1] ; (1,2) (3,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation IRIW+fence.r.rw+addr %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time IRIW+fence.r.rw+addr %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "IRIW+fence.r.rw+addr: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("IRIW+fence.r.rw+addr, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int IRIW_2B_fence_2E_r_2E_rw_2B_addr(int argc, char **argv, FILE *out) {
