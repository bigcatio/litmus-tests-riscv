...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("IRRWIW+fence.rw.rw+ctrlfencei, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("IRRWIW+fence.rw.rw+ctrlfencei, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"IRRWIW+fence.rw.rw+ctrlfencei");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("IRRWIW+fence.rw.rw+ctrlfencei, global x unstabilized") ;
      }
...
#else
#include "IRRWIW+fence.rw.rw+ctrlfencei.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/IRRWIW+fence.rw.rw+ctrlfencei.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV IRRWIW+fence.rw.rw+ctrlfencei");
  fprintf(out,"%s\n","\"Rfe Fence.rw.rwdRR Fre Rfe DpCtrlFenceIdW Wse\"");
  fprintf(out,"%s\n","{");
  fprintf(out,"%s\n"," 0:x5=2; 0:x6=x;");
  fprintf(out,"%s\n"," 1:x6=x; 1:x8=y;");
  fprintf(out,"%s\n"," 2:x5=1; 2:x6=y;");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test IRRWIW+fence.rw.rw+ctrlfencei Allowed\n");
  ...
  fprintf(out,"Hash=91eaca8e093770ccb34c9aef89163084\n");
  fprintf(out,"Cycle=Rfe Fence.rw.rwdRR Fre Rfe DpCtrlFenceIdW Wse\n");
  fprintf(out,"Relax IRRWIW+fence.rw.rw+ctrlfencei %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"Safe=Rfe Fre Wse Fence.rw.rwdRR DpCtrlFenceIdW\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Fr Rf Ws\n");
  fprintf(out,"Orig=Rfe Fence.rw.rwdRR Fre Rfe DpCtrlFenceIdW Wse\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[2, 3] [0, 1] ; (1,2)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation IRRWIW+fence.rw.rw+ctrlfencei %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time IRRWIW+fence.rw.rw+ctrlfencei %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "IRRWIW+fence.rw.rw+ctrlfencei: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("IRRWIW+fence.rw.rw+ctrlfencei, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int IRRWIW_2B_fence_2E_rw_2E_rw_2B_ctrlfencei(int argc, char **argv, FILE *out) {
