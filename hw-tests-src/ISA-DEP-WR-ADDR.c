...
inline static int final_cond(int* _out_0_x5,int _out_1_x8,int _x) {
  return ((_x == 1) && (_out_0_x5 == 0)) && (_out_1_x8 == 0);
}
...
static int idx_addr(ctx_t *_a,int _i,void *v_addr) {
  ...
  else if (v_addr == (void *)&(_a->y[_i])) return 1;
  else if (v_addr == (void *)&(_a->x[_i])) return 2;
  else { fatal("ISA-DEP-WR-ADDR, ???"); return -1;}
}

static char *pretty_addr[3] = {"0","y","x",};
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x5=%s; 1:x8=%d; [x]=%d;\n",c,show ? '*' : ':',pretty_addr[o[out_0_x5_f]],(int)o[out_1_x8_f],(int)o[x_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA-DEP-WR-ADDR, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA-DEP-WR-ADDR, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA-DEP-WR-ADDR");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x5],0(%[x11])\n"
"#END _litmus_P0\n"
:[x5] "=&r" (_a->out_0_x5[_i]),[x11] "=&r" (trashed_x11),[x9] "=&r" (trashed_x9),[x8] "=&r" (trashed_x8),[x7] "=&r" (trashed_x7),[x6] "=&r" (trashed_x6)
:"[x5]" (&_a->x[_i]),[x10] "r" (&_a->y[_i])
:"cc","memory"
);
    }
  }
...
"lw %[x8],0(%[x9])\n"
"#END _litmus_P1\n"
:[x8] "=&r" (_a->out_1_x8[_i]),[x5] "=&r" (trashed_x5)
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("ISA-DEP-WR-ADDR, global x unstabilized") ;
      }
      cond = final_ok(final_cond(_out_0_x5_i,_out_1_x8_i,_x_i));
      o[out_0_x5_f] = idx_addr(&ctx,_i,_out_0_x5_i);
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA-DEP-WR-ADDR Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([x]=1 /\\ 0:x5=0 /\\ 1:x8=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=197f26b3f09a9e6df106756a1953b123\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ISA-DEP-WR-ADDR %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA-DEP-WR-ADDR %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA-DEP-WR-ADDR: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("ISA-DEP-WR-ADDR, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int ISA_2D_DEP_2D_WR_2D_ADDR(int argc, char **argv, FILE *out) {
