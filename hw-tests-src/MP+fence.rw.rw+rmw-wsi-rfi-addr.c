...
inline static int final_cond(int _out_1_x6,int _out_1_x9,int _out_1_x11,int _y) {
  switch (_out_1_x6) {
  case 1:
    switch (_out_1_x9) {
    case 3:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x6=%d; 1:x9=%d; 1:x11=%d; [y]=%d;\n",c,show ? '*' : ':',(int)o[out_1_x6_f],(int)o[out_1_x9_f],(int)o[out_1_x11_f],(int)o[y_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("MP+fence.rw.rw+rmw-wsi-rfi-addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("MP+fence.rw.rw+rmw-wsi-rfi-addr, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"MP+fence.rw.rw+rmw-wsi-rfi-addr");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x11],0(%[x13])\n"
"#END _litmus_P1\n"
:[x11] "=&r" (_a->out_1_x11[_i]),[x9] "=&r" (_a->out_1_x9[_i]),[x6] "=&r" (_a->out_1_x6[_i]),[x13] "=&r" (trashed_x13),[x10] "=&r" (trashed_x10),[x8] "=&r" (trashed_x8),[x7] "=&r" (trashed_x7)
:[x5] "r" (&_a->y[_i]),[x12] "r" (&_a->x[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("MP+fence.rw.rw+rmw-wsi-rfi-addr, global y unstabilized") ;
      }
      cond = final_ok(final_cond(_out_1_x6_i,_out_1_x9_i,_out_1_x11_i,_y_i));
...
#else
#include "MP+fence.rw.rw+rmw-wsi-rfi-addr.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test MP+fence.rw.rw+rmw-wsi-rfi-addr Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([y]=3 /\\ 1:x6=1 /\\ 1:x9=3 /\\ 1:x11=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=e3b9f4c2fefeefadf3f26d5868247fd3\n");
  fprintf(out,"Generator=diyone7 (version 7.48+2(dev))\n");
  fprintf(out,"Com=Rf Fr\n");
  fprintf(out,"Orig=Fence.rw.rwdWW Rfe Rmw Wsi Rfi DpAddrdR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0] [1] ; (1,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation MP+fence.rw.rw+rmw-wsi-rfi-addr %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time MP+fence.rw.rw+rmw-wsi-rfi-addr %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "MP+fence.rw.rw+rmw-wsi-rfi-addr: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("MP+fence.rw.rw+rmw-wsi-rfi-addr, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int MP_2B_fence_2E_rw_2E_rw_2B_rmw_2D_wsi_2D_rfi_2D_addr(int argc, char **argv, FILE *out) {
