...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("CO-SBI, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"CO-SBI");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("CO-SBI, global x unstabilized") ;
      }
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test CO-SBI Required\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","forall ([x]=2 /\\ 1:x8=2 /\\ 1:x7=2 /\\ (0:x8=2 /\\ (0:x7=2 \\/ 0:x7=1) \\/ 0:x8=1 /\\ 0:x7=1) \\/ [x]=1 /\\ 0:x8=1 /\\ 0:x7=1 /\\ (1:x8=2 /\\ 1:x7=2 \\/ 1:x8=1 /\\ (1:x7=2 \\/ 1:x7=1)))",cond ? "" : "NOT ");
  fprintf(out,"Hash=64ff848153265345020811e6039201bc\n");
  fprintf(out,"Generator=diyone7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Fr Fr\n");
  fprintf(out,"Orig=Rfi PosRR Fre Rfi PosRR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1] [0] ; (0,1) (1,0)\n");
  }
  count_t cond_true = p_false;
  count_t cond_false = p_true;
  fprintf(out,"Observation CO-SBI %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time CO-SBI %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "CO-SBI: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int CO_2D_SBI(int argc, char **argv, FILE *out) {
