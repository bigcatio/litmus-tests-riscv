...
inline static int final_cond(int _out_0_x10,int* _out_1_x10,void *x) {
  return (_out_0_x10 == 1) && (_out_1_x10 == x);
}
...
static int idx_addr(ctx_t *_a,int _i,void *v_addr) {
  ...
  else { fatal("ISA-LB-DEP-ADDR-SUCCESS, ???"); return -1;}
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("ISA-LB-DEP-ADDR-SUCCESS, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA-LB-DEP-ADDR-SUCCESS, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA-LB-DEP-ADDR-SUCCESS, check_globals failed");
    if (rand_bit(&(_a->seed)) && p[_i] != &(_a->z[_i])) fatal("ISA-LB-DEP-ADDR-SUCCESS, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA-LB-DEP-ADDR-SUCCESS");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      cond = final_ok(final_cond(_out_0_x10_i,_out_1_x10_i,&(ctx.x[_i])));
...
#else
#include "ISA-LB-DEP-ADDR-SUCCESS.h"
#endif
...
static void prelude(FILE *out) {
  ...
  fprintf(out,"%s\n"," int y=0;");
  fprintf(out,"%s\n"," int *p=z;");
  fprintf(out,"%s\n"," 0:x18=x; 0:x8=p; 0:x9=y;");
  fprintf(out,"%s\n"," int *1:x10=0; 1:x8=p; 1:x9=y;");
  fprintf(out,"%s\n","}");
  fprintf(out,"%s\n"," P0             | P1                 ;");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA-LB-DEP-ADDR-SUCCESS Forbidden\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","~exists (0:x10=1 /\\ 1:x10=x)",cond ? "" : "NOT ");
  fprintf(out,"Hash=5aca6105ff01a32bff1dc2f210cb18f3\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ISA-LB-DEP-ADDR-SUCCESS %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA-LB-DEP-ADDR-SUCCESS %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA-LB-DEP-ADDR-SUCCESS: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("ISA-LB-DEP-ADDR-SUCCESS, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int ISA_2D_LB_2D_DEP_2D_ADDR_2D_SUCCESS(int argc, char **argv, FILE *out) {
