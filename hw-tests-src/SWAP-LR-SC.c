...
inline static int filter_cond(int _out_0_x8,int _out_1_x8) {
  switch (_out_0_x8) {
  case 0:
    switch (_out_1_x8) {
    case 0:
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("SWAP-LR-SC, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"SWAP-LR-SC");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("SWAP-LR-SC, global x unstabilized") ;
      }
      if (filter_cond(_out_0_x8_i,_out_1_x8_i)) {
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test SWAP-LR-SC Required\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","forall ([x]=1 /\\ 1:x7=0 /\\ 0:x7=2 \\/ 0:x7=0 /\\ [x]=2 /\\ 1:x7=1)",cond ? "" : "NOT ");
  fprintf(out,"Hash=b854653b4d145793cc8945d971529393\n");
  count_t cond_true = p_false;
  count_t cond_false = p_true;
  fprintf(out,"Observation SWAP-LR-SC %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time SWAP-LR-SC %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "SWAP-LR-SC: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int SWAP_2D_LR_2D_SC(int argc, char **argv, FILE *out) {
