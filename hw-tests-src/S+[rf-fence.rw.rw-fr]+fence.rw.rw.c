...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("S+[rf-fence.rw.rw-fr]+fence.rw.rw, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("S+[rf-fence.rw.rw-fr]+fence.rw.rw, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"S+[rf-fence.rw.rw-fr]+fence.rw.rw");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("S+[rf-fence.rw.rw-fr]+fence.rw.rw, global x unstabilized") ;
      }
...
#else
#include "S+[rf-fence.rw.rw-fr]+fence.rw.rw.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/S+[rf-fence.rw.rw-fr]+fence.rw.rw.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV S+[rf-fence.rw.rw-fr]+fence.rw.rw");
  fprintf(out,"%s\n","\"RfLeave Fence.rw.rwdRR FrBack Rfe Fence.rw.rwdRW Wse\"");
  fprintf(out,"%s\n","{");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test S+[rf-fence.rw.rw-fr]+fence.rw.rw Allowed\n");
  ...
  fprintf(out,"Hash=b470da8dd7b51d5615d01c5bce2e4deb\n");
  fprintf(out,"Cycle=Rfe Fence.rw.rwdRW Wse RfLeave Fence.rw.rwdRR FrBack\n");
  fprintf(out,"Relax S+[rf-fence.rw.rw-fr]+fence.rw.rw %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"Safe=Rfe Wse Fence.rw.rwdRW Fence.rw.rwdRR RfLeave FrBack\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Fr Ws\n");
  fprintf(out,"Orig=RfLeave Fence.rw.rwdRR FrBack Rfe Fence.rw.rwdRW Wse\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0, 1] [2] ; (1,2)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation S+[rf-fence.rw.rw-fr]+fence.rw.rw %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time S+[rf-fence.rw.rw-fr]+fence.rw.rw %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "S+[rf-fence.rw.rw-fr]+fence.rw.rw: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("S+[rf-fence.rw.rw-fr]+fence.rw.rw, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int S_2B__5B_rf_2D_fence_2E_rw_2E_rw_2D_fr_5D__2B_fence_2E_rw_2E_rw(int argc, char **argv, FILE *out) {
