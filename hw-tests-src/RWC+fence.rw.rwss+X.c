...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("RWC+fence.rw.rwss+X, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"RWC+fence.rw.rwss+X");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("RWC+fence.rw.rwss+X, global x unstabilized") ;
      }
...
#else
#include "RWC+fence.rw.rwss+X.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/ATOMICS/CO/RWC+fence.rw.rwss+X.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test RWC+fence.rw.rwss+X Allowed\n");
  ...
  fprintf(out,"Hash=beb459d5d1aa6ff16cf09ee141d04229\n");
  fprintf(out,"Cycle=Fre Fence.rw.rwsWR FrePX RfeXP Fence.rw.rwsRR\n");
  fprintf(out,"Relax RWC+fence.rw.rwss+X %s %s\n",p_true > 0 ? "Ok" : "No","FrePX");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "RWC+fence.rw.rwss+X: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("RWC+fence.rw.rwss+X, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int RWC_2B_fence_2E_rw_2E_rwss_2B_X(int argc, char **argv, FILE *out) {
