...
inline static int final_cond(int _out_1_x5,int _out_1_x8,int _out_1_x11,int _x) {
  switch (_out_1_x5) {
  case 1:
    switch (_out_1_x8) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x5=%d; 1:x8=%d; 1:x11=%d; [x]=%d;\n",c,show ? '*' : ':',(int)o[out_1_x5_f],(int)o[out_1_x8_f],(int)o[out_1_x11_f],(int)o[x_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("MP+fence.w.w+addr-[fr-rf], check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("MP+fence.w.w+addr-[fr-rf], check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"MP+fence.w.w+addr-[fr-rf]");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x11],0(%[x9])\n"
"#END _litmus_P1\n"
:[x11] "=&r" (_a->out_1_x11[_i]),[x8] "=&r" (_a->out_1_x8[_i]),[x5] "=&r" (_a->out_1_x5[_i]),[x10] "=&r" (trashed_x10),[x7] "=&r" (trashed_x7)
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("MP+fence.w.w+addr-[fr-rf], global x unstabilized") ;
      }
      cond = final_ok(final_cond(_out_1_x5_i,_out_1_x8_i,_out_1_x11_i,_x_i));
...
#else
#include "MP+fence.w.w+addr-[fr-rf].h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/MP+fence.w.w+addr-[fr-rf].litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV MP+fence.w.w+addr-[fr-rf]");
  fprintf(out,"%s\n","\"Fence.w.wdWW Rfe DpAddrdR FrLeave RfBack Fre\"");
  fprintf(out,"%s\n","{");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test MP+fence.w.w+addr-[fr-rf] Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([x]=2 /\\ 1:x5=1 /\\ 1:x8=0 /\\ 1:x11=1)",cond ? "" : "NOT ");
  fprintf(out,"Hash=813363da9ae9877adec2c9c12bf99959\n");
  fprintf(out,"Cycle=Rfe DpAddrdR FrLeave RfBack Fre Fence.w.wdWW\n");
  fprintf(out,"Relax MP+fence.w.w+addr-[fr-rf] %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"Safe=Rfe Fre Fence.w.wdWW DpAddrdR FrLeave RfBack\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Fr Rf\n");
  fprintf(out,"Orig=Fence.w.wdWW Rfe DpAddrdR FrLeave RfBack Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[2, 0] [1] ; (1,2)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation MP+fence.w.w+addr-[fr-rf] %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time MP+fence.w.w+addr-[fr-rf] %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "MP+fence.w.w+addr-[fr-rf]: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("MP+fence.w.w+addr-[fr-rf], sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int MP_2B_fence_2E_w_2E_w_2B_addr_2D__5B_fr_2D_rf_5D_(int argc, char **argv, FILE *out) {
