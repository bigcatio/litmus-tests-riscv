...
inline static int final_cond(int _a,int _b,int _x,int _y) {
  switch (_a) {
  case 1:
    switch (_b) {
    case 1:
...
static const int a_f = 0 ;
static const int b_f = 1 ;
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>[a]=%d; [b]=%d; [x]=%d; [y]=%d;\n",c,show ? '*' : ':',(int)o[a_f],(int)o[b_f],(int)o[x_f],(int)o[y_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("LB+data-amoadd-datas, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("LB+data-amoadd-datas, check_globals failed");
    if (rand_bit(&(_a->seed)) && b[_i] != 0) fatal("LB+data-amoadd-datas, check_globals failed");
    if (rand_bit(&(_a->seed)) && a[_i] != 0) fatal("LB+data-amoadd-datas, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"LB+data-amoadd-datas");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sw %[x8],0(%[x9])\n"
"#END _litmus_P0\n"
:[x8] "=&r" (trashed_x8),[x7] "=&r" (trashed_x7),[x6] "=&r" (trashed_x6),[x1] "=&r" (trashed_x1)
:[x2] "r" (&_a->a[_i]),[x5] "r" (&_a->x[_i]),[x9] "r" (&_a->y[_i])
:"cc","memory"
);
    }
  }
...
"#END _litmus_P1\n"
:[x8] "=&r" (trashed_x8),[x7] "=&r" (trashed_x7),[x6] "=&r" (trashed_x6),[x1] "=&r" (trashed_x1)
:[x2] "r" (&_a->b[_i]),[x5] "r" (&_a->y[_i]),[x9] "r" (&_a->x[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_a_i != ctx.cpy_a[_p][_i]) fatal("LB+data-amoadd-datas, global a unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_b_i != ctx.cpy_b[_p][_i]) fatal("LB+data-amoadd-datas, global b unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("LB+data-amoadd-datas, global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("LB+data-amoadd-datas, global y unstabilized") ;
      }
      cond = final_ok(final_cond(_a_i,_b_i,_x_i,_y_i));
      o[a_f] = _a_i;
      o[b_f] = _b_i;
      o[x_f] = _x_i;
      o[y_f] = _y_i;
...
#else
#include "LB+data-amoadd-datas.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LB+data-amoadd-datas Required\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","forall ([x]=1 /\\ [y]=1 /\\ [a]=1 /\\ [b]=1)",cond ? "" : "NOT ");
  fprintf(out,"Hash=d126c7f6529a9f9fd6d332770932da4f\n");
  count_t cond_true = p_false;
  count_t cond_false = p_true;
  fprintf(out,"Observation LB+data-amoadd-datas %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time LB+data-amoadd-datas %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LB+data-amoadd-datas: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("LB+data-amoadd-datas, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int LB_2B_data_2D_amoadd_2D_datas(int argc, char **argv, FILE *out) {
