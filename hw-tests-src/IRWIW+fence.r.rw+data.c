...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("IRWIW+fence.r.rw+data, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("IRWIW+fence.r.rw+data, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"IRWIW+fence.r.rw+data");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("IRWIW+fence.r.rw+data, global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("IRWIW+fence.r.rw+data, global y unstabilized") ;
      }
...
#else
#include "IRWIW+fence.r.rw+data.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/IRWIW+fence.r.rw+data.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV IRWIW+fence.r.rw+data");
  fprintf(out,"%s\n","\"Rfe Fence.r.rwdRW Wse Rfe DpDatadW Wse\"");
  fprintf(out,"%s\n","{");
  fprintf(out,"%s\n"," 0:x5=2; 0:x6=x;");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test IRWIW+fence.r.rw+data Allowed\n");
  ...
  fprintf(out,"Hash=c16a645cf8ed1b43b6cbf66d91ca337c\n");
  fprintf(out,"Cycle=Rfe Fence.r.rwdRW Wse Rfe DpDatadW Wse\n");
  fprintf(out,"Relax IRWIW+fence.r.rw+data %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"Safe=Rfe Wse Fence.r.rwdRW DpDatadW\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Ws Rf Ws\n");
  fprintf(out,"Orig=Rfe Fence.r.rwdRW Wse Rfe DpDatadW Wse\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[2, 3] [0, 1] ; \n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation IRWIW+fence.r.rw+data %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time IRWIW+fence.r.rw+data %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "IRWIW+fence.r.rw+data: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("IRWIW+fence.r.rw+data, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int IRWIW_2B_fence_2E_r_2E_rw_2B_data(int argc, char **argv, FILE *out) {
