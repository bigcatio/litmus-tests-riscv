...
inline static int final_cond(int _out_1_x5,int _out_1_x9,int _x,int _y) {
  switch (_out_1_x5) {
  case 2:
    switch (_out_1_x9) {
    case 1:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x5=%d; 1:x9=%d; [x]=%d; [y]=%d;\n",c,show ? '*' : ':',(int)o[out_1_x5_f],(int)o[out_1_x9_f],(int)o[x_f],(int)o[y_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("Luc01+BIS, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("Luc01+BIS, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"Luc01+BIS");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"ori %[x5],x0,2\n"
"#_litmus_P0_1\n\t"
"amoswap.w.aqrl %[x1],%[x5],(%[x6])\n"
"#END _litmus_P0\n"
:[x5] "=&r" (trashed_x5),[x1] "=&r" (trashed_x1)
:[x6] "r" (&_a->x[_i])
:"cc","memory"
);
    }
...
"#START _litmus_P1\n"
"#_litmus_P1_0\n\t"
"amoor.w.aqrl %[x5],%[x1],(%[x6])\n"
"#_litmus_P1_1\n\t"
"ori %[x7],x0,1\n"
"#_litmus_P1_2\n\t"
"sw %[x7],0(%[x8])\n"
"#_litmus_P1_3\n\t"
"amoor.w.aqrl %[x9],%[x1],(%[x8])\n"
"#END _litmus_P1\n"
:[x9] "=&r" (_a->out_1_x9[_i]),[x5] "=&r" (_a->out_1_x5[_i]),[x7] "=&r" (trashed_x7)
:[x1] "r" ((int)(0)),[x6] "r" (&_a->x[_i]),[x8] "r" (&_a->y[_i])
:"cc","memory"
);
    }
  }
...
"ori %[x7],x0,1\n"
"#_litmus_P2_3\n\t"
"amoswap.w.aqrl %[x2],%[x7],(%[x8])\n"
"#END _litmus_P2\n"
:[x7] "=&r" (trashed_x7),[x5] "=&r" (trashed_x5),[x2] "=&r" (trashed_x2),[x1] "=&r" (trashed_x1)
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("Luc01+BIS, global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("Luc01+BIS, global y unstabilized") ;
      }
      cond = final_ok(final_cond(_out_1_x5_i,_out_1_x9_i,_x_i,_y_i));
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/HAND/Luc01+BIS.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV Luc01+BIS");
  fprintf(out,"%s\n","{");
  fprintf(out,"%s\n"," 0:x6=x;");
  fprintf(out,"%s\n"," 1:x1=0; 1:x6=x; 1:x8=y;");
  fprintf(out,"%s\n"," 2:x6=y; 2:x8=x;");
  fprintf(out,"%s\n","}");
  fprintf(out,"%s\n"," P0                        | P1                      | P2                        ;");
  fprintf(out,"%s\n"," ori x5,x0,2               | amoor.w.aqrl x5,x1,(x6) | ori x5,x0,2               ;");
  fprintf(out,"%s\n"," amoswap.w.aqrl x1,x5,(x6) | ori x7,x0,1             | amoswap.w.aqrl x1,x5,(x6) ;");
  fprintf(out,"%s\n","                           | sw x7,0(x8)             | ori x7,x0,1               ;");
  fprintf(out,"%s\n","                           | amoor.w.aqrl x9,x1,(x8) | amoswap.w.aqrl x2,x7,(x8) ;");
  fprintf(out,"%s\n","");
  fprintf(out,"%s\n","exists ([x]=2 /\\ [y]=2 /\\ 1:x5=2 /\\ 1:x9=1)");
  fprintf(out,"%s\n","Generated assembler");
  ass(out);
}
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test Luc01+BIS Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([x]=2 /\\ [y]=2 /\\ 1:x5=2 /\\ 1:x9=1)",cond ? "" : "NOT ");
  fprintf(out,"Hash=8f85228e54298811006a6bf9c056d3d8\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation Luc01+BIS %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time Luc01+BIS %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "Luc01+BIS: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int Luc01_2B_BIS(int argc, char **argv, FILE *out) {
