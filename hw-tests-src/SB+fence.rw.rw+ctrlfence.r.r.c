...
inline static int final_cond(int _out_0_x7,int _out_1_x7,int _out_1_x8,int _out_1_x12,int _y) {
  switch (_out_0_x7) {
  case 0:
    switch (_out_1_x7) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x7=%d; 1:x7=%d; 1:x8=%d; 1:x12=%d; [y]=%d;\n",c,show ? '*' : ':',(int)o[out_0_x7_f],(int)o[out_1_x7_f],(int)o[out_1_x8_f],(int)o[out_1_x12_f],(int)o[y_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("SB+fence.rw.rw+ctrlfence.r.r, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("SB+fence.rw.rw+ctrlfence.r.r, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"SB+fence.rw.rw+ctrlfence.r.r");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x7],0(%[x8])\n"
"#END _litmus_P0\n"
:[x7] "=&r" (_a->out_0_x7[_i]),[x5] "=&r" (trashed_x5)
...
"lw %[x12],0(%[x13])\n"
"#END _litmus_P1\n"
:[x12] "=&r" (_a->out_1_x12[_i]),[x8] "=&r" (_a->out_1_x8[_i]),[x7] "=&r" (_a->out_1_x7[_i]),[x6] "=&r" (trashed_x6)
:[x5] "r" (&_a->y[_i]),[x13] "r" (&_a->x[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("SB+fence.rw.rw+ctrlfence.r.r, global y unstabilized") ;
      }
      cond = final_ok(final_cond(_out_0_x7_i,_out_1_x7_i,_out_1_x8_i,_out_1_x12_i,_y_i));
...
#else
#include "SB+fence.rw.rw+ctrlfence.r.r.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test SB+fence.rw.rw+ctrlfence.r.r Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([y]=1 /\\ 0:x7=0 /\\ 1:x8=0 /\\ 1:x7=0 /\\ 1:x12=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=8f8d55dcde4eb384a08c9dd7553b0c13\n");
  fprintf(out,"Generator=diyone7 (version 7.47+6(dev))\n");
  fprintf(out,"Com=Fr Fr\n");
  fprintf(out,"Orig=Fence.rw.rwdWR FrePX RfiXP DpCtrldR Fence.r.rdRR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1] [0] ; (0,1) (1,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation SB+fence.rw.rw+ctrlfence.r.r %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time SB+fence.rw.rw+ctrlfence.r.r %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "SB+fence.rw.rw+ctrlfence.r.r: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("SB+fence.rw.rw+ctrlfence.r.r, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int SB_2B_fence_2E_rw_2E_rw_2B_ctrlfence_2E_r_2E_r(int argc, char **argv, FILE *out) {
