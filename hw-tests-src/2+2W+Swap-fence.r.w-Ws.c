...
inline static int final_cond(int _out_0_x1,int _out_1_x1) {
  switch (_out_0_x1) {
  case 2:
    switch (_out_1_x1) {
    case 2:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x1=%d; 1:x1=%d;\n",c,show ? '*' : ':',(int)o[out_0_x1_f],(int)o[out_1_x1_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("2+2W+Swap-fence.r.w-Ws, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("2+2W+Swap-fence.r.w-Ws, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"2+2W+Swap-fence.r.w-Ws");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x1],0(%[x8])\n"
"#END _litmus_P0\n"
:[x1] "=&r" (_a->out_0_x1[_i]),[x7] "=&r" (trashed_x7),[x5] "=&r" (trashed_x5)
...
"lw %[x1],0(%[x8])\n"
"#END _litmus_P1\n"
:[x1] "=&r" (_a->out_1_x1[_i]),[x7] "=&r" (trashed_x7),[x5] "=&r" (trashed_x5)
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      cond = final_ok(final_cond(_out_0_x1_i,_out_1_x1_i));
...
#else
#include "2+2W+Swap-fence.r.w-Ws.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test 2+2W+Swap-fence.r.w-Ws Allowed\n");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "2+2W+Swap-fence.r.w-Ws: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("2+2W+Swap-fence.r.w-Ws, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int _X2_2B_2W_2B_Swap_2D_fence_2E_r_2E_w_2D_Ws(int argc, char **argv, FILE *out) {
