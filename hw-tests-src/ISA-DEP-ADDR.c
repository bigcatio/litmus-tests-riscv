...
inline static int final_cond(int _out_1_x11,int _out_1_x15) {
  switch (_out_1_x11) {
  case 1:
    switch (_out_1_x15) {
    case 0:
...
static const int out_1_x11_f = 0 ;
static const int out_1_x15_f = 1 ;
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x11=%d; 1:x15=%d;\n",c,show ? '*' : ':',(int)o[out_1_x11_f],(int)o[out_1_x15_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA-DEP-ADDR, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA-DEP-ADDR, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA-DEP-ADDR");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"ld %[x15],0(%[x18])\n"
"#END _litmus_P1\n"
:[x15] "=&r" (_a->out_1_x15[_i]),[x11] "=&r" (_a->out_1_x11[_i]),[x18] "=&r" (trashed_x18),[x12] "=&r" (trashed_x12)
:[x8] "r" (&_a->x[_i]),[x9] "r" (&_a->y[_i])
:"cc","memory"
);
    }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      cond = final_ok(final_cond(_out_1_x11_i,_out_1_x15_i));
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA-DEP-ADDR Forbidden\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","~exists (1:x11=1 /\\ 1:x15=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=71a7d5abdbc419e1dbecebc4ecdf8b52\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ISA-DEP-ADDR %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA-DEP-ADDR %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA-DEP-ADDR: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int ISA_2D_DEP_2D_ADDR(int argc, char **argv, FILE *out) {
