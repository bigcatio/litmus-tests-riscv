...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && lock[_i] != 0) fatal("ISA03+SIMPLE+BIS, check_globals failed");
    if (rand_bit(&(_a->seed)) && a[_i] != 0) fatal("ISA03+SIMPLE+BIS, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA03+SIMPLE+BIS");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_a_i != ctx.cpy_a[_p][_i]) fatal("ISA03+SIMPLE+BIS, global a unstabilized") ;
      }
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA03+SIMPLE+BIS Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (not ([a]=2))",cond ? "" : "NOT ");
  fprintf(out,"Hash=01bf4938d5b4cdab1bd650dfc7a4fdf4\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ISA03+SIMPLE+BIS %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA03+SIMPLE+BIS %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA03+SIMPLE+BIS: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int ISA03_2B_SIMPLE_2B_BIS(int argc, char **argv, FILE *out) {
