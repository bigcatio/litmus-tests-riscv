...
inline static int final_cond(int _out_0_x9,int _out_1_x7) {
  switch (_out_0_x9) {
  case 0:
    switch (_out_1_x7) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x2=%d; 0:x9=%d; 1:x7=%d;\n",c,show ? '*' : ':',(int)o[out_0_x2_f],(int)o[out_0_x9_f],(int)o[out_1_x7_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("C-Will02+HEAD, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("C-Will02+HEAD, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("C-Will02+HEAD, check_globals failed");
    if (rand_bit(&(_a->seed)) && ok[_i] != 1) fatal("C-Will02+HEAD, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"C-Will02+HEAD");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x2],0(%[x3])\n"
"#END _litmus_P0\n"
:[x9] "=&r" (_a->out_0_x9[_i]),[x2] "=&r" (_a->out_0_x2[_i]),[x8] "=&r" (trashed_x8),[x5] "=&r" (trashed_x5)
:[x3] "r" (&_a->z[_i]),[x6] "r" (&_a->x[_i]),[x7] "r" (&_a->y[_i])
:"cc","memory"
);
    }
  }
...
"lw %[x7],0(%[x6])\n"
"#END _litmus_P1\n"
:[x7] "=&r" (_a->out_1_x7[_i]),[x1] "=&r" (trashed_x1)
:[x3] "r" (&_a->z[_i]),[x6] "r" (&_a->y[_i])
:"cc","memory"
);
    }
  }
...
static void init(ctx_t *_a) {
  ...
  _a->seed = rand();
  _a->out_0_x2 = malloc_check(size_of_test*sizeof(*(_a->out_0_x2)));
  ...
  _a->ok = malloc_check(size_of_test*sizeof(*(_a->ok)));
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      int _out_0_x2_i = ctx.out_0_x2[_i];
      int _out_0_x9_i = ctx.out_0_x9[_i];
      int _out_1_x7_i = ctx.out_1_x7[_i];
      outcome_t o;
      int cond;

      cond = final_ok(final_cond(_out_0_x9_i,_out_1_x7_i));
      o[out_0_x2_f] = _out_0_x2_i;
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test C-Will02+HEAD Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (0:x9=0 /\\ 1:x7=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=2e871110536e293e85fd831b51b2c42f\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation C-Will02+HEAD %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time C-Will02+HEAD %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "C-Will02+HEAD: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int C_2D_Will02_2B_HEAD(int argc, char **argv, FILE *out) {
