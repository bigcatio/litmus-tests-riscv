...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("ForwardSc, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ForwardSc, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ForwardSc, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ForwardSc");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"0:\n"
"#END _litmus_P1\n"
:[x5] "=&r" (_a->out_1_x5[_i]),[x4] "=&r" (_a->out_1_x4[_i]),[x3] "=&r" (trashed_x3),[x2] "=&r" (trashed_x2),[x1] "=&r" (trashed_x1)
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ForwardSc Allowed\n");
  ...
  fprintf(out,"Hash=23265dd7fce262d6cc48693b5f586a19\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ForwardSc %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ForwardSc %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ForwardSc: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int ForwardSc(int argc, char **argv, FILE *out) {
