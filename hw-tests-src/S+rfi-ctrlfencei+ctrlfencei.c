...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("S+rfi-ctrlfencei+ctrlfencei, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("S+rfi-ctrlfencei+ctrlfencei, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"S+rfi-ctrlfencei+ctrlfencei");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("S+rfi-ctrlfencei+ctrlfencei, global x unstabilized") ;
      }
...
#else
#include "S+rfi-ctrlfencei+ctrlfencei.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test S+rfi-ctrlfencei+ctrlfencei Allowed\n");
  ...
  fprintf(out,"Hash=3b32b528052a1e800397d30ab6bd7aaf\n");
  fprintf(out,"Cycle=Rfi DpCtrlFenceIdW Rfe DpCtrlFenceIdW Wse\n");
  fprintf(out,"Relax S+rfi-ctrlfencei+ctrlfencei %s %s\n",p_true > 0 ? "Ok" : "No","Rfi");
  fprintf(out,"Safe=Rfe Wse DpCtrlFenceIdW\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Ws\n");
  fprintf(out,"Orig=Rfi DpCtrlFenceIdW Rfe DpCtrlFenceIdW Wse\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0] [1] ; \n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation S+rfi-ctrlfencei+ctrlfencei %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time S+rfi-ctrlfencei+ctrlfencei %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "S+rfi-ctrlfencei+ctrlfencei: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("S+rfi-ctrlfencei+ctrlfencei, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int S_2B_rfi_2D_ctrlfencei_2B_ctrlfencei(int argc, char **argv, FILE *out) {
