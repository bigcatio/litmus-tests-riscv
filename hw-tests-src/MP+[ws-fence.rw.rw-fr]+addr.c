...
inline static int final_cond(int _out_1_x7,int _out_2_x5,int _out_2_x8,int _x) {
  switch (_out_1_x7) {
  case 0:
    switch (_out_2_x5) {
    case 1:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x7=%d; 2:x5=%d; 2:x8=%d; [x]=%d;\n",c,show ? '*' : ':',(int)o[out_1_x7_f],(int)o[out_2_x5_f],(int)o[out_2_x8_f],(int)o[x_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("MP+[ws-fence.rw.rw-fr]+addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("MP+[ws-fence.rw.rw-fr]+addr, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"MP+[ws-fence.rw.rw-fr]+addr");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"#END _litmus_P1\n"
:[x7] "=&r" (_a->out_1_x7[_i])
:[x5] "r" ((int)(2)),[x6] "r" (&_a->x[_i]),[x8] "r" (&_a->y[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("MP+[ws-fence.rw.rw-fr]+addr, global x unstabilized") ;
      }
      cond = final_ok(final_cond(_out_1_x7_i,_out_2_x5_i,_out_2_x8_i,_x_i));
...
#else
#include "MP+[ws-fence.rw.rw-fr]+addr.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/MP+[ws-fence.rw.rw-fr]+addr.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV MP+[ws-fence.rw.rw-fr]+addr");
  fprintf(out,"%s\n","\"WsLeave Fence.rw.rwdWR FrBack Rfe DpAddrdR Fre\"");
  fprintf(out,"%s\n","{");
  ...
  fprintf(out,"%s\n"," sw x5,0(x7) | fence rw,rw | xor x7,x5,x5  ;");
  fprintf(out,"%s\n","             | lw x7,0(x8) | add x10,x9,x7 ;");
  fprintf(out,"%s\n","             |             | lw x8,0(x10)  ;");
  fprintf(out,"%s\n","");
  fprintf(out,"%s\n","exists ([x]=2 /\\ 1:x7=0 /\\ 2:x5=1 /\\ 2:x8=0)");
  fprintf(out,"%s\n","Generated assembler");
  ass(out);
}
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test MP+[ws-fence.rw.rw-fr]+addr Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([x]=2 /\\ 1:x7=0 /\\ 2:x5=1 /\\ 2:x8=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=729ab75e6f0ac01dadc40a7c5c6207dc\n");
  fprintf(out,"Cycle=Rfe DpAddrdR Fre WsLeave Fence.rw.rwdWR FrBack\n");
  fprintf(out,"Relax MP+[ws-fence.rw.rw-fr]+addr %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"Safe=Rfe Fre Fence.rw.rwdWR DpAddrdR WsLeave FrBack\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Fr Fr\n");
  fprintf(out,"Orig=WsLeave Fence.rw.rwdWR FrBack Rfe DpAddrdR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0, 1] [2] ; (1,2) (2,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation MP+[ws-fence.rw.rw-fr]+addr %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time MP+[ws-fence.rw.rw-fr]+addr %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "MP+[ws-fence.rw.rw-fr]+addr: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("MP+[ws-fence.rw.rw-fr]+addr, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int MP_2B__5B_ws_2D_fence_2E_rw_2E_rw_2D_fr_5D__2B_addr(int argc, char **argv, FILE *out) {
