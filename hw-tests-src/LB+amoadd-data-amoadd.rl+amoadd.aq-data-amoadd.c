...
inline static int final_cond(int _out_0_x1,int _out_0_x5,int _out_1_x1,int _out_1_x5,int _x,int _y) {
  switch (_out_0_x1) {
  case 1:
    switch (_out_0_x5) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x1=%d; 0:x5=%d; 1:x1=%d; 1:x5=%d; [x]=%d; [y]=%d;\n",c,show ? '*' : ':',(int)o[out_0_x1_f],(int)o[out_0_x5_f],(int)o[out_1_x1_f],(int)o[out_1_x5_f],(int)o[x_f],(int)o[y_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("LB+amoadd-data-amoadd.rl+amoadd.aq-data-amoadd, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("LB+amoadd-data-amoadd.rl+amoadd.aq-data-amoadd, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"LB+amoadd-data-amoadd.rl+amoadd.aq-data-amoadd");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"ori %[x4],%[x4],1\n"
"#_litmus_P0_4\n\t"
"amoadd.w.rl %[x5],%[x4],(%[x6])\n"
"#END _litmus_P0\n"
:[x5] "=&r" (_a->out_0_x5[_i]),[x1] "=&r" (_a->out_0_x1[_i]),[x4] "=&r" (trashed_x4),[x2] "=&r" (trashed_x2)
:[x3] "r" (&_a->x[_i]),[x6] "r" (&_a->y[_i])
:"cc","memory"
);
    }
  }
...
"amoadd.w %[x5],%[x4],(%[x6])\n"
"#END _litmus_P1\n"
:[x5] "=&r" (_a->out_1_x5[_i]),[x1] "=&r" (_a->out_1_x1[_i]),[x4] "=&r" (trashed_x4),[x2] "=&r" (trashed_x2)
:[x3] "r" (&_a->y[_i]),[x6] "r" (&_a->x[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("LB+amoadd-data-amoadd.rl+amoadd.aq-data-amoadd, global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("LB+amoadd-data-amoadd.rl+amoadd.aq-data-amoadd, global y unstabilized") ;
      }
      cond = final_ok(final_cond(_out_0_x1_i,_out_0_x5_i,_out_1_x1_i,_out_1_x5_i,_x_i,_y_i));
...
#else
#include "LB+amoadd-data-amoadd.rl+amoadd.aq-data-amoadd.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LB+amoadd-data-amoadd.rl+amoadd.aq-data-amoadd Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([x]=2 /\\ [y]=2 /\\ 0:x1=1 /\\ 0:x5=0 /\\ 1:x1=1 /\\ 1:x5=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=02c052191eb1538ba909cf6fc7b1ead9\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation LB+amoadd-data-amoadd.rl+amoadd.aq-data-amoadd %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time LB+amoadd-data-amoadd.rl+amoadd.aq-data-amoadd %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LB+amoadd-data-amoadd.rl+amoadd.aq-data-amoadd: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("LB+amoadd-data-amoadd.rl+amoadd.aq-data-amoadd, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int LB_2B_amoadd_2D_data_2D_amoadd_2E_rl_2B_amoadd_2E_aq_2D_data_2D_amoadd(int argc, char **argv, FILE *out) {
