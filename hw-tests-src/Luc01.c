...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("Luc01, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("Luc01, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"Luc01");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("Luc01, global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("Luc01, global y unstabilized") ;
      }
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/HAND/Luc01.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV Luc01");
  fprintf(out,"%s\n","{");
  fprintf(out,"%s\n"," 0:x6=x;");
  ...
  fprintf(out,"%s\n","}");
  fprintf(out,"%s\n"," P0                        | P1                      | P2                        ;");
  fprintf(out,"%s\n"," ori x5,x0,2               | amoor.w.aqrl x5,x0,(x6) | ori x5,x0,2               ;");
  fprintf(out,"%s\n"," amoswap.w.aqrl x0,x5,(x6) | ori x7,x0,1             | amoswap.w.aqrl x0,x5,(x6) ;");
  fprintf(out,"%s\n","                           | sw x7,0(x8)             | ori x7,x0,1               ;");
  fprintf(out,"%s\n","                           | amoor.w.aqrl x9,x0,(x8) | amoswap.w.aqrl x0,x7,(x8) ;");
  fprintf(out,"%s\n","");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test Luc01 Allowed\n");
  ...
  fprintf(out,"Hash=93dac17cc5f32ab8ff2662af2bb66ef0\n");
  fprintf(out,"Orig=RfeARAR PodRWARP RfiPAR FreARAR PodWWARAR WseARAR\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation Luc01 %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time Luc01 %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "Luc01: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int Luc01(int argc, char **argv, FILE *out) {
