...
inline static int final_cond(int _out_0_x5,int _out_0_x8,int _out_1_x5,int _out_1_x8,int _x,int _y) {
  switch (_out_0_x5) {
  case 1:
    switch (_out_0_x8) {
    case 2:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x5=%d; 0:x8=%d; 1:x5=%d; 1:x8=%d; [x]=%d; [y]=%d;\n",c,show ? '*' : ':',(int)o[out_0_x5_f],(int)o[out_0_x8_f],(int)o[out_1_x5_f],(int)o[out_1_x8_f],(int)o[x_f],(int)o[y_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("LB+fri-rfi-addr+fri-rfi-data, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("LB+fri-rfi-addr+fri-rfi-data, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"LB+fri-rfi-addr+fri-rfi-data");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sw %[x10],0(%[x12])\n"
"#END _litmus_P0\n"
:[x8] "=&r" (_a->out_0_x8[_i]),[x5] "=&r" (_a->out_0_x5[_i]),[x12] "=&r" (trashed_x12),[x9] "=&r" (trashed_x9)
:[x6] "r" (&_a->x[_i]),[x7] "r" ((int)(2)),[x10] "r" ((int)(1)),[x11] "r" (&_a->y[_i])
:"cc","memory"
);
    }
  }
...
"sw %[x9],0(%[x10])\n"
"#END _litmus_P1\n"
:[x8] "=&r" (_a->out_1_x8[_i]),[x5] "=&r" (_a->out_1_x5[_i]),[x9] "=&r" (trashed_x9)
:[x6] "r" (&_a->y[_i]),[x7] "r" ((int)(2)),[x10] "r" (&_a->x[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("LB+fri-rfi-addr+fri-rfi-data, global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("LB+fri-rfi-addr+fri-rfi-data, global y unstabilized") ;
      }
      cond = final_ok(final_cond(_out_0_x5_i,_out_0_x8_i,_out_1_x5_i,_out_1_x8_i,_x_i,_y_i));
...
#else
#include "LB+fri-rfi-addr+fri-rfi-data.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LB+fri-rfi-addr+fri-rfi-data Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([x]=2 /\\ [y]=2 /\\ 0:x5=1 /\\ 0:x8=2 /\\ 1:x5=1 /\\ 1:x8=2)",cond ? "" : "NOT ");
  fprintf(out,"Hash=03e2e832aeedba6598733d6f51e382a1\n");
  fprintf(out,"Cycle=Rfi DpAddrdW Rfe Fri Rfi DpDatadW Rfe Fri\n");
  fprintf(out,"Relax LB+fri-rfi-addr+fri-rfi-data %s %s\n",p_true > 0 ? "Ok" : "No","[Fri,Rfi]");
  fprintf(out,"Safe=Rfe DpAddrdW DpDatadW\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Rf\n");
  fprintf(out,"Orig=Fri Rfi DpAddrdW Rfe Fri Rfi DpDatadW Rfe\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1, 0] ; \n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation LB+fri-rfi-addr+fri-rfi-data %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time LB+fri-rfi-addr+fri-rfi-data %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LB+fri-rfi-addr+fri-rfi-data: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("LB+fri-rfi-addr+fri-rfi-data, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int LB_2B_fri_2D_rfi_2D_addr_2B_fri_2D_rfi_2D_data(int argc, char **argv, FILE *out) {
