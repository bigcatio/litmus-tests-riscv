...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA2+pos+addr+fence.rw.rw, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA2+pos+addr+fence.rw.rw, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA2+pos+addr+fence.rw.rw");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("ISA2+pos+addr+fence.rw.rw, global x unstabilized") ;
      }
...
#else
#include "ISA2+pos+addr+fence.rw.rw.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/ISA2+pos+addr+fence.rw.rw.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV ISA2+pos+addr+fence.rw.rw");
  fprintf(out,"%s\n","\"PosWW Rfe DpAddrdW Rfe Fence.rw.rwdRR Fre\"");
  fprintf(out,"%s\n","{");
  ...
  fprintf(out,"%s\n"," sw x7,0(x6) | xor x7,x5,x5  | fence rw,rw ;");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA2+pos+addr+fence.rw.rw Allowed\n");
  ...
  fprintf(out,"Hash=2d065f26c9ddb647870f066238aada50\n");
  fprintf(out,"Cycle=Rfe Fence.rw.rwdRR Fre PosWW Rfe DpAddrdW\n");
  fprintf(out,"Relax ISA2+pos+addr+fence.rw.rw %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"Safe=Rfe Fre PosWW Fence.rw.rwdRR DpAddrdW\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Rf Fr\n");
  fprintf(out,"Orig=PosWW Rfe DpAddrdW Rfe Fence.rw.rwdRR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0, 1] [2] ; (2,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ISA2+pos+addr+fence.rw.rw %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA2+pos+addr+fence.rw.rw %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA2+pos+addr+fence.rw.rw: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("ISA2+pos+addr+fence.rw.rw, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int ISA2_2B_pos_2B_addr_2B_fence_2E_rw_2E_rw(int argc, char **argv, FILE *out) {
