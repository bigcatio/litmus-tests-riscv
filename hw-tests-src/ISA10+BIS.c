...
inline static int final_cond(int _out_1_x10,int _out_1_x11,int _out_1_x12,int _out_1_x13) {
  switch (_out_1_x10) {
  case 1:
    switch (_out_1_x11) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x10=%d; 1:x11=%d; 1:x12=%d; 1:x13=%d;\n",c,show ? '*' : ':',(int)o[out_1_x10_f],(int)o[out_1_x11_f],(int)o[out_1_x12_f],(int)o[out_1_x13_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("ISA10+BIS, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA10+BIS, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA10+BIS, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA10+BIS");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x13],0(%[x8])\n"
"#END _litmus_P1\n"
:[x13] "=&r" (_a->out_1_x13[_i]),[x12] "=&r" (_a->out_1_x12[_i]),[x11] "=&r" (_a->out_1_x11[_i]),[x10] "=&r" (_a->out_1_x10[_i]),[x28] "=&r" (trashed_x28),[x19] "=&r" (trashed_x19),[x8] "=&r" (trashed_x8),[x7] "=&r" (trashed_x7)
:"[x8]" (&_a->x[_i]),[x9] "r" (&_a->y[_i]),[x18] "r" (&_a->z[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      cond = final_ok(final_cond(_out_1_x10_i,_out_1_x11_i,_out_1_x12_i,_out_1_x13_i));
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/HAND/ISA10+BIS.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV ISA10+BIS");
  fprintf(out,"%s\n","{");
  fprintf(out,"%s\n"," int x=0;");
  ...
  fprintf(out,"%s\n"," int z=0;");
  fprintf(out,"%s\n"," 0:x8=x; 0:x9=y;");
  fprintf(out,"%s\n"," 1:x8=x; 1:x9=y; 1:x18=z;");
  fprintf(out,"%s\n"," 2:x18=z;");
  fprintf(out,"%s\n","}");
  fprintf(out,"%s\n"," P0          | P1              | P2           ;");
  fprintf(out,"%s\n"," ori x6,x0,1 | lw x10,0(x9)    | ori x6,x0,1  ;");
  fprintf(out,"%s\n"," sw x6,0(x8) | xor x7,x10,x10  | sw x6,0(x18) ;");
  ...
  fprintf(out,"%s\n","             | lw x13,0(x8)    |              ;");
  fprintf(out,"%s\n","");
  fprintf(out,"%s\n","~exists (1:x10=1 /\\ 1:x13=0 /\\ 1:x11=0 /\\ 1:x12=1)");
  fprintf(out,"%s\n","Generated assembler");
  ass(out);
}
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA10+BIS Forbidden\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","~exists (1:x10=1 /\\ 1:x13=0 /\\ 1:x11=0 /\\ 1:x12=1)",cond ? "" : "NOT ");
  fprintf(out,"Hash=6891d4d517ec84aebea9e4fa481e2119\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ISA10+BIS %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA10+BIS %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA10+BIS: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int ISA10_2B_BIS(int argc, char **argv, FILE *out) {
