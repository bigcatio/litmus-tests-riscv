...
inline static int final_cond(int _out_0_x9,int _out_1_x10,int _z) {
  switch (_out_0_x9) {
  case 0:
    switch (_out_1_x10) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x9=%d; 1:x10=%d; [z]=%d;\n",c,show ? '*' : ':',(int)o[out_0_x9_f],(int)o[out_1_x10_f],(int)o[z_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("SB+po-ctrlfencei+pos-po-ctrlfencei, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("SB+po-ctrlfencei+pos-po-ctrlfencei, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("SB+po-ctrlfencei+pos-po-ctrlfencei, check_globals failed");
    if (rand_bit(&(_a->seed)) && a[_i] != 0) fatal("SB+po-ctrlfencei+pos-po-ctrlfencei, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"SB+po-ctrlfencei+pos-po-ctrlfencei");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x9],0(%[x10])\n"
"#END _litmus_P0\n"
:[x9] "=&r" (_a->out_0_x9[_i]),[x7] "=&r" (trashed_x7)
:[x5] "r" ((int)(1)),[x6] "r" (&_a->x[_i]),[x8] "r" (&_a->y[_i]),[x10] "r" (&_a->z[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_z_i != ctx.cpy_z[_p][_i]) fatal("SB+po-ctrlfencei+pos-po-ctrlfencei, global z unstabilized") ;
      }
      cond = final_ok(final_cond(_out_0_x9_i,_out_1_x10_i,_z_i));
...
#else
#include "SB+po-ctrlfencei+pos-po-ctrlfencei.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test SB+po-ctrlfencei+pos-po-ctrlfencei Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([z]=2 /\\ 0:x9=0 /\\ 1:x10=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=050beffa87307c1428eaa6809f39d62f\n");
  fprintf(out,"Cycle=Fre PosWW PodWR DpCtrlFenceIdR Fre PodWR DpCtrlFenceIdR\n");
  fprintf(out,"Relax SB+po-ctrlfencei+pos-po-ctrlfencei %s %s\n",p_true > 0 ? "Ok" : "No","PodWR");
  fprintf(out,"Safe=Fre PosWW DpCtrlFenceIdR\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Fr Fr\n");
  fprintf(out,"Orig=PodWR DpCtrlFenceIdR Fre PosWW PodWR DpCtrlFenceIdR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1] [0] ; (0,1) (1,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation SB+po-ctrlfencei+pos-po-ctrlfencei %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time SB+po-ctrlfencei+pos-po-ctrlfencei %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "SB+po-ctrlfencei+pos-po-ctrlfencei: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("SB+po-ctrlfencei+pos-po-ctrlfencei, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int SB_2B_po_2D_ctrlfencei_2B_pos_2D_po_2D_ctrlfencei(int argc, char **argv, FILE *out) {
