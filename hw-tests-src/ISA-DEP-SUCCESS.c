...
inline static int final_cond(int _out_0_x10,int _out_0_x12,int _out_1_x13,uint64_t _y) {
  switch (_out_0_x10) {
  case 0:
    return 0;
  case 2:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x10=%d; 0:x12=%d; 1:x13=%d; [y]=%"PRIu64";\n",c,show ? '*' : ':',(int)o[out_0_x10_f],(int)o[out_0_x12_f],(int)o[out_1_x13_f],(uint64_t)o[y_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("ISA-DEP-SUCCESS, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA-DEP-SUCCESS, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA-DEP-SUCCESS, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA-DEP-SUCCESS");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sd %[x12],0(%[x18])\n"
"#END _litmus_P0\n"
:[x12] "=&r" (_a->out_0_x12[_i]),[x10] "=&r" (_a->out_0_x10[_i]),[x11] "=&r" (trashed_x11)
:[x8] "r" (&_a->x[_i]),[x9] "r" (&_a->y[_i]),[x18] "r" (&_a->z[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("ISA-DEP-SUCCESS, global y unstabilized") ;
      }
      cond = final_ok(final_cond(_out_0_x10_i,_out_0_x12_i,_out_1_x13_i,_y_i));
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA-DEP-SUCCESS Forbidden\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","~exists (not (0:x10=0) /\\ not (1:x13=0) /\\ [y]=0 /\\ not (0:x12=0) \\/ 0:x10=2 /\\ 1:x13=2 /\\ [y]=2 /\\ 0:x12=2)",cond ? "" : "NOT ");
  fprintf(out,"Hash=a03f5744cdb07976b8e604cf5c013076\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ISA-DEP-SUCCESS %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA-DEP-SUCCESS %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA-DEP-SUCCESS: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("ISA-DEP-SUCCESS, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int ISA_2D_DEP_2D_SUCCESS(int argc, char **argv, FILE *out) {
