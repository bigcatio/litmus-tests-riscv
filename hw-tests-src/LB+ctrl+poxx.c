...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("LB+ctrl+poxx, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("LB+ctrl+poxx, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"LB+ctrl+poxx");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("LB+ctrl+poxx, global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("LB+ctrl+poxx, global y unstabilized") ;
      }
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LB+ctrl+poxx Allowed\n");
  ...
  fprintf(out,"Hash=03897d2aac91247f727b3c67ff4ea988\n");
  fprintf(out,"Cycle=DpCtrldW RfePX PodRWXX RfeXP\n");
  fprintf(out,"Relax LB+ctrl+poxx %s %s\n",p_true > 0 ? "Ok" : "No","PodRWXX");
  fprintf(out,"Safe=Rfe DpCtrldW\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Rf\n");
  fprintf(out,"Orig=DpCtrldW RfePX PodRWXX RfeXP\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1, 0] ; \n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation LB+ctrl+poxx %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time LB+ctrl+poxx %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LB+ctrl+poxx: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int LB_2B_ctrl_2B_poxx(int argc, char **argv, FILE *out) {
