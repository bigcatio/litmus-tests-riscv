...
inline static int final_cond(int _out_1_x5,int _out_1_x8,int _out_2_x5,int _out_2_x7) {
  switch (_out_1_x5) {
  case 1:
    switch (_out_1_x8) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x5=%d; 1:x8=%d; 2:x5=%d; 2:x7=%d;\n",c,show ? '*' : ':',(int)o[out_1_x5_f],(int)o[out_1_x8_f],(int)o[out_2_x5_f],(int)o[out_2_x7_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("MP+[rf-addr-fr]+fence.rw.rw, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("MP+[rf-addr-fr]+fence.rw.rw, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"MP+[rf-addr-fr]+fence.rw.rw");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      cond = final_ok(final_cond(_out_1_x5_i,_out_1_x8_i,_out_2_x5_i,_out_2_x7_i));
...
#else
#include "MP+[rf-addr-fr]+fence.rw.rw.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/MP+[rf-addr-fr]+fence.rw.rw.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV MP+[rf-addr-fr]+fence.rw.rw");
  fprintf(out,"%s\n","\"RfLeave DpAddrdR FrBack Rfe Fence.rw.rwdRR Fre\"");
  fprintf(out,"%s\n","{");
  ...
  fprintf(out,"%s\n","             | lw x8,0(x10)  |             ;");
  fprintf(out,"%s\n","");
  fprintf(out,"%s\n","exists (1:x5=1 /\\ 1:x8=0 /\\ 2:x5=1 /\\ 2:x7=0)");
  fprintf(out,"%s\n","Generated assembler");
  ass(out);
}
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test MP+[rf-addr-fr]+fence.rw.rw Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (1:x5=1 /\\ 1:x8=0 /\\ 2:x5=1 /\\ 2:x7=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=0e2e09959b1e0427342d48790d0ef504\n");
  fprintf(out,"Cycle=Rfe Fence.rw.rwdRR Fre RfLeave DpAddrdR FrBack\n");
  fprintf(out,"Relax MP+[rf-addr-fr]+fence.rw.rw %s %s\n",p_true > 0 ? "Ok" : "No","");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "MP+[rf-addr-fr]+fence.rw.rw: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("MP+[rf-addr-fr]+fence.rw.rw, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int MP_2B__5B_rf_2D_addr_2D_fr_5D__2B_fence_2E_rw_2E_rw(int argc, char **argv, FILE *out) {
