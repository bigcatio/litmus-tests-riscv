...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("2+2W+poxxs, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("2+2W+poxxs, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"2+2W+poxxs");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("2+2W+poxxs, global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("2+2W+poxxs, global y unstabilized") ;
      }
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test 2+2W+poxxs Allowed\n");
  ...
  fprintf(out,"Hash=10406ef2630808830eaa9112127f6ce1\n");
  fprintf(out,"Cycle=WseXX PodWWXX WseXX PodWWXX\n");
  fprintf(out,"Relax 2+2W+poxxs %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"Safe=PodWW WseXX\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Ws Ws\n");
  fprintf(out,"Orig=PodWWXX WseXX PodWWXX WseXX\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1] [0] ; \n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation 2+2W+poxxs %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time 2+2W+poxxs %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "2+2W+poxxs: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int _X2_2B_2W_2B_poxxs(int argc, char **argv, FILE *out) {
