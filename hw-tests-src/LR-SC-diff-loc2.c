...
inline static int final_cond(int _out_0_x1,int _out_0_x4,int _out_0_x8,int _out_1_x1,int _out_1_x4,int _out_1_x8,int _a,int _b,int _x,int _y,int _z) {
  switch (_out_0_x1) {
  case 0:
    switch (_out_0_x4) {
    case 1:
...
static const int out_0_x4_f = 1 ;
static const int out_0_x8_f = 2 ;
static const int out_1_x1_f = 3 ;
static const int out_1_x4_f = 4 ;
static const int out_1_x8_f = 5 ;
static const int a_f = 6 ;
static const int b_f = 7 ;
static const int x_f = 8 ;
static const int y_f = 9 ;
static const int z_f = 10 ;
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x1=%d; 0:x4=%d; 0:x8=%d; 1:x1=%d; 1:x4=%d; 1:x8=%d; [a]=%d; [b]=%d; [x]=%d; [y]=%d; [z]=%d;\n",c,show ? '*' : ':',(int)o[out_0_x1_f],(int)o[out_0_x4_f],(int)o[out_0_x8_f],(int)o[out_1_x1_f],(int)o[out_1_x4_f],(int)o[out_1_x8_f],(int)o[a_f],(int)o[b_f],(int)o[x_f],(int)o[y_f],(int)o[z_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("LR-SC-diff-loc2, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("LR-SC-diff-loc2, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("LR-SC-diff-loc2, check_globals failed");
    if (rand_bit(&(_a->seed)) && b[_i] != 0) fatal("LR-SC-diff-loc2, check_globals failed");
    if (rand_bit(&(_a->seed)) && a[_i] != 0) fatal("LR-SC-diff-loc2, check_globals failed");
  }
...
static void stabilize_globals(int _id, ctx_t *_a) {
  ...
  for ( ; ; ) {
    ...
    for (int _i = size_of_test-1 ; _i >= 0 && !_found ; _i--) {
      if (cpy_a[_id][_i] != cpy_a[_nxt_id][_i]) { _found = 1; }
      if (cpy_b[_id][_i] != cpy_b[_nxt_id][_i]) { _found = 1; }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"LR-SC-diff-loc2");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sc.w %[x8],%[x7],0(%[x9])\n"
"#END _litmus_P0\n"
:[x8] "=&r" (_a->out_0_x8[_i]),[x4] "=&r" (_a->out_0_x4[_i]),[x1] "=&r" (_a->out_0_x1[_i]),[x7] "=&r" (trashed_x7),[x2] "=&r" (trashed_x2)
:[x3] "r" (&_a->a[_i]),[x5] "r" (&_a->b[_i]),[x6] "r" (&_a->x[_i]),[x9] "r" (&_a->y[_i])
:"cc","memory"
);
    }
  }
...
"sc.w %[x8],%[x7],0(%[x9])\n"
"#END _litmus_P1\n"
:[x8] "=&r" (_a->out_1_x8[_i]),[x4] "=&r" (_a->out_1_x4[_i]),[x1] "=&r" (_a->out_1_x1[_i]),[x7] "=&r" (trashed_x7),[x2] "=&r" (trashed_x2)
:[x3] "r" (&_a->b[_i]),[x5] "r" (&_a->a[_i]),[x6] "r" (&_a->x[_i]),[x9] "r" (&_a->z[_i])
:"cc","memory"
);
    }
  }
...
static void init(ctx_t *_a) {
  ...
  _a->out_0_x4 = malloc_check(size_of_test*sizeof(*(_a->out_0_x4)));
  ...
  _a->b = malloc_check(size_of_test*sizeof(*(_a->b)));
  _a->a = malloc_check(size_of_test*sizeof(*(_a->a)));
  _a->fst_barrier = pb_create(N);
  _a->s_or = po_create(N);
  for (int _p = N-1 ; _p >= 0 ; _p--) {
    _a->cpy_a[_p] = malloc_check(size_of_test*sizeof(*(_a->cpy_a[_p])));
    _a->cpy_b[_p] = malloc_check(size_of_test*sizeof(*(_a->cpy_b[_p])));
...
static void finalize(ctx_t *_a) {
  ...
  po_free(_a->s_or);
  for (int _p = N-1 ; _p >= 0 ; _p--) {
    free((void *)_a->cpy_a[_p]);
    free((void *)_a->cpy_b[_p]);
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      int _out_0_x4_i = ctx.out_0_x4[_i];
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_a_i != ctx.cpy_a[_p][_i]) fatal("LR-SC-diff-loc2, global a unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_b_i != ctx.cpy_b[_p][_i]) fatal("LR-SC-diff-loc2, global b unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("LR-SC-diff-loc2, global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("LR-SC-diff-loc2, global y unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_z_i != ctx.cpy_z[_p][_i]) fatal("LR-SC-diff-loc2, global z unstabilized") ;
      }
      cond = final_ok(final_cond(_out_0_x1_i,_out_0_x4_i,_out_0_x8_i,_out_1_x1_i,_out_1_x4_i,_out_1_x8_i,_a_i,_b_i,_x_i,_y_i,_z_i));
      o[out_0_x1_f] = _out_0_x1_i;
      o[out_0_x4_f] = _out_0_x4_i;
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LR-SC-diff-loc2 Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([x]=0 /\\ [y]=1 /\\ [z]=1 /\\ [a]=1 /\\ [b]=1 /\\ 0:x1=0 /\\ 0:x4=1 /\\ 0:x8=0 /\\ 1:x1=0 /\\ 1:x4=1 /\\ 1:x8=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=d56c5c61a11d1cf31c0f661139801b35\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation LR-SC-diff-loc2 %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time LR-SC-diff-loc2 %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LR-SC-diff-loc2: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("LR-SC-diff-loc2, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int LR_2D_SC_2D_diff_2D_loc2(int argc, char **argv, FILE *out) {
