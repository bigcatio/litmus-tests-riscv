...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("R+fence.rw.rw+[rf-ctrl-rf], check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("R+fence.rw.rw+[rf-ctrl-rf], check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"R+fence.rw.rw+[rf-ctrl-rf]");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("R+fence.rw.rw+[rf-ctrl-rf], global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("R+fence.rw.rw+[rf-ctrl-rf], global y unstabilized") ;
      }
...
#else
#include "R+fence.rw.rw+[rf-ctrl-rf].h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/R+fence.rw.rw+[rf-ctrl-rf].litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV R+fence.rw.rw+[rf-ctrl-rf]");
  fprintf(out,"%s\n","\"Fence.rw.rwdWW Wse RfLeave DpCtrldW RfBack Fre\"");
  fprintf(out,"%s\n","{");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test R+fence.rw.rw+[rf-ctrl-rf] Allowed\n");
  ...
  fprintf(out,"Hash=284088a8c4f797b988dba49c87270a35\n");
  fprintf(out,"Cycle=Fre Fence.rw.rwdWW Wse RfLeave DpCtrldW RfBack\n");
  fprintf(out,"Relax R+fence.rw.rw+[rf-ctrl-rf] %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"Safe=Fre Wse Fence.rw.rwdWW DpCtrldW RfLeave RfBack\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Ws Fr Rf\n");
  fprintf(out,"Orig=Fence.rw.rwdWW Wse RfLeave DpCtrldW RfBack Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[2, 0] [1] ; (1,2)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation R+fence.rw.rw+[rf-ctrl-rf] %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time R+fence.rw.rw+[rf-ctrl-rf] %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "R+fence.rw.rw+[rf-ctrl-rf]: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("R+fence.rw.rw+[rf-ctrl-rf], sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int R_2B_fence_2E_rw_2E_rw_2B__5B_rf_2D_ctrl_2D_rf_5D_(int argc, char **argv, FILE *out) {
