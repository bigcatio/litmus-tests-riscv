...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("PPOAA, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("PPOAA, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("PPOAA, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"PPOAA");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x13],0(%[x15])\n"
"#END _litmus_P1\n"
:[x13] "=&r" (_a->out_1_x13[_i]),[x11] "=&r" (_a->out_1_x11[_i]),[x5] "=&r" (_a->out_1_x5[_i]),[x15] "=&r" (trashed_x15),[x12] "=&r" (trashed_x12),[x10] "=&r" (trashed_x10),[x8] "=&r" (trashed_x8),[x7] "=&r" (trashed_x7)
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test PPOAA Allowed\n");
  ...
  fprintf(out,"Hash=c6e27ac0cf171094d6c7f3d2847a4ed7\n");
  ...
  fprintf(out,"Observation PPOAA %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time PPOAA %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "PPOAA: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int PPOAA(int argc, char **argv, FILE *out) {
