...
inline static int final_cond(int _out_1_x5,int _out_1_x8,int _out_1_x10) {
  switch (_out_1_x5) {
  case 1:
    switch (_out_1_x8) {
    case 2:
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA09+BIS, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA09+BIS, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA09+BIS");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x10],0(%[x12])\n"
"#END _litmus_P1\n"
:[x10] "=&r" (_a->out_1_x10[_i]),[x8] "=&r" (_a->out_1_x8[_i]),[x5] "=&r" (_a->out_1_x5[_i]),[x12] "=&r" (trashed_x12),[x9] "=&r" (trashed_x9),[x7] "=&r" (trashed_x7)
:[x6] "r" (&_a->y[_i]),[x11] "r" (&_a->x[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("ISA09+BIS, global y unstabilized") ;
      }
      cond = final_ok(final_cond(_out_1_x5_i,_out_1_x8_i,_out_1_x10_i));
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/HAND/ISA09+BIS.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV ISA09+BIS");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA09+BIS Allowed\n");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA09+BIS: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int ISA09_2B_BIS(int argc, char **argv, FILE *out) {
