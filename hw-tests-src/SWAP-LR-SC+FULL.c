...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("SWAP-LR-SC+FULL, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"SWAP-LR-SC+FULL");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sc.w %[x8],%[x6],0(%[x5])\n"
"#END _litmus_P0\n"
:[x8] "=&r" (_a->out_0_x8[_i]),[x7] "=&r" (_a->out_0_x7[_i]),[x6] "=&r" (trashed_x6)
:[x5] "r" (&_a->x[_i])
:"cc","memory"
);
    }
...
"sc.w %[x8],%[x6],0(%[x5])\n"
"#END _litmus_P1\n"
:[x8] "=&r" (_a->out_1_x8[_i]),[x7] "=&r" (_a->out_1_x7[_i]),[x6] "=&r" (trashed_x6)
:[x5] "r" (&_a->x[_i])
:"cc","memory"
);
    }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("SWAP-LR-SC+FULL, global x unstabilized") ;
      }
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test SWAP-LR-SC+FULL Required\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","forall ([x]=1 /\\ 1:x7=0 /\\ 0:x7=2 \\/ 0:x7=0 /\\ [x]=2 /\\ 1:x7=1 \\/ not (0:x8=0) \\/ not (1:x8=0))",cond ? "" : "NOT ");
  fprintf(out,"Hash=a8cf3f2bc46a0f7ca32246ac29f09572\n");
  count_t cond_true = p_false;
  count_t cond_false = p_true;
  fprintf(out,"Observation SWAP-LR-SC+FULL %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time SWAP-LR-SC+FULL %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "SWAP-LR-SC+FULL: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("SWAP-LR-SC+FULL, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int SWAP_2D_LR_2D_SC_2B_FULL(int argc, char **argv, FILE *out) {
