...
inline static int final_cond(int _out_1_x5,int _out_2_x5,int _out_2_x8,int _y) {
  switch (_out_1_x5) {
  case 1:
    switch (_out_2_x5) {
    case 2:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x5=%d; 2:x5=%d; 2:x8=%d; [y]=%d;\n",c,show ? '*' : ':',(int)o[out_1_x5_f],(int)o[out_2_x5_f],(int)o[out_2_x8_f],(int)o[y_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("MP+fence.rw.rw-[rf-fr]+addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("MP+fence.rw.rw-[rf-fr]+addr, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"MP+fence.rw.rw-[rf-fr]+addr");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"#END _litmus_P0\n"
:
:[x5] "r" ((int)(1)),[x6] "r" (&_a->x[_i]),[x7] "r" (&_a->y[_i]),[x8] "r" ((int)(2))
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("MP+fence.rw.rw-[rf-fr]+addr, global y unstabilized") ;
      }
      cond = final_ok(final_cond(_out_1_x5_i,_out_2_x5_i,_out_2_x8_i,_y_i));
...
#else
#include "MP+fence.rw.rw-[rf-fr]+addr.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/MP+fence.rw.rw-[rf-fr]+addr.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV MP+fence.rw.rw-[rf-fr]+addr");
  fprintf(out,"%s\n","\"Fence.rw.rwdWW RfLeave FrBack Rfe DpAddrdR Fre\"");
  fprintf(out,"%s\n","{");
  ...
  fprintf(out,"%s\n","");
  fprintf(out,"%s\n","exists ([y]=2 /\\ 1:x5=1 /\\ 2:x5=2 /\\ 2:x8=0)");
  fprintf(out,"%s\n","Generated assembler");
  ass(out);
}
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test MP+fence.rw.rw-[rf-fr]+addr Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([y]=2 /\\ 1:x5=1 /\\ 2:x5=2 /\\ 2:x8=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=e21d604733dd1043b354ef217a77e779\n");
  fprintf(out,"Cycle=Rfe DpAddrdR Fre Fence.rw.rwdWW RfLeave FrBack\n");
  fprintf(out,"Relax MP+fence.rw.rw-[rf-fr]+addr %s %s\n",p_true > 0 ? "Ok" : "No","");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "MP+fence.rw.rw-[rf-fr]+addr: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("MP+fence.rw.rw-[rf-fr]+addr, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int MP_2B_fence_2E_rw_2E_rw_2D__5B_rf_2D_fr_5D__2B_addr(int argc, char **argv, FILE *out) {
