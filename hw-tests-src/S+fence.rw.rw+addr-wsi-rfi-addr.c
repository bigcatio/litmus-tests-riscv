...
inline static int final_cond(int _out_1_x5,int _out_1_x12,int _x,int _z) {
  switch (_out_1_x5) {
  case 1:
    switch (_out_1_x12) {
    case 2:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x5=%d; 1:x12=%d; [x]=%d; [z]=%d;\n",c,show ? '*' : ':',(int)o[out_1_x5_f],(int)o[out_1_x12_f],(int)o[x_f],(int)o[z_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("S+fence.rw.rw+addr-wsi-rfi-addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("S+fence.rw.rw+addr-wsi-rfi-addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("S+fence.rw.rw+addr-wsi-rfi-addr, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"S+fence.rw.rw+addr-wsi-rfi-addr");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sw %[x8],0(%[x15])\n"
"#END _litmus_P1\n"
:[x12] "=&r" (_a->out_1_x12[_i]),[x5] "=&r" (_a->out_1_x5[_i]),[x15] "=&r" (trashed_x15),[x13] "=&r" (trashed_x13),[x10] "=&r" (trashed_x10),[x7] "=&r" (trashed_x7)
:[x6] "r" (&_a->y[_i]),[x8] "r" ((int)(1)),[x9] "r" (&_a->z[_i]),[x11] "r" ((int)(2)),[x14] "r" (&_a->x[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("S+fence.rw.rw+addr-wsi-rfi-addr, global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_z_i != ctx.cpy_z[_p][_i]) fatal("S+fence.rw.rw+addr-wsi-rfi-addr, global z unstabilized") ;
      }
      cond = final_ok(final_cond(_out_1_x5_i,_out_1_x12_i,_x_i,_z_i));
...
#else
#include "S+fence.rw.rw+addr-wsi-rfi-addr.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test S+fence.rw.rw+addr-wsi-rfi-addr Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([x]=2 /\\ [z]=2 /\\ 1:x5=1 /\\ 1:x12=2)",cond ? "" : "NOT ");
  fprintf(out,"Hash=e6428938634fde27d833b3f78dbf5001\n");
  fprintf(out,"Cycle=Rfi DpAddrdW Wse Fence.rw.rwdWW Rfe DpAddrdW Wsi\n");
  fprintf(out,"Relax S+fence.rw.rw+addr-wsi-rfi-addr %s %s\n",p_true > 0 ? "Ok" : "No","[Wsi,Rfi]");
  fprintf(out,"Safe=Rfe Wse Fence.rw.rwdWW DpAddrdW\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Ws\n");
  fprintf(out,"Orig=Fence.rw.rwdWW Rfe DpAddrdW Wsi Rfi DpAddrdW Wse\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0] [1] ; \n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation S+fence.rw.rw+addr-wsi-rfi-addr %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time S+fence.rw.rw+addr-wsi-rfi-addr %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "S+fence.rw.rw+addr-wsi-rfi-addr: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("S+fence.rw.rw+addr-wsi-rfi-addr, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int S_2B_fence_2E_rw_2E_rw_2B_addr_2D_wsi_2D_rfi_2D_addr(int argc, char **argv, FILE *out) {
