...
inline static int final_cond(int _out_0_x9,int _out_0_x10,int _out_1_x7,int _out_1_x8,int _out_1_x11,int _out_1_x12,int _x,int _y) {
  switch (_out_0_x9) {
  case 0:
    switch (_out_0_x10) {
    case 0:
...
static const int out_0_x9_f = 0 ;
static const int out_0_x10_f = 1 ;
static const int out_1_x7_f = 2 ;
static const int out_1_x8_f = 3 ;
static const int out_1_x11_f = 4 ;
static const int out_1_x12_f = 5 ;
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x9=%d; 0:x10=%d; 1:x7=%d; 1:x8=%d; 1:x11=%d; 1:x12=%d; [x]=%d; [y]=%d;\n",c,show ? '*' : ':',(int)o[out_0_x9_f],(int)o[out_0_x10_f],(int)o[out_1_x7_f],(int)o[out_1_x8_f],(int)o[out_1_x11_f],(int)o[out_1_x12_f],(int)o[x_f],(int)o[y_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("2+2W+fence.tsopx+fence.tsoxx, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("2+2W+fence.tsopx+fence.tsoxx, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"2+2W+fence.tsopx+fence.tsoxx");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sc.w %[x10],%[x8],0(%[x7])\n"
"#END _litmus_P0\n"
:[x10] "=&r" (_a->out_0_x10[_i]),[x9] "=&r" (_a->out_0_x9[_i])
:[x5] "r" ((int)(2)),[x6] "r" (&_a->x[_i]),[x7] "r" (&_a->y[_i]),[x8] "r" ((int)(1))
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("2+2W+fence.tsopx+fence.tsoxx, global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("2+2W+fence.tsopx+fence.tsoxx, global y unstabilized") ;
      }
      cond = final_ok(final_cond(_out_0_x9_i,_out_0_x10_i,_out_1_x7_i,_out_1_x8_i,_out_1_x11_i,_out_1_x12_i,_x_i,_y_i));
...
#else
#include "2+2W+fence.tsopx+fence.tsoxx.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test 2+2W+fence.tsopx+fence.tsoxx Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([x]=2 /\\ [y]=2 /\\ 0:x10=0 /\\ 0:x9=0 /\\ 1:x12=0 /\\ 1:x8=0 /\\ 1:x7=1 /\\ 1:x11=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=adee767bfe844fe0ee5e6a178ec7543e\n");
  fprintf(out,"Cycle=Fence.tsodWWPX WseXX Fence.tsodWWXX WseXP\n");
  fprintf(out,"Relax 2+2W+fence.tsopx+fence.tsoxx %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"Safe=Wse Fence.tsodWW X\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Ws Ws\n");
  fprintf(out,"Orig=Fence.tsodWWPX WseXX Fence.tsodWWXX WseXP\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1] [0] ; \n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation 2+2W+fence.tsopx+fence.tsoxx %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time 2+2W+fence.tsopx+fence.tsoxx %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "2+2W+fence.tsopx+fence.tsoxx: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("2+2W+fence.tsopx+fence.tsoxx, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int _X2_2B_2W_2B_fence_2E_tsopx_2B_fence_2E_tsoxx(int argc, char **argv, FILE *out) {
