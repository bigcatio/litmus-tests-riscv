...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("WRC+posxp+fence.rw.rws+X, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"WRC+posxp+fence.rw.rws+X");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("WRC+posxp+fence.rw.rws+X, global x unstabilized") ;
      }
...
#else
#include "WRC+posxp+fence.rw.rws+X.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/ATOMICS/CO/WRC+posxp+fence.rw.rws+X.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV WRC+posxp+fence.rw.rws+X");
  fprintf(out,"%s\n","\"RfeXX PosRWXP Rfe Fence.rw.rwsRR FrePX\"");
  fprintf(out,"%s\n","{");
  fprintf(out,"%s\n"," 0:x5=x; 0:x6=1;");
  fprintf(out,"%s\n"," 1:x5=x; 1:x8=2;");
  fprintf(out,"%s\n"," 2:x6=x;");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test WRC+posxp+fence.rw.rws+X Allowed\n");
  ...
  fprintf(out,"Hash=1bbb2942ddaab91548fd610133e324cf\n");
  fprintf(out,"Cycle=Rfe Fence.rw.rwsRR FrePX RfeXX PosRWXP\n");
  fprintf(out,"Relax WRC+posxp+fence.rw.rws+X %s %s\n",p_true > 0 ? "Ok" : "No","RfeXX");
  fprintf(out,"Safe=Rfe Fre PosRW Fence.rw.rwsRR\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Rf Fr\n");
  fprintf(out,"Orig=RfeXX PosRWXP Rfe Fence.rw.rwsRR FrePX\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0, 1] [2] ; (2,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation WRC+posxp+fence.rw.rws+X %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time WRC+posxp+fence.rw.rws+X %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "WRC+posxp+fence.rw.rws+X: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("WRC+posxp+fence.rw.rws+X, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int WRC_2B_posxp_2B_fence_2E_rw_2E_rws_2B_X(int argc, char **argv, FILE *out) {
