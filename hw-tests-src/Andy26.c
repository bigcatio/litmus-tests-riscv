...
inline static int final_cond(int _out_0_x1,int _out_0_x2,int _out_0_x3,int _out_1_x4) {
  switch (_out_0_x1) {
  case 0:
    switch (_out_0_x2) {
    case 1:
...
static const int out_0_x2_f = 1 ;
static const int out_0_x3_f = 2 ;
static const int out_1_x4_f = 3 ;
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x1=%d; 0:x2=%d; 0:x3=%d; 1:x4=%d;\n",c,show ? '*' : ':',(int)o[out_0_x1_f],(int)o[out_0_x2_f],(int)o[out_0_x3_f],(int)o[out_1_x4_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && C[_i] != 0) fatal("Andy26, check_globals failed");
    if (rand_bit(&(_a->seed)) && B[_i] != 0) fatal("Andy26, check_globals failed");
    if (rand_bit(&(_a->seed)) && A[_i] != 0) fatal("Andy26, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"Andy26");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sw %[x4],0(%[x9])\n"
"#END _litmus_P0\n"
:[x3] "=&r" (_a->out_0_x3[_i]),[x2] "=&r" (_a->out_0_x2[_i]),[x1] "=&r" (_a->out_0_x1[_i]),[x4] "=&r" (trashed_x4)
:[x7] "r" (&_a->A[_i]),[x8] "r" (&_a->B[_i]),[x9] "r" (&_a->C[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      cond = final_ok(final_cond(_out_0_x1_i,_out_0_x2_i,_out_0_x3_i,_out_1_x4_i));
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test Andy26 Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (0:x1=0 /\\ 0:x2=1 /\\ 0:x3=0 /\\ 1:x4=1)",cond ? "" : "NOT ");
  fprintf(out,"Hash=f47bcd4672da5738b824adcecf0d3b3b\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation Andy26 %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time Andy26 %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "Andy26: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int Andy26(int argc, char **argv, FILE *out) {
