...
inline static int final_cond(int _out_1_x5,int _out_1_x12,int _x) {
  switch (_out_1_x5) {
  case 1:
    switch (_out_1_x12) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x5=%d; 1:x12=%d; [x]=%d;\n",c,show ? '*' : ':',(int)o[out_1_x5_f],(int)o[out_1_x12_f],(int)o[x_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("ISA15, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA15, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA15, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA15");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x12],0(%[x11])\n"
"#END _litmus_P1\n"
:[x12] "=&r" (_a->out_1_x12[_i]),[x5] "=&r" (_a->out_1_x5[_i]),[x11] "=&r" (trashed_x11),[x10] "=&r" (trashed_x10),[x9] "=&r" (trashed_x9)
:[x6] "r" (&_a->y[_i]),[x8] "r" (&_a->z[_i]),[x20] "r" (&_a->x[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("ISA15, global x unstabilized") ;
      }
      cond = final_ok(final_cond(_out_1_x5_i,_out_1_x12_i,_x_i));
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA15 Allowed\n");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA15: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int ISA15(int argc, char **argv, FILE *out) {
