...
inline static int final_cond(int _out_0_x10,int _out_0_x11,int _out_1_x10,int _out_1_x11,int _x,int _y) {
  switch (_out_0_x10) {
  case 1:
    switch (_out_0_x11) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x10=%d; 0:x11=%d; 1:x10=%d; 1:x11=%d; [x]=%d; [y]=%d;\n",c,show ? '*' : ':',(int)o[out_0_x10_f],(int)o[out_0_x11_f],(int)o[out_1_x10_f],(int)o[out_1_x11_f],(int)o[x_f],(int)o[y_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("2+2Swap+Acqs, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("2+2Swap+Acqs, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"2+2Swap+Acqs");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"ori %[x5],x0,2\n"
"#_litmus_P0_1\n\t"
"amoswap.w.aq %[x10],%[x5],(%[x6])\n"
"#_litmus_P0_2\n\t"
"ori %[x7],x0,1\n"
"#_litmus_P0_3\n\t"
"amoswap.w %[x11],%[x7],(%[x8])\n"
"#END _litmus_P0\n"
:[x11] "=&r" (_a->out_0_x11[_i]),[x10] "=&r" (_a->out_0_x10[_i]),[x7] "=&r" (trashed_x7),[x5] "=&r" (trashed_x5)
...
"amoswap.w %[x11],%[x7],(%[x8])\n"
"#END _litmus_P1\n"
:[x11] "=&r" (_a->out_1_x11[_i]),[x10] "=&r" (_a->out_1_x10[_i]),[x7] "=&r" (trashed_x7),[x5] "=&r" (trashed_x5)
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("2+2Swap+Acqs, global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("2+2Swap+Acqs, global y unstabilized") ;
      }
      cond = final_ok(final_cond(_out_0_x10_i,_out_0_x11_i,_out_1_x10_i,_out_1_x11_i,_x_i,_y_i));
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test 2+2Swap+Acqs Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([x]=2 /\\ [y]=2 /\\ 0:x10=1 /\\ 0:x11=0 /\\ 1:x10=1 /\\ 1:x11=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=c2032400db2bb9ba382f95a93f42cffa\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation 2+2Swap+Acqs %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time 2+2Swap+Acqs %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "2+2Swap+Acqs: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int _X2_2B_2Swap_2B_Acqs(int argc, char **argv, FILE *out) {
