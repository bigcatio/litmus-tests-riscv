...
inline static int final_cond(int _out_1_x6,int _out_1_x7,int _out_1_x10,int _out_1_x12,int _y) {
  switch (_out_1_x6) {
  case 1:
    switch (_out_1_x7) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x6=%d; 1:x7=%d; 1:x10=%d; 1:x12=%d; [y]=%d;\n",c,show ? '*' : ':',(int)o[out_1_x6_f],(int)o[out_1_x7_f],(int)o[out_1_x10_f],(int)o[out_1_x12_f],(int)o[y_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("PPOLDSTLD02, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("PPOLDSTLD02, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("PPOLDSTLD02, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"PPOLDSTLD02");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x12],0(%[x14])\n"
"#END _litmus_P1\n"
:[x12] "=&r" (_a->out_1_x12[_i]),[x10] "=&r" (_a->out_1_x10[_i]),[x7] "=&r" (_a->out_1_x7[_i]),[x6] "=&r" (_a->out_1_x6[_i]),[x14] "=&r" (trashed_x14),[x11] "=&r" (trashed_x11),[x8] "=&r" (trashed_x8)
:[x5] "r" (&_a->y[_i]),[x9] "r" (&_a->z[_i]),[x13] "r" (&_a->x[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("PPOLDSTLD02, global y unstabilized") ;
      }
      cond = final_ok(final_cond(_out_1_x6_i,_out_1_x7_i,_out_1_x10_i,_out_1_x12_i,_y_i));
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test PPOLDSTLD02 Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([y]=1 /\\ 1:x7=0 /\\ 1:x6=1 /\\ 1:x10=1 /\\ 1:x12=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=7a312a0c5ee7061e5c07790db90a3060\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation PPOLDSTLD02 %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time PPOLDSTLD02 %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "PPOLDSTLD02: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int PPOLDSTLD02(int argc, char **argv, FILE *out) {
