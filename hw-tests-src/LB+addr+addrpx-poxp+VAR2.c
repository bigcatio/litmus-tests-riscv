...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("LB+addr+addrpx-poxp+VAR2, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("LB+addr+addrpx-poxp+VAR2, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("LB+addr+addrpx-poxp+VAR2, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"LB+addr+addrpx-poxp+VAR2");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sw %[x8],0(%[x10])\n"
"#END _litmus_P0\n"
:[x5] "=&r" (_a->out_0_x5[_i]),[x10] "=&r" (trashed_x10),[x8] "=&r" (trashed_x8),[x7] "=&r" (trashed_x7)
...
"sw %[x12],0(%[x13])\n"
"#END _litmus_P1\n"
:[x11] "=&r" (_a->out_1_x11[_i]),[x5] "=&r" (_a->out_1_x5[_i]),[x12] "=&r" (trashed_x12),[x10] "=&r" (trashed_x10),[x9] "=&r" (trashed_x9),[x7] "=&r" (trashed_x7),[x4] "=&r" (trashed_x4)
...
#else
#include "LB+addr+addrpx-poxp+VAR2.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LB+addr+addrpx-poxp+VAR2 Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (0:x5=1 /\\ 1:x5=1 /\\ not (1:x11=0))",cond ? "" : "NOT ");
  fprintf(out,"Hash=3cf37efaaa2c713e4a4a294b387f5dc6\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation LB+addr+addrpx-poxp+VAR2 %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time LB+addr+addrpx-poxp+VAR2 %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LB+addr+addrpx-poxp+VAR2: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("LB+addr+addrpx-poxp+VAR2, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int LB_2B_addr_2B_addrpx_2D_poxp_2B_VAR2(int argc, char **argv, FILE *out) {
