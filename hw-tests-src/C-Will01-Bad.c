...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("C-Will01-Bad, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("C-Will01-Bad, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("C-Will01-Bad, check_globals failed");
    if (rand_bit(&(_a->seed)) && ok[_i] != 1) fatal("C-Will01-Bad, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"C-Will01-Bad");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sw %[x11],0(%[x12])\n"
"#END _litmus_P0\n"
:[x11] "=&r" (trashed_x11),[x9] "=&r" (trashed_x9),[x8] "=&r" (trashed_x8),[x5] "=&r" (trashed_x5)
:[x6] "r" (&_a->x[_i]),[x7] "r" (&_a->y[_i]),[x12] "r" (&_a->z[_i])
:"cc","memory"
);
    }
  }
...
#else
#include "C-Will01-Bad.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/HAND/C-Will01-Bad.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV C-Will01-Bad");
  fprintf(out,"%s\n","{");
  fprintf(out,"%s\n"," [ok]=1;");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test C-Will01-Bad Allowed\n");
  ...
  fprintf(out,"Hash=2d7ccb2c8404226d25fc643eee262eef\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation C-Will01-Bad %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time C-Will01-Bad %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "C-Will01-Bad: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int C_2D_Will01_2D_Bad(int argc, char **argv, FILE *out) {
