...
inline static int final_cond(int _out_0_x6,int* _out_1_x9,void *y) {
  return (_out_0_x6 == 1) && (_out_1_x9 == y);
}
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x6=%d; 1:x9=%s;\n",c,show ? '*' : ':',(int)o[out_0_x6_f],pretty_addr[o[out_1_x9_f]]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("ISA16, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA16, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA16, check_globals failed");
    if (rand_bit(&(_a->seed)) && p[_i] != &(_a->z[_i])) fatal("ISA16, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA16");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sd %[x9],0(%[x18])\n"
"#END _litmus_P0\n"
:[x6] "=&r" (_a->out_0_x6[_i]),[x9] "=&r" (trashed_x9),[x7] "=&r" (trashed_x7)
:[x8] "r" (&_a->x[_i]),"[x9]" (&_a->y[_i]),[x18] "r" (&_a->p[_i])
:"cc","memory"
);
    }
  }
...
"sd %[x7],0(%[x8])\n"
"#END _litmus_P1\n"
:[x9] "=&r" (_a->out_1_x9[_i]),[x7] "=&r" (trashed_x7),[x6] "=&r" (trashed_x6)
:[x8] "r" (&_a->x[_i]),[x18] "r" (&_a->p[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      cond = final_ok(final_cond(_out_0_x6_i,_out_1_x9_i,&(ctx.y[_i])));
      o[out_0_x6_f] = _out_0_x6_i;
      o[out_1_x9_f] = idx_addr(&ctx,_i,_out_1_x9_i);
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA16 Forbidden\n");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA16: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int ISA16(int argc, char **argv, FILE *out) {
