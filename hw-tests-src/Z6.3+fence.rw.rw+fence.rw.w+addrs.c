...
inline static int final_cond(int _out_2_x5,int _out_2_x8,int _x,int _y) {
  switch (_out_2_x5) {
  case 1:
    switch (_out_2_x8) {
    case 1:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>2:x5=%d; 2:x8=%d; [x]=%d; [y]=%d;\n",c,show ? '*' : ':',(int)o[out_2_x5_f],(int)o[out_2_x8_f],(int)o[x_f],(int)o[y_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("Z6.3+fence.rw.rw+fence.rw.w+addrs, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("Z6.3+fence.rw.rw+fence.rw.w+addrs, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"Z6.3+fence.rw.rw+fence.rw.w+addrs");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"#END _litmus_P0\n"
:
:[x5] "r" ((int)(2)),[x6] "r" (&_a->x[_i]),[x7] "r" ((int)(1)),[x8] "r" (&_a->y[_i])
:"cc","memory"
);
    }
  }
...
"#END _litmus_P1\n"
:
:[x5] "r" ((int)(2)),[x6] "r" (&_a->y[_i]),[x7] "r" ((int)(1)),[x8] "r" (&_a->x[_i])
:"cc","memory"
);
    }
  }
...
"lw %[x8],0(%[x9])\n"
"#END _litmus_P2\n"
:[x8] "=&r" (_a->out_2_x8[_i]),[x5] "=&r" (_a->out_2_x5[_i]),[x9] "=&r" (trashed_x9),[x7] "=&r" (trashed_x7)
:[x6] "r" (&_a->x[_i])
:"cc","memory"
);
    }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("Z6.3+fence.rw.rw+fence.rw.w+addrs, global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("Z6.3+fence.rw.rw+fence.rw.w+addrs, global y unstabilized") ;
      }
      cond = final_ok(final_cond(_out_2_x5_i,_out_2_x8_i,_x_i,_y_i));
...
#else
#include "Z6.3+fence.rw.rw+fence.rw.w+addrs.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/Z6.3+fence.rw.rw+fence.rw.w+addrs.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV Z6.3+fence.rw.rw+fence.rw.w+addrs");
  fprintf(out,"%s\n","\"Fence.rw.rwdWW Wse Fence.rw.wdWW Rfe DpAddrsR Fre\"");
  fprintf(out,"%s\n","{");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test Z6.3+fence.rw.rw+fence.rw.w+addrs Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([x]=2 /\\ [y]=2 /\\ 2:x5=1 /\\ 2:x8=1)",cond ? "" : "NOT ");
  fprintf(out,"Hash=f17a1de51c2a14e6c5b8fde13ab7ad73\n");
  fprintf(out,"Cycle=Rfe DpAddrsR Fre Fence.rw.rwdWW Wse Fence.rw.wdWW\n");
  fprintf(out,"Relax Z6.3+fence.rw.rw+fence.rw.w+addrs %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"Safe=Rfe Fre Wse Fence.rw.wdWW Fence.rw.rwdWW DpAddrsR\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Ws Rf Fr\n");
  fprintf(out,"Orig=Fence.rw.rwdWW Wse Fence.rw.wdWW Rfe DpAddrsR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1, 2] [0] ; (2,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation Z6.3+fence.rw.rw+fence.rw.w+addrs %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time Z6.3+fence.rw.rw+fence.rw.w+addrs %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "Z6.3+fence.rw.rw+fence.rw.w+addrs: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("Z6.3+fence.rw.rw+fence.rw.w+addrs, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int Z6_2E_3_2B_fence_2E_rw_2E_rw_2B_fence_2E_rw_2E_w_2B_addrs(int argc, char **argv, FILE *out) {
