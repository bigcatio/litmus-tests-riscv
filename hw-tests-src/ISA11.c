...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA11, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA11, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA11");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sd %[x6],0(%[x8])\n"
"#_litmus_P0_3\n\t"
"amoor.d.aq %[x10],%[x7],(%[x8])\n"
"#_litmus_P0_4\n\t"
"sd %[x7],0(%[x9])\n"
"#END _litmus_P0\n"
:[x10] "=&r" (_a->out_0_x10[_i]),[x7] "=&r" (trashed_x7),[x6] "=&r" (trashed_x6)
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("ISA11, global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("ISA11, global y unstabilized") ;
      }
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA11 Allowed\n");
  ...
  fprintf(out,"Hash=9c75b18755bee77b73da486af50f3961\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ISA11 %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA11 %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA11: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int ISA11(int argc, char **argv, FILE *out) {
