...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("CoRR+fence.rw.rwspx+X, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"CoRR+fence.rw.rwspx+X");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("CoRR+fence.rw.rwspx+X, global x unstabilized") ;
      }
...
#else
#include "CoRR+fence.rw.rwspx+X.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test CoRR+fence.rw.rwspx+X Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (not (0:x7=0 /\\ (0:x8=0 /\\ [x]=1 /\\ (1:x5=0 /\\ (1:x7=0 /\\ (1:x8=1 \\/ 1:x8=0) \\/ 1:x7=1 /\\ (1:x8=0 \\/ 1:x8=1)) \\/ 1:x5=1 /\\ 1:x7=1 /\\ (1:x8=0 \\/ 1:x8=1)) \\/ 0:x8=1 /\\ 1:x5=0 /\\ 1:x7=0 /\\ [x]=0 /\\ (1:x8=0 \\/ 1:x8=1))))",cond ? "" : "NOT ");
  fprintf(out,"Hash=8b04781b5e7094946064c9dafa3bd2e7\n");
  fprintf(out,"Cycle=Fence.rw.rwsRRPX FreXX RfeXP\n");
  fprintf(out,"Relax CoRR+fence.rw.rwspx+X %s %s\n",p_true > 0 ? "Ok" : "No","FreXX");
  fprintf(out,"Safe=Rfe Fence.rw.rwsRR\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Fr\n");
  fprintf(out,"Orig=RfeXP Fence.rw.rwsRRPX FreXX\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0] [1] ; (1,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation CoRR+fence.rw.rwspx+X %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time CoRR+fence.rw.rwspx+X %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "CoRR+fence.rw.rwspx+X: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("CoRR+fence.rw.rwspx+X, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int CoRR_2B_fence_2E_rw_2E_rwspx_2B_X(int argc, char **argv, FILE *out) {
