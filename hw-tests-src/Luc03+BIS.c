...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("Luc03+BIS, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("Luc03+BIS, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"Luc03+BIS");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sw %[x7],0(%[x8])\n"
"#END _litmus_P0\n"
:[x7] "=&r" (trashed_x7),[x5] "=&r" (trashed_x5),[x1] "=&r" (trashed_x1)
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("Luc03+BIS, global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("Luc03+BIS, global y unstabilized") ;
      }
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/HAND/Luc03+BIS.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV Luc03+BIS");
  fprintf(out,"%s\n","{");
  fprintf(out,"%s\n"," 0:x6=x; 0:x8=y;");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test Luc03+BIS Allowed\n");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "Luc03+BIS: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int Luc03_2B_BIS(int argc, char **argv, FILE *out) {
