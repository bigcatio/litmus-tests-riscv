...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("MP+[rf-ctrlfencei-fr]+addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("MP+[rf-ctrlfencei-fr]+addr, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"MP+[rf-ctrlfencei-fr]+addr");
  check_globals(_a);
  int _th_id = _b->th_id;
...
#else
#include "MP+[rf-ctrlfencei-fr]+addr.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/MP+[rf-ctrlfencei-fr]+addr.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  ...
  fprintf(out,"%s\n"," sw x5,0(x7) | bne x5,x0,LC00 | xor x7,x5,x5  ;");
  fprintf(out,"%s\n","             | LC00:          | add x10,x9,x7 ;");
  fprintf(out,"%s\n","             | fence.i        | lw x8,0(x10)  ;");
  fprintf(out,"%s\n","             | lw x7,0(x8)    |               ;");
  fprintf(out,"%s\n","");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test MP+[rf-ctrlfencei-fr]+addr Allowed\n");
  ...
  fprintf(out,"Hash=259b1400874e651da6fbed9878845242\n");
  fprintf(out,"Cycle=Rfe DpAddrdR Fre RfLeave DpCtrlFenceIdR FrBack\n");
  fprintf(out,"Relax MP+[rf-ctrlfencei-fr]+addr %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"Safe=Rfe Fre DpAddrdR DpCtrlFenceIdR RfLeave FrBack\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Fr Fr\n");
  fprintf(out,"Orig=RfLeave DpCtrlFenceIdR FrBack Rfe DpAddrdR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0, 1] [2] ; (1,2) (2,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation MP+[rf-ctrlfencei-fr]+addr %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time MP+[rf-ctrlfencei-fr]+addr %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "MP+[rf-ctrlfencei-fr]+addr: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("MP+[rf-ctrlfencei-fr]+addr, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int MP_2B__5B_rf_2D_ctrlfencei_2D_fr_5D__2B_addr(int argc, char **argv, FILE *out) {
