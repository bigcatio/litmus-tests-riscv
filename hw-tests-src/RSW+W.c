...
inline static int final_cond(int _out_1_x5,int _out_1_x13) {
  switch (_out_1_x5) {
  case 1:
    switch (_out_1_x13) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x5=%d; 1:x13=%d;\n",c,show ? '*' : ':',(int)o[out_1_x5_f],(int)o[out_1_x13_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("RSW+W, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("RSW+W, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("RSW+W, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"RSW+W");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sw %[x7],0(%[x8])\n"
"#END _litmus_P0\n"
:[x7] "=&r" (trashed_x7),[x5] "=&r" (trashed_x5),[x3] "=&r" (trashed_x3)
:[x4] "r" (&_a->z[_i]),[x6] "r" (&_a->x[_i]),[x8] "r" (&_a->y[_i])
:"cc","memory"
);
    }
  }
...
"lw %[x13],0(%[x15])\n"
"#END _litmus_P1\n"
:[x13] "=&r" (_a->out_1_x13[_i]),[x5] "=&r" (_a->out_1_x5[_i]),[x15] "=&r" (trashed_x15),[x12] "=&r" (trashed_x12),[x11] "=&r" (trashed_x11),[x10] "=&r" (trashed_x10),[x8] "=&r" (trashed_x8),[x7] "=&r" (trashed_x7)
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      cond = final_ok(final_cond(_out_1_x5_i,_out_1_x13_i));
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test RSW+W Allowed\n");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "RSW+W: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int RSW_2B_W(int argc, char **argv, FILE *out) {
