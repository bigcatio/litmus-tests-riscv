...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && d[_i] != 0) fatal("AMO-FENCE, check_globals failed");
    if (rand_bit(&(_a->seed)) && c[_i] != 0) fatal("AMO-FENCE, check_globals failed");
    if (rand_bit(&(_a->seed)) && b[_i] != 0) fatal("AMO-FENCE, check_globals failed");
    if (rand_bit(&(_a->seed)) && a[_i] != 0) fatal("AMO-FENCE, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"AMO-FENCE");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sw %[x5],0(%[x6])\n"
"#_litmus_P0_2\n\t"
"amoswap.w.aqrl x0,%[x1],(%[x10])\n"
"#_litmus_P0_3\n\t"
"lw %[x7],0(%[x8])\n"
"#END _litmus_P0\n"
:[x7] "=&r" (_a->out_0_x7[_i]),[x5] "=&r" (trashed_x5)
:[x1] "r" (0),[x6] "r" (&_a->a[_i]),[x8] "r" (&_a->b[_i]),[x10] "r" (&_a->c[_i])
:"cc","memory"
);
    }
  }
...
"lw %[x7],0(%[x8])\n"
"#END _litmus_P1\n"
:[x7] "=&r" (_a->out_1_x7[_i]),[x5] "=&r" (trashed_x5)
:[x1] "r" (0),[x6] "r" (&_a->b[_i]),[x8] "r" (&_a->a[_i]),[x10] "r" (&_a->d[_i])
:"cc","memory"
);
    }
  }
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/HAND/AMO-FENCE.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV AMO-FENCE");
  fprintf(out,"%s\n","{");
  ...
  fprintf(out,"%s\n"," amoswap.w.aqrl x0,x1,(x10) | amoswap.w.aqrl x0,x1,(x10) ;");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test AMO-FENCE Forbidden\n");
  ...
  fprintf(out,"Hash=1067b69e237d22fb7cf8514e358a5a42\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation AMO-FENCE %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time AMO-FENCE %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "AMO-FENCE: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int AMO_2D_FENCE(int argc, char **argv, FILE *out) {
