...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("LB+ctrl-rfi-data+ctrlfencei-rfi-ctrl, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("LB+ctrl-rfi-data+ctrlfencei-rfi-ctrl, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("LB+ctrl-rfi-data+ctrlfencei-rfi-ctrl, check_globals failed");
    if (rand_bit(&(_a->seed)) && a[_i] != 0) fatal("LB+ctrl-rfi-data+ctrlfencei-rfi-ctrl, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"LB+ctrl-rfi-data+ctrlfencei-rfi-ctrl");
  check_globals(_a);
  int _th_id = _b->th_id;
...
#else
#include "LB+ctrl-rfi-data+ctrlfencei-rfi-ctrl.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LB+ctrl-rfi-data+ctrlfencei-rfi-ctrl Allowed\n");
  ...
  fprintf(out,"Hash=e10b1658f9268f4beae8d8ec77f43f8d\n");
  fprintf(out,"Cycle=Rfi DpDatadW Rfe DpCtrlFenceIdW Rfi DpCtrldW Rfe DpCtrldW\n");
  fprintf(out,"Relax LB+ctrl-rfi-data+ctrlfencei-rfi-ctrl %s %s\n",p_true > 0 ? "Ok" : "No","Rfi");
  fprintf(out,"Safe=Rfe DpDatadW DpCtrldW DpCtrlFenceIdW\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Rf\n");
  fprintf(out,"Orig=DpCtrldW Rfi DpDatadW Rfe DpCtrlFenceIdW Rfi DpCtrldW Rfe\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1, 0] ; \n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation LB+ctrl-rfi-data+ctrlfencei-rfi-ctrl %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time LB+ctrl-rfi-data+ctrlfencei-rfi-ctrl %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LB+ctrl-rfi-data+ctrlfencei-rfi-ctrl: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("LB+ctrl-rfi-data+ctrlfencei-rfi-ctrl, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int LB_2B_ctrl_2D_rfi_2D_data_2B_ctrlfencei_2D_rfi_2D_ctrl(int argc, char **argv, FILE *out) {
