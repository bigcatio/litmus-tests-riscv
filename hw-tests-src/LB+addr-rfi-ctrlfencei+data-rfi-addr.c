...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("LB+addr-rfi-ctrlfencei+data-rfi-addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("LB+addr-rfi-ctrlfencei+data-rfi-addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("LB+addr-rfi-ctrlfencei+data-rfi-addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && a[_i] != 0) fatal("LB+addr-rfi-ctrlfencei+data-rfi-addr, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"LB+addr-rfi-ctrlfencei+data-rfi-addr");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sw %[x11],0(%[x13])\n"
"#END _litmus_P1\n"
:[x9] "=&r" (_a->out_1_x9[_i]),[x5] "=&r" (_a->out_1_x5[_i]),[x13] "=&r" (trashed_x13),[x10] "=&r" (trashed_x10),[x7] "=&r" (trashed_x7)
:[x6] "r" (&_a->z[_i]),[x8] "r" (&_a->a[_i]),[x11] "r" ((int)(1)),[x12] "r" (&_a->x[_i])
:"cc","memory"
);
    }
  }
...
#else
#include "LB+addr-rfi-ctrlfencei+data-rfi-addr.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LB+addr-rfi-ctrlfencei+data-rfi-addr Allowed\n");
  ...
  fprintf(out,"Hash=9e7a03fc02db7070f112d8fed0a875d6\n");
  fprintf(out,"Cycle=Rfi DpAddrdW Rfe DpAddrdW Rfi DpCtrlFenceIdW Rfe DpDatadW\n");
  fprintf(out,"Relax LB+addr-rfi-ctrlfencei+data-rfi-addr %s %s\n",p_true > 0 ? "Ok" : "No","Rfi");
  fprintf(out,"Safe=Rfe DpAddrdW DpDatadW DpCtrlFenceIdW\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Rf\n");
  fprintf(out,"Orig=DpAddrdW Rfi DpCtrlFenceIdW Rfe DpDatadW Rfi DpAddrdW Rfe\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1, 0] ; \n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation LB+addr-rfi-ctrlfencei+data-rfi-addr %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time LB+addr-rfi-ctrlfencei+data-rfi-addr %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LB+addr-rfi-ctrlfencei+data-rfi-addr: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("LB+addr-rfi-ctrlfencei+data-rfi-addr, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int LB_2B_addr_2D_rfi_2D_ctrlfencei_2B_data_2D_rfi_2D_addr(int argc, char **argv, FILE *out) {
