...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("ISA-DEP-CTRL, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA-DEP-CTRL, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA-DEP-CTRL, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA-DEP-CTRL");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sd %[x6],0(%[x8])\n"
"#END _litmus_P0\n"
:[x10] "=&r" (_a->out_0_x10[_i]),[x6] "=&r" (trashed_x6)
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA-DEP-CTRL Forbidden\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","~exists (0:x10=1 /\\ 1:x10=1)",cond ? "" : "NOT ");
  fprintf(out,"Hash=4b7cc27655fc88b787996b02dcf86e17\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ISA-DEP-CTRL %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA-DEP-CTRL %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA-DEP-CTRL: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int ISA_2D_DEP_2D_CTRL(int argc, char **argv, FILE *out) {
