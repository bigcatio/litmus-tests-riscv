...
inline static int final_cond(int _out_1_x10,int _out_1_x11,int _out_1_x12) {
  switch (_out_1_x10) {
  case 1:
    switch (_out_1_x11) {
    case 2:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x10=%d; 1:x11=%d; 1:x12=%d;\n",c,show ? '*' : ':',(int)o[out_1_x10_f],(int)o[out_1_x11_f],(int)o[out_1_x12_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA-OLD+BIS, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA-OLD+BIS, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA-OLD+BIS");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"amoswap.w %[x10],%[x7],(%[x9])\n"
"#_litmus_P1_2\n\t"
"amoor.w.aq %[x11],x0,(%[x9])\n"
"#_litmus_P1_3\n\t"
"lw %[x12],0(%[x8])\n"
"#END _litmus_P1\n"
:[x12] "=&r" (_a->out_1_x12[_i]),[x11] "=&r" (_a->out_1_x11[_i]),[x10] "=&r" (_a->out_1_x10[_i]),[x7] "=&r" (trashed_x7)
:[x8] "r" (&_a->x[_i]),[x9] "r" (&_a->y[_i])
:"cc","memory"
);
    }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      cond = final_ok(final_cond(_out_1_x10_i,_out_1_x11_i,_out_1_x12_i));
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA-OLD+BIS Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (1:x10=1 /\\ 1:x11=2 /\\ 1:x12=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=f752e383ae008c58229bb934b0d6dbf0\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ISA-OLD+BIS %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA-OLD+BIS %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA-OLD+BIS: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int ISA_2D_OLD_2B_BIS(int argc, char **argv, FILE *out) {
