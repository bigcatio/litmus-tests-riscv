...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("MP+fence.w.w+ctrlfencei-rfi-addrs, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("MP+fence.w.w+ctrlfencei-rfi-addrs, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"MP+fence.w.w+ctrlfencei-rfi-addrs");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("MP+fence.w.w+ctrlfencei-rfi-addrs, global x unstabilized") ;
      }
...
#else
#include "MP+fence.w.w+ctrlfencei-rfi-addrs.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test MP+fence.w.w+ctrlfencei-rfi-addrs Allowed\n");
  ...
  fprintf(out,"Hash=254f6c4b3881f54b3c90e8fae9036668\n");
  fprintf(out,"Cycle=Rfi DpAddrsR Fre Fence.w.wdWW Rfe DpCtrlFenceIdW\n");
  fprintf(out,"Relax MP+fence.w.w+ctrlfencei-rfi-addrs %s %s\n",p_true > 0 ? "Ok" : "No","Rfi");
  ...
  fprintf(out,"Orig=Fence.w.wdWW Rfe DpCtrlFenceIdW Rfi DpAddrsR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0] [1] ; (1,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation MP+fence.w.w+ctrlfencei-rfi-addrs %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time MP+fence.w.w+ctrlfencei-rfi-addrs %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "MP+fence.w.w+ctrlfencei-rfi-addrs: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("MP+fence.w.w+ctrlfencei-rfi-addrs, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int MP_2B_fence_2E_w_2E_w_2B_ctrlfencei_2D_rfi_2D_addrs(int argc, char **argv, FILE *out) {
