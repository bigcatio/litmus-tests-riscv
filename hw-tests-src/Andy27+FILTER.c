...
inline static int filter_cond(int _out_0_x3) {
  switch (_out_0_x3) {
  case 0:
    return 1;
  default:
...
inline static int final_cond(int _out_0_x1,int _out_0_x4,int _out_0_x6,int _out_1_x1) {
  switch (_out_0_x1) {
  case 1:
    switch (_out_0_x4) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x1=%d; 0:x4=%d; 0:x6=%d; 1:x1=%d;\n",c,show ? '*' : ':',(int)o[out_0_x1_f],(int)o[out_0_x4_f],(int)o[out_0_x6_f],(int)o[out_1_x1_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && B[_i] != 0) fatal("Andy27+FILTER, check_globals failed");
    if (rand_bit(&(_a->seed)) && A[_i] != 0) fatal("Andy27+FILTER, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"Andy27+FILTER");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"0:\n"
"#END _litmus_P0\n"
:[x6] "=&r" (_a->out_0_x6[_i]),[x4] "=&r" (_a->out_0_x4[_i]),[x3] "=&r" (_a->out_0_x3[_i]),[x1] "=&r" (_a->out_0_x1[_i]),[x5] "=&r" (trashed_x5),[x2] "=&r" (trashed_x2)
:"[x4]" ((int)(0)),"[x6]" ((int)(0)),[x7] "r" (&_a->A[_i]),[x8] "r" (&_a->B[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      if (filter_cond(_out_0_x3_i)) {
        cond = final_ok(final_cond(_out_0_x1_i,_out_0_x4_i,_out_0_x6_i,_out_1_x1_i));
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test Andy27+FILTER Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (0:x4=0 /\\ 0:x6=0 /\\ 0:x1=1 /\\ 1:x1=1)",cond ? "" : "NOT ");
  fprintf(out,"Hash=fcd48199728de2bde42a5f903073f713\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation Andy27+FILTER %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time Andy27+FILTER %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "Andy27+FILTER: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int Andy27_2B_FILTER(int argc, char **argv, FILE *out) {
