...
inline static int final_cond(int _out_0_x6,int _out_0_x8,int _out_1_x6,int _out_1_x9,int _out_1_x10,int _x) {
  switch (_out_0_x6) {
  case 0:
    switch (_out_0_x8) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x6=%d; 0:x8=%d; 1:x6=%d; 1:x9=%d; 1:x10=%d; [x]=%d;\n",c,show ? '*' : ':',(int)o[out_0_x6_f],(int)o[out_0_x8_f],(int)o[out_1_x6_f],(int)o[out_1_x9_f],(int)o[out_1_x10_f],(int)o[x_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("RR+RR+rmw-fence.tso+rmw-fence.tsopx, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("RR+RR+rmw-fence.tso+rmw-fence.tsopx, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"RR+RR+rmw-fence.tso+rmw-fence.tsopx");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x8],0(%[x9])\n"
"#END _litmus_P0\n"
:[x8] "=&r" (_a->out_0_x8[_i]),[x6] "=&r" (_a->out_0_x6[_i])
:[x5] "r" (&_a->x[_i]),[x7] "r" ((int)(1)),[x9] "r" (&_a->y[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("RR+RR+rmw-fence.tso+rmw-fence.tsopx, global x unstabilized") ;
      }
      cond = final_ok(final_cond(_out_0_x6_i,_out_0_x8_i,_out_1_x6_i,_out_1_x9_i,_out_1_x10_i,_x_i));
...
#else
#include "RR+RR+rmw-fence.tso+rmw-fence.tsopx.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test RR+RR+rmw-fence.tso+rmw-fence.tsopx Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([x]=1 /\\ 0:x6=0 /\\ 0:x8=0 /\\ 1:x10=0 /\\ 1:x6=0 /\\ 1:x9=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=8c9344cd5edd51a0f85084e2fbf20f55\n");
  fprintf(out,"Cycle=Hat Rmw Fence.tsodWRPX HatXP Rmw Fence.tsodWR\n");
  fprintf(out,"Relax RR+RR+rmw-fence.tso+rmw-fence.tsopx %s %s\n",p_true > 0 ? "Ok" : "No","[Rmw,Fence.tsodWR]");
  fprintf(out,"Safe=Hat X\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Fr Fr\n");
  fprintf(out,"Orig=Rmw Fence.tsodWR Hat Rmw Fence.tsodWRPX HatXP\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1] [0] ; (0,1) (1,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation RR+RR+rmw-fence.tso+rmw-fence.tsopx %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time RR+RR+rmw-fence.tso+rmw-fence.tsopx %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "RR+RR+rmw-fence.tso+rmw-fence.tsopx: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("RR+RR+rmw-fence.tso+rmw-fence.tsopx, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int RR_2B_RR_2B_rmw_2D_fence_2E_tso_2B_rmw_2D_fence_2E_tsopx(int argc, char **argv, FILE *out) {
