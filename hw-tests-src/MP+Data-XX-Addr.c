...
inline static int final_cond(int _out_1_x5,int _out_1_x10,int _out_1_x14,int _out_1_x15,int _a,int _z) {
  switch (_out_1_x5) {
  case 1:
    switch (_out_1_x10) {
    case 0:
...
static const int out_1_x15_f = 3 ;
static const int a_f = 4 ;
static const int z_f = 5 ;
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x5=%d; 1:x10=%d; 1:x14=%d; 1:x15=%d; [a]=%d; [z]=%d;\n",c,show ? '*' : ':',(int)o[out_1_x5_f],(int)o[out_1_x10_f],(int)o[out_1_x14_f],(int)o[out_1_x15_f],(int)o[a_f],(int)o[z_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("MP+Data-XX-Addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("MP+Data-XX-Addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("MP+Data-XX-Addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && a[_i] != 0) fatal("MP+Data-XX-Addr, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"MP+Data-XX-Addr");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x15],0(%[x18])\n"
"#END _litmus_P1\n"
:[x15] "=&r" (_a->out_1_x15[_i]),[x14] "=&r" (_a->out_1_x14[_i]),[x10] "=&r" (_a->out_1_x10[_i]),[x5] "=&r" (_a->out_1_x5[_i]),[x18] "=&r" (trashed_x18),[x17] "=&r" (trashed_x17),[x13] "=&r" (trashed_x13),[x12] "=&r" (trashed_x12),[x9] "=&r" (trashed_x9),[x7] "=&r" (trashed_x7)
:[x6] "r" (&_a->y[_i]),[x8] "r" (&_a->z[_i]),[x11] "r" (&_a->a[_i]),[x16] "r" (&_a->x[_i])
:"cc","memory"
);
    }
  }
...
static void init(ctx_t *_a) {
  ...
  _a->out_1_x15 = malloc_check(size_of_test*sizeof(*(_a->out_1_x15)));
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      int _out_1_x15_i = ctx.out_1_x15[_i];
      int _a_i = ctx.a[_i];
      int _z_i = ctx.z[_i];
      outcome_t o;
      int cond;
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_a_i != ctx.cpy_a[_p][_i]) fatal("MP+Data-XX-Addr, global a unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_z_i != ctx.cpy_z[_p][_i]) fatal("MP+Data-XX-Addr, global z unstabilized") ;
      }
      cond = final_ok(final_cond(_out_1_x5_i,_out_1_x10_i,_out_1_x14_i,_out_1_x15_i,_a_i,_z_i));
      o[out_1_x5_f] = _out_1_x5_i;
      o[out_1_x10_f] = _out_1_x10_i;
      o[out_1_x14_f] = _out_1_x14_i;
      o[out_1_x15_f] = _out_1_x15_i;
      o[a_f] = _a_i;
      o[z_f] = _z_i;
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test MP+Data-XX-Addr Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([a]=1 /\\ [z]=1 /\\ 1:x14=0 /\\ 1:x10=0 /\\ 1:x5=1 /\\ 1:x15=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=f9bcc7d1a764da66ca1c2807f17e3671\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation MP+Data-XX-Addr %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time MP+Data-XX-Addr %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "MP+Data-XX-Addr: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("MP+Data-XX-Addr, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int MP_2B_Data_2D_XX_2D_Addr(int argc, char **argv, FILE *out) {
