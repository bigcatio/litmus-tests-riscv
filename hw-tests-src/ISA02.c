...
inline static int final_cond(int _out_0_x10,int _out_0_x11,int _out_1_x12,int _out_1_x13) {
  switch (_out_0_x10) {
  case 1:
    switch (_out_0_x11) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x10=%d; 0:x11=%d; 1:x12=%d; 1:x13=%d;\n",c,show ? '*' : ':',(int)o[out_0_x10_f],(int)o[out_0_x11_f],(int)o[out_1_x12_f],(int)o[out_1_x13_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA02, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA02, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA02");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x11],0(%[x9])\n"
"#END _litmus_P0\n"
:[x11] "=&r" (_a->out_0_x11[_i]),[x10] "=&r" (_a->out_0_x10[_i]),[x6] "=&r" (trashed_x6)
...
"lw %[x13],0(%[x8])\n"
"#END _litmus_P1\n"
:[x13] "=&r" (_a->out_1_x13[_i]),[x12] "=&r" (_a->out_1_x12[_i]),[x6] "=&r" (trashed_x6)
:[x8] "r" (&_a->x[_i]),[x9] "r" (&_a->y[_i])
:"cc","memory"
);
    }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      cond = final_ok(final_cond(_out_0_x10_i,_out_0_x11_i,_out_1_x12_i,_out_1_x13_i));
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA02 Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (0:x10=1 /\\ 0:x11=0 /\\ 1:x12=1 /\\ 1:x13=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=7ae69cc5a3ccf9c5bde7c1b639db18b0\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ISA02 %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA02 %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA02: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int ISA02(int argc, char **argv, FILE *out) {
