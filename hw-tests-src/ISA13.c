...
inline static int final_cond(int _out_1_x11,int _out_1_x12) {
  switch (_out_1_x11) {
  case 1:
    switch (_out_1_x12) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x11=%d; 1:x12=%d;\n",c,show ? '*' : ':',(int)o[out_1_x11_f],(int)o[out_1_x12_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA13, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA13, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA13");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sd %[x5],0(%[x9])\n"
"#END _litmus_P0\n"
:[x10] "=&r" (trashed_x10),[x5] "=&r" (trashed_x5)
...
"#START _litmus_P1\n"
"#_litmus_P1_0\n\t"
"amoor.d.aq %[x11],x0,(%[x9])\n"
"#_litmus_P1_1\n\t"
"ld %[x12],0(%[x8])\n"
"#END _litmus_P1\n"
:[x12] "=&r" (_a->out_1_x12[_i]),[x11] "=&r" (_a->out_1_x11[_i])
:[x8] "r" (&_a->x[_i]),[x9] "r" (&_a->y[_i])
:"cc","memory"
);
    }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      cond = final_ok(final_cond(_out_1_x11_i,_out_1_x12_i));
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA13 Allowed\n");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA13: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int ISA13(int argc, char **argv, FILE *out) {
