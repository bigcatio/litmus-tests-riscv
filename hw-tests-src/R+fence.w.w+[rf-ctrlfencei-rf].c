...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("R+fence.w.w+[rf-ctrlfencei-rf], check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("R+fence.w.w+[rf-ctrlfencei-rf], check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"R+fence.w.w+[rf-ctrlfencei-rf]");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("R+fence.w.w+[rf-ctrlfencei-rf], global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("R+fence.w.w+[rf-ctrlfencei-rf], global y unstabilized") ;
      }
...
#else
#include "R+fence.w.w+[rf-ctrlfencei-rf].h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/R+fence.w.w+[rf-ctrlfencei-rf].litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV R+fence.w.w+[rf-ctrlfencei-rf]");
  fprintf(out,"%s\n","\"Fence.w.wdWW Wse RfLeave DpCtrlFenceIdW RfBack Fre\"");
  fprintf(out,"%s\n","{");
  ...
  fprintf(out,"%s\n"," fence w,w   | lw x7,0(x8) | bne x5,x0,LC00 ;");
  fprintf(out,"%s\n"," sw x7,0(x8) |             | LC00:          ;");
  fprintf(out,"%s\n","             |             | fence.i        ;");
  fprintf(out,"%s\n","             |             | sw x7,0(x8)    ;");
  fprintf(out,"%s\n","");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test R+fence.w.w+[rf-ctrlfencei-rf] Allowed\n");
  ...
  fprintf(out,"Hash=75b63d6b4074520530132cf5fcfa9ce5\n");
  fprintf(out,"Cycle=Fre Fence.w.wdWW Wse RfLeave DpCtrlFenceIdW RfBack\n");
  fprintf(out,"Relax R+fence.w.w+[rf-ctrlfencei-rf] %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"Safe=Fre Wse Fence.w.wdWW DpCtrlFenceIdW RfLeave RfBack\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Ws Fr Rf\n");
  fprintf(out,"Orig=Fence.w.wdWW Wse RfLeave DpCtrlFenceIdW RfBack Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[2, 0] [1] ; (1,2)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation R+fence.w.w+[rf-ctrlfencei-rf] %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time R+fence.w.w+[rf-ctrlfencei-rf] %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "R+fence.w.w+[rf-ctrlfencei-rf]: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("R+fence.w.w+[rf-ctrlfencei-rf], sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int R_2B_fence_2E_w_2E_w_2B__5B_rf_2D_ctrlfencei_2D_rf_5D_(int argc, char **argv, FILE *out) {
