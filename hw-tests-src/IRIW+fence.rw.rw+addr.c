...
inline static int final_cond(int _out_1_x5,int _out_1_x7,int _out_3_x5,int _out_3_x8) {
  switch (_out_1_x5) {
  case 1:
    switch (_out_1_x7) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x5=%d; 1:x7=%d; 3:x5=%d; 3:x8=%d;\n",c,show ? '*' : ':',(int)o[out_1_x5_f],(int)o[out_1_x7_f],(int)o[out_3_x5_f],(int)o[out_3_x8_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("IRIW+fence.rw.rw+addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("IRIW+fence.rw.rw+addr, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"IRIW+fence.rw.rw+addr");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x8],0(%[x10])\n"
"#END _litmus_P3\n"
:[x8] "=&r" (_a->out_3_x8[_i]),[x5] "=&r" (_a->out_3_x5[_i]),[x10] "=&r" (trashed_x10),[x7] "=&r" (trashed_x7)
...
static void init(ctx_t *_a) {
  ...
  _a->out_3_x8 = malloc_check(size_of_test*sizeof(*(_a->out_3_x8)));
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      int _out_3_x8_i = ctx.out_3_x8[_i];
      outcome_t o;
      int cond;

      cond = final_ok(final_cond(_out_1_x5_i,_out_1_x7_i,_out_3_x5_i,_out_3_x8_i));
      o[out_1_x5_f] = _out_1_x5_i;
      o[out_1_x7_f] = _out_1_x7_i;
      o[out_3_x5_f] = _out_3_x5_i;
      o[out_3_x8_f] = _out_3_x8_i;
...
#else
#include "IRIW+fence.rw.rw+addr.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/IRIW+fence.rw.rw+addr.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV IRIW+fence.rw.rw+addr");
  fprintf(out,"%s\n","\"Rfe Fence.rw.rwdRR Fre Rfe DpAddrdR Fre\"");
  fprintf(out,"%s\n","{");
  fprintf(out,"%s\n"," 0:x5=1; 0:x6=x;");
  fprintf(out,"%s\n"," 1:x6=x; 1:x8=y;");
  fprintf(out,"%s\n"," 2:x5=1; 2:x6=y;");
  ...
  fprintf(out,"%s\n","             |             |             | lw x8,0(x10)  ;");
  fprintf(out,"%s\n","");
  fprintf(out,"%s\n","exists (1:x5=1 /\\ 1:x7=0 /\\ 3:x5=1 /\\ 3:x8=0)");
  fprintf(out,"%s\n","Generated assembler");
  ass(out);
}
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test IRIW+fence.rw.rw+addr Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (1:x5=1 /\\ 1:x7=0 /\\ 3:x5=1 /\\ 3:x8=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=e4fdff2d367a95af6c190efa153b7b11\n");
  fprintf(out,"Cycle=Rfe Fence.rw.rwdRR Fre Rfe DpAddrdR Fre\n");
  fprintf(out,"Relax IRIW+fence.rw.rw+addr %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"Safe=Rfe Fre Fence.rw.rwdRR DpAddrdR\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Fr Rf Fr\n");
  fprintf(out,"Orig=Rfe Fence.rw.rwdRR Fre Rfe DpAddrdR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[2, 3] [0, 1] ; (1,2) (3,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation IRIW+fence.rw.rw+addr %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time IRIW+fence.rw.rw+addr %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "IRIW+fence.rw.rw+addr: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("IRIW+fence.rw.rw+addr, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int IRIW_2B_fence_2E_rw_2E_rw_2B_addr(int argc, char **argv, FILE *out) {
