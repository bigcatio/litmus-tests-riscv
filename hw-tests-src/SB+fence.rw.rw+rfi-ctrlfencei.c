...
inline static int final_cond(int _out_0_x7,int _out_1_x7,int _out_1_x8) {
  switch (_out_0_x7) {
  case 0:
    switch (_out_1_x7) {
    case 1:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x7=%d; 1:x7=%d; 1:x8=%d;\n",c,show ? '*' : ':',(int)o[out_0_x7_f],(int)o[out_1_x7_f],(int)o[out_1_x8_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("SB+fence.rw.rw+rfi-ctrlfencei, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("SB+fence.rw.rw+rfi-ctrlfencei, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"SB+fence.rw.rw+rfi-ctrlfencei");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      cond = final_ok(final_cond(_out_0_x7_i,_out_1_x7_i,_out_1_x8_i));
...
#else
#include "SB+fence.rw.rw+rfi-ctrlfencei.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test SB+fence.rw.rw+rfi-ctrlfencei Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (0:x7=0 /\\ 1:x7=1 /\\ 1:x8=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=23491dc8b2887a7919c947db58914cb5\n");
  fprintf(out,"Cycle=Rfi DpCtrlFenceIdR Fre Fence.rw.rwdWR Fre\n");
  fprintf(out,"Relax SB+fence.rw.rw+rfi-ctrlfencei %s %s\n",p_true > 0 ? "Ok" : "No","Rfi");
  fprintf(out,"Safe=Fre Fence.rw.rwdWR DpCtrlFenceIdR\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Fr Fr\n");
  fprintf(out,"Orig=Fence.rw.rwdWR Fre Rfi DpCtrlFenceIdR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1] [0] ; (0,1) (1,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation SB+fence.rw.rw+rfi-ctrlfencei %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time SB+fence.rw.rw+rfi-ctrlfencei %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "SB+fence.rw.rw+rfi-ctrlfencei: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("SB+fence.rw.rw+rfi-ctrlfencei, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int SB_2B_fence_2E_rw_2E_rw_2B_rfi_2D_ctrlfencei(int argc, char **argv, FILE *out) {
