...
inline static int final_cond(int _out_1_x5,int _out_1_x9,int _out_1_x11,int _z) {
  switch (_z) {
  case 1:
    switch (_out_1_x5) {
    case 1:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x5=%d; 1:x9=%d; 1:x11=%d; [z]=%d;\n",c,show ? '*' : ':',(int)o[out_1_x5_f],(int)o[out_1_x9_f],(int)o[out_1_x11_f],(int)o[z_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("ISA14+TER, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA14+TER, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA14+TER, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA14+TER");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_z_i != ctx.cpy_z[_p][_i]) fatal("ISA14+TER, global z unstabilized") ;
      }
      cond = final_ok(final_cond(_out_1_x5_i,_out_1_x9_i,_out_1_x11_i,_z_i));
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/HAND/ISA14+TER.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV ISA14+TER");
  ...
  fprintf(out,"%s\n","             | lw x11,0(x13)   |             ;");
  fprintf(out,"%s\n","");
  fprintf(out,"%s\n","exists (1:x5=1 /\\ 1:x11=0 /\\ ([z]=2 \\/ [z]=1 /\\ 1:x9=1))");
  fprintf(out,"%s\n","Generated assembler");
  ass(out);
}
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA14+TER Allowed\n");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA14+TER: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int ISA14_2B_TER(int argc, char **argv, FILE *out) {
