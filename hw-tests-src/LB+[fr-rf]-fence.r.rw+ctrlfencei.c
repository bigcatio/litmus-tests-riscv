...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("LB+[fr-rf]-fence.r.rw+ctrlfencei, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("LB+[fr-rf]-fence.r.rw+ctrlfencei, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"LB+[fr-rf]-fence.r.rw+ctrlfencei");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("LB+[fr-rf]-fence.r.rw+ctrlfencei, global x unstabilized") ;
      }
...
#else
#include "LB+[fr-rf]-fence.r.rw+ctrlfencei.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/LB+[fr-rf]-fence.r.rw+ctrlfencei.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV LB+[fr-rf]-fence.r.rw+ctrlfencei");
  fprintf(out,"%s\n","\"FrLeave RfBack Fence.r.rwdRW Rfe DpCtrlFenceIdW Rfe\"");
  fprintf(out,"%s\n","{");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LB+[fr-rf]-fence.r.rw+ctrlfencei Allowed\n");
  ...
  fprintf(out,"Hash=aeee378ecd13b8eafb567279256b41cd\n");
  fprintf(out,"Cycle=Rfe DpCtrlFenceIdW Rfe FrLeave RfBack Fence.r.rwdRW\n");
  fprintf(out,"Relax LB+[fr-rf]-fence.r.rw+ctrlfencei %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"Safe=Rfe Fence.r.rwdRW DpCtrlFenceIdW FrLeave RfBack\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Rf Rf\n");
  fprintf(out,"Orig=FrLeave RfBack Fence.r.rwdRW Rfe DpCtrlFenceIdW Rfe\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1, 2] [0] ; \n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation LB+[fr-rf]-fence.r.rw+ctrlfencei %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time LB+[fr-rf]-fence.r.rw+ctrlfencei %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LB+[fr-rf]-fence.r.rw+ctrlfencei: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("LB+[fr-rf]-fence.r.rw+ctrlfencei, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int LB_2B__5B_fr_2D_rf_5D__2D_fence_2E_r_2E_rw_2B_ctrlfencei(int argc, char **argv, FILE *out) {
