...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("RSW, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("RSW, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("RSW, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"RSW");
  check_globals(_a);
  int _th_id = _b->th_id;
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "RSW: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int RSW(int argc, char **argv, FILE *out) {
