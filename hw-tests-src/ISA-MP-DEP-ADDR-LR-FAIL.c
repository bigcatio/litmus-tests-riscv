...
static int idx_addr(ctx_t *_a,int _i,void *v_addr) {
  ...
  else { fatal("ISA-MP-DEP-ADDR-LR-FAIL, ???"); return -1;}
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("ISA-MP-DEP-ADDR-LR-FAIL, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != &(_a->z[_i])) fatal("ISA-MP-DEP-ADDR-LR-FAIL, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA-MP-DEP-ADDR-LR-FAIL, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA-MP-DEP-ADDR-LR-FAIL");
  check_globals(_a);
  int _th_id = _b->th_id;
...
#else
#include "ISA-MP-DEP-ADDR-LR-FAIL.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA-MP-DEP-ADDR-LR-FAIL Allowed\n");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA-MP-DEP-ADDR-LR-FAIL: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("ISA-MP-DEP-ADDR-LR-FAIL, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int ISA_2D_MP_2D_DEP_2D_ADDR_2D_LR_2D_FAIL(int argc, char **argv, FILE *out) {
