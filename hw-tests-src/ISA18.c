...
inline static int final_cond(int _out_1_x6,int* _out_1_x18,void *y) {
  return (_out_1_x18 == y) && (_out_1_x6 == 0);
}
...
static const int out_1_x18_f = 1 ;
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x6=%d; 1:x18=%s;\n",c,show ? '*' : ':',(int)o[out_1_x6_f],pretty_addr[o[out_1_x18_f]]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("ISA18, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA18, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA18, check_globals failed");
    if (rand_bit(&(_a->seed)) && p[_i] != &(_a->z[_i])) fatal("ISA18, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA18");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"#END _litmus_P0\n"
:[x6] "=&r" (trashed_x6)
:[x8] "r" (&_a->x[_i]),[x9] "r" (&_a->p[_i]),[x18] "r" (&_a->y[_i])
:"cc","memory"
);
    }
  }
...
"lw %[x6],0(%[x8])\n"
"#END _litmus_P1\n"
:[x18] "=&r" (_a->out_1_x18[_i]),[x6] "=&r" (_a->out_1_x6[_i])
:[x8] "r" (&_a->x[_i]),[x9] "r" (&_a->p[_i])
:"cc","memory"
);
    }
  }
...
static void init(ctx_t *_a) {
  ...
  _a->out_1_x18 = malloc_check(size_of_test*sizeof(*(_a->out_1_x18)));
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      int* _out_1_x18_i = ctx.out_1_x18[_i];
      outcome_t o;
      int cond;

      cond = final_ok(final_cond(_out_1_x6_i,_out_1_x18_i,&(ctx.y[_i])));
      o[out_1_x6_f] = _out_1_x6_i;
      o[out_1_x18_f] = idx_addr(&ctx,_i,_out_1_x18_i);
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA18 Forbidden\n");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA18: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int ISA18(int argc, char **argv, FILE *out) {
