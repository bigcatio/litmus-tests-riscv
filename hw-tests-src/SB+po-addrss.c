...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("SB+po-addrss, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("SB+po-addrss, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"SB+po-addrss");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x10],0(%[x11])\n"
"#END _litmus_P1\n"
:[x10] "=&r" (_a->out_1_x10[_i]),[x11] "=&r" (trashed_x11),[x9] "=&r" (trashed_x9),[x7] "=&r" (trashed_x7)
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test SB+po-addrss Allowed\n");
  ...
  fprintf(out,"Hash=e926ea2a9acf8996334b61a03b9ebc14\n");
  fprintf(out,"Cycle=Fre PodWR DpAddrsR Fre PodWR DpAddrsR\n");
  fprintf(out,"Relax SB+po-addrss %s %s\n",p_true > 0 ? "Ok" : "No","PodWR");
  fprintf(out,"Safe=Fre DpAddrsR\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Fr Fr\n");
  fprintf(out,"Orig=PodWR DpAddrsR Fre PodWR DpAddrsR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1] [0] ; (0,1) (1,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation SB+po-addrss %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time SB+po-addrss %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "SB+po-addrss: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int SB_2B_po_2D_addrss(int argc, char **argv, FILE *out) {
