...
inline static int final_cond(int _out_0_x10,int* _out_1_x10,int _out_1_x12,void *x) {
  return ((_out_0_x10 == 1) && (_out_1_x10 == x)) && (!(_out_1_x12 == 0));
}
...
static int idx_addr(ctx_t *_a,int _i,void *v_addr) {
  ...
  else if (v_addr == (void *)&(_a->p[_i])) return 4;
  else { fatal("ISA-LB-DEP-ADDR2-SUCCESS, ???"); return -1;}
}

static char *pretty_addr[5] = {"0","z","y","x","p",};
...
static const int out_1_x12_f = 2 ;
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x10=%d; 1:x10=%s; 1:x12=%d;\n",c,show ? '*' : ':',(int)o[out_0_x10_f],pretty_addr[o[out_1_x10_f]],(int)o[out_1_x12_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("ISA-LB-DEP-ADDR2-SUCCESS, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA-LB-DEP-ADDR2-SUCCESS, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA-LB-DEP-ADDR2-SUCCESS, check_globals failed");
    if (rand_bit(&(_a->seed)) && p[_i] != &(_a->z[_i])) fatal("ISA-LB-DEP-ADDR2-SUCCESS, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA-LB-DEP-ADDR2-SUCCESS");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sd %[x18],0(%[x8])\n"
"#END _litmus_P0\n"
:[x10] "=&r" (_a->out_0_x10[_i]),[x18] "=&r" (trashed_x18),[x6] "=&r" (trashed_x6)
:[x8] "r" (&_a->p[_i]),[x9] "r" (&_a->y[_i]),"[x18]" (&_a->x[_i])
:"cc","memory"
);
    }
  }
...
"sw %[x7],0(%[x9])\n"
"#END _litmus_P1\n"
:[x12] "=&r" (_a->out_1_x12[_i]),[x10] "=&r" (_a->out_1_x10[_i]),[x11] "=&r" (trashed_x11),[x7] "=&r" (trashed_x7),[x6] "=&r" (trashed_x6)
:[x8] "r" (&_a->p[_i]),[x9] "r" (&_a->y[_i]),[x18] "r" (&_a->x[_i])
:"cc","memory"
);
    }
  }
...
static void init(ctx_t *_a) {
  ...
  _a->p = malloc_check(size_of_test*sizeof(*(_a->p)));
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      cond = final_ok(final_cond(_out_0_x10_i,_out_1_x10_i,_out_1_x12_i,&(ctx.x[_i])));
      o[out_0_x10_f] = _out_0_x10_i;
      o[out_1_x10_f] = idx_addr(&ctx,_i,_out_1_x10_i);
...
#else
#include "ISA-LB-DEP-ADDR2-SUCCESS.h"
#endif
...
static void prelude(FILE *out) {
  ...
  fprintf(out,"%s\n"," int y=0;");
  fprintf(out,"%s\n"," int *p=z;");
  fprintf(out,"%s\n"," 0:x8=p; 0:x9=y; 0:x18=x;");
  fprintf(out,"%s\n"," int *1:x10=0; 1:x8=p; 1:x9=y; 1:x18=x;");
  fprintf(out,"%s\n","}");
  fprintf(out,"%s\n"," P0             | P1                 ;");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA-LB-DEP-ADDR2-SUCCESS Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (0:x10=1 /\\ 1:x10=x /\\ not (1:x12=0))",cond ? "" : "NOT ");
  fprintf(out,"Hash=85cfd28551276cbfd29ae2b8f9a41b40\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ISA-LB-DEP-ADDR2-SUCCESS %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA-LB-DEP-ADDR2-SUCCESS %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA-LB-DEP-ADDR2-SUCCESS: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("ISA-LB-DEP-ADDR2-SUCCESS, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int ISA_2D_LB_2D_DEP_2D_ADDR2_2D_SUCCESS(int argc, char **argv, FILE *out) {
