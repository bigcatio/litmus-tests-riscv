...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA03+SB01, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA03+SB01, check_globals failed");
    if (rand_bit(&(_a->seed)) && lock[_i] != 0) fatal("ISA03+SB01, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA03+SB01");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"#END _litmus_P0\n"
:[x7] "=&r" (_a->out_0_x7[_i]),[x5] "=&r" (_a->out_0_x5[_i]),[x6] "=&r" (trashed_x6)
:[x10] "r" (&_a->lock[_i]),[x11] "r" (&_a->x[_i]),[x12] "r" (&_a->y[_i])
:"cc","memory"
);
    }
  }
...
"amoswap.w.rl x0,x0,(%[x10])\n"
"#END _litmus_P1\n"
:[x7] "=&r" (_a->out_1_x7[_i]),[x5] "=&r" (_a->out_1_x5[_i]),[x6] "=&r" (trashed_x6)
:[x10] "r" (&_a->lock[_i]),[x11] "r" (&_a->x[_i]),[x12] "r" (&_a->y[_i])
:"cc","memory"
);
    }
...
static void prelude(FILE *out) {
  ...
  fprintf(out,"%s\n","RISCV ISA03+SB01");
  fprintf(out,"%s\n","{");
  fprintf(out,"%s\n"," 0:x10=lock; 0:x11=x; 0:x12=y;");
  fprintf(out,"%s\n"," 1:x10=lock; 1:x11=x; 1:x12=y;");
  fprintf(out,"%s\n","}");
  fprintf(out,"%s\n"," P0                       | P1                       ;");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA03+SB01 Forbidden\n");
  ...
  fprintf(out,"Hash=8e5fd8bfbffcbc6bdd8e91bcda1f55a2\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ISA03+SB01 %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA03+SB01 %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA03+SB01: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int ISA03_2B_SB01(int argc, char **argv, FILE *out) {
