...
inline static int final_cond(int _out_1_x5,int _out_1_x12) {
  switch (_out_1_x5) {
  case 1:
    switch (_out_1_x12) {
    case 0:
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("MP+fence.w.w+data-[ws-ws]-rfi-addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("MP+fence.w.w+data-[ws-ws]-rfi-addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("MP+fence.w.w+data-[ws-ws]-rfi-addr, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"MP+fence.w.w+data-[ws-ws]-rfi-addr");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x12],0(%[x14])\n"
"#END _litmus_P1\n"
:[x12] "=&r" (_a->out_1_x12[_i]),[x10] "=&r" (_a->out_1_x10[_i]),[x5] "=&r" (_a->out_1_x5[_i]),[x14] "=&r" (trashed_x14),[x11] "=&r" (trashed_x11),[x9] "=&r" (trashed_x9),[x7] "=&r" (trashed_x7)
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_z_i != ctx.cpy_z[_p][_i]) fatal("MP+fence.w.w+data-[ws-ws]-rfi-addr, global z unstabilized") ;
      }
      cond = final_ok(final_cond(_out_1_x5_i,_out_1_x12_i));
...
#else
#include "MP+fence.w.w+data-[ws-ws]-rfi-addr.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/HAND/MP+fence.w.w+data-[ws-ws]-rfi-addr.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test MP+fence.w.w+data-[ws-ws]-rfi-addr Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (1:x5=1 /\\ 1:x12=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=109f74aa439e463207d919e846d31725\n");
  fprintf(out,"Generator=diyone7 (version 7.47+4(dev))\n");
  fprintf(out,"Com=Rf Fr Ws\n");
  fprintf(out,"Orig=Fence.w.wdWW Rfe DpDatadW WsLeave WsBack Rfi DpAddrdR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0, 1] [2] ; (1,2)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation MP+fence.w.w+data-[ws-ws]-rfi-addr %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time MP+fence.w.w+data-[ws-ws]-rfi-addr %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "MP+fence.w.w+data-[ws-ws]-rfi-addr: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("MP+fence.w.w+data-[ws-ws]-rfi-addr, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int MP_2B_fence_2E_w_2E_w_2B_data_2D__5B_ws_2D_ws_5D__2D_rfi_2D_addr(int argc, char **argv, FILE *out) {
