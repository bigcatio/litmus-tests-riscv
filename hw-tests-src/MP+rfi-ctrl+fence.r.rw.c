...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("MP+rfi-ctrl+fence.r.rw, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("MP+rfi-ctrl+fence.r.rw, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"MP+rfi-ctrl+fence.r.rw");
  check_globals(_a);
  int _th_id = _b->th_id;
...
#else
#include "MP+rfi-ctrl+fence.r.rw.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test MP+rfi-ctrl+fence.r.rw Allowed\n");
  ...
  fprintf(out,"Hash=676eb8df283ceb65a147bfbbcc75bd92\n");
  fprintf(out,"Cycle=Rfi DpCtrldW Rfe Fence.r.rwdRR Fre\n");
  fprintf(out,"Relax MP+rfi-ctrl+fence.r.rw %s %s\n",p_true > 0 ? "Ok" : "No","Rfi");
  fprintf(out,"Safe=Rfe Fre Fence.r.rwdRR DpCtrldW\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Fr\n");
  fprintf(out,"Orig=Rfi DpCtrldW Rfe Fence.r.rwdRR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0] [1] ; (1,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation MP+rfi-ctrl+fence.r.rw %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time MP+rfi-ctrl+fence.r.rw %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "MP+rfi-ctrl+fence.r.rw: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("MP+rfi-ctrl+fence.r.rw, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int MP_2B_rfi_2D_ctrl_2B_fence_2E_r_2E_rw(int argc, char **argv, FILE *out) {
