...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("C-Will03, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("C-Will03, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"C-Will03");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"ori %[x5],x0,1\n"
"#_litmus_P0_2\n\t"
"amoadd.w.aq %[x28],%[x5],(%[x11])\n"
"#_litmus_P0_3\n\t"
"lw %[x7],0(%[x10])\n"
"#END _litmus_P0\n"
:[x7] "=&r" (_a->out_0_x7[_i]),[x28] "=&r" (trashed_x28),[x5] "=&r" (trashed_x5)
:[x10] "r" (&_a->x[_i]),[x11] "r" (&_a->y[_i])
:"cc","memory"
);
    }
  }
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test C-Will03 Allowed\n");
  ...
  fprintf(out,"Hash=458903a25c67b1666fbff6ecd1d20bc3\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation C-Will03 %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time C-Will03 %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "C-Will03: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int C_2D_Will03(int argc, char **argv, FILE *out) {
