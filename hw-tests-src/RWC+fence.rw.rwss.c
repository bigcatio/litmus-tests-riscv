...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("RWC+fence.rw.rwss, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"RWC+fence.rw.rwss");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("RWC+fence.rw.rwss, global x unstabilized") ;
      }
...
#else
#include "RWC+fence.rw.rwss.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/CO/RWC+fence.rw.rwss.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV RWC+fence.rw.rwss");
  fprintf(out,"%s\n","\"Rfe Fence.rw.rwsRR Fre Fence.rw.rwsWR Fre\"");
  fprintf(out,"%s\n","{");
  fprintf(out,"%s\n"," 0:x5=1; 0:x6=x;");
  fprintf(out,"%s\n"," 1:x6=x;");
  fprintf(out,"%s\n"," 2:x5=2; 2:x6=x;");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test RWC+fence.rw.rwss Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (not (2:x7=2 /\\ (1:x5=0 /\\ ([x]=1 /\\ (1:x7=2 \\/ 1:x7=1 \\/ 1:x7=0) \\/ [x]=2 /\\ (1:x7=0 \\/ 1:x7=1 \\/ 1:x7=2)) \\/ 1:x5=1 /\\ (1:x7=1 /\\ ([x]=1 \\/ [x]=2) \\/ 1:x7=2 /\\ [x]=2) \\/ 1:x5=2 /\\ (1:x7=2 /\\ ([x]=2 \\/ [x]=1) \\/ 1:x7=1 /\\ [x]=1)) \\/ 2:x7=1 /\\ [x]=1 /\\ (1:x5=0 /\\ (1:x7=0 \\/ 1:x7=1 \\/ 1:x7=2) \\/ 1:x5=2 /\\ (1:x7=2 \\/ 1:x7=1) \\/ 1:x5=1 /\\ 1:x7=1)))",cond ? "" : "NOT ");
  fprintf(out,"Hash=7b282e9b515f1d27f78375bb40feefbd\n");
  fprintf(out,"Cycle=Rfe Fence.rw.rwsRR Fre Fence.rw.rwsWR Fre\n");
  fprintf(out,"Relax RWC+fence.rw.rwss %s %s\n",p_true > 0 ? "Ok" : "No","");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "RWC+fence.rw.rwss: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("RWC+fence.rw.rwss, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int RWC_2B_fence_2E_rw_2E_rwss(int argc, char **argv, FILE *out) {
