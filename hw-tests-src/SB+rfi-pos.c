...
inline static int final_cond(uint64_t _out_0_x7,uint64_t _out_0_x8,uint64_t _out_1_x7,uint64_t _out_1_x8) {
  switch (_out_0_x7) {
  case 1:
    switch (_out_0_x8) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x7=%"PRIu64"; 0:x8=%"PRIu64"; 1:x7=%"PRIu64"; 1:x8=%"PRIu64";\n",c,show ? '*' : ':',(uint64_t)o[out_0_x7_f],(uint64_t)o[out_0_x8_f],(uint64_t)o[out_1_x7_f],(uint64_t)o[out_1_x8_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("SB+rfi-pos, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("SB+rfi-pos, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"SB+rfi-pos");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      uint64_t _out_0_x8_i = ctx.out_0_x8[_i];
      uint64_t _out_1_x7_i = ctx.out_1_x7[_i];
      uint64_t _out_1_x8_i = ctx.out_1_x8[_i];
      outcome_t o;
      int cond;

      cond = final_ok(final_cond(_out_0_x7_i,_out_0_x8_i,_out_1_x7_i,_out_1_x8_i));
...
static void prelude(FILE *out) {
  ...
  fprintf(out,"%s\n"," uint64_t y=0;");
  fprintf(out,"%s\n"," uint64_t x=0;");
  fprintf(out,"%s\n"," uint64_t 0:x8=0; uint64_t 0:x7=0; 0:x5=1; 0:x6=x; 0:x9=y;");
  fprintf(out,"%s\n"," uint64_t 1:x8=0; uint64_t 1:x7=0; 1:x5=1; 1:x6=y; 1:x9=x;");
  fprintf(out,"%s\n","}");
  fprintf(out,"%s\n"," P0          | P1          ;");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test SB+rfi-pos Allowed\n");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "SB+rfi-pos: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int SB_2B_rfi_2D_pos(int argc, char **argv, FILE *out) {
