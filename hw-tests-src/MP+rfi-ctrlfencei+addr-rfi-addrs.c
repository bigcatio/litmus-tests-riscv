...
inline static int final_cond(int _out_0_x7,int _out_1_x5,int _out_1_x11,int _out_1_x13,int _x) {
  switch (_out_0_x7) {
  case 2:
    switch (_out_1_x5) {
    case 1:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x7=%d; 1:x5=%d; 1:x11=%d; 1:x13=%d; [x]=%d;\n",c,show ? '*' : ':',(int)o[out_0_x7_f],(int)o[out_1_x5_f],(int)o[out_1_x11_f],(int)o[out_1_x13_f],(int)o[x_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("MP+rfi-ctrlfencei+addr-rfi-addrs, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("MP+rfi-ctrlfencei+addr-rfi-addrs, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"MP+rfi-ctrlfencei+addr-rfi-addrs");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x13],0(%[x14])\n"
"#END _litmus_P1\n"
:[x13] "=&r" (_a->out_1_x13[_i]),[x11] "=&r" (_a->out_1_x11[_i]),[x5] "=&r" (_a->out_1_x5[_i]),[x14] "=&r" (trashed_x14),[x12] "=&r" (trashed_x12),[x10] "=&r" (trashed_x10),[x7] "=&r" (trashed_x7)
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("MP+rfi-ctrlfencei+addr-rfi-addrs, global x unstabilized") ;
      }
      cond = final_ok(final_cond(_out_0_x7_i,_out_1_x5_i,_out_1_x11_i,_out_1_x13_i,_x_i));
...
#else
#include "MP+rfi-ctrlfencei+addr-rfi-addrs.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test MP+rfi-ctrlfencei+addr-rfi-addrs Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([x]=2 /\\ 0:x7=2 /\\ 1:x5=1 /\\ 1:x11=1 /\\ 1:x13=1)",cond ? "" : "NOT ");
  fprintf(out,"Hash=1780d6eb6123bba83a4bcbe1617eea33\n");
  fprintf(out,"Cycle=Rfi DpAddrsR Fre Rfi DpCtrlFenceIdW Rfe DpAddrdW\n");
  fprintf(out,"Relax MP+rfi-ctrlfencei+addr-rfi-addrs %s %s\n",p_true > 0 ? "Ok" : "No","Rfi");
  ...
  fprintf(out,"Orig=Rfi DpCtrlFenceIdW Rfe DpAddrdW Rfi DpAddrsR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0] [1] ; (1,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation MP+rfi-ctrlfencei+addr-rfi-addrs %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time MP+rfi-ctrlfencei+addr-rfi-addrs %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "MP+rfi-ctrlfencei+addr-rfi-addrs: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("MP+rfi-ctrlfencei+addr-rfi-addrs, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int MP_2B_rfi_2D_ctrlfencei_2B_addr_2D_rfi_2D_addrs(int argc, char **argv, FILE *out) {
