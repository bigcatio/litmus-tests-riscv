...
inline static int final_cond(int _out_0_x5,int _out_1_x4,int _out_1_x5) {
  switch (_out_0_x5) {
  case 1:
    switch (_out_1_x4) {
    case 1:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x5=%d; 1:x4=%d; 1:x5=%d;\n",c,show ? '*' : ':',(int)o[out_0_x5_f],(int)o[out_1_x4_f],(int)o[out_1_x5_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("ForwardAMO, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ForwardAMO, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ForwardAMO, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ForwardAMO");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"0:\n"
"#END _litmus_P1\n"
:[x5] "=&r" (_a->out_1_x5[_i]),[x4] "=&r" (_a->out_1_x4[_i]),[x3] "=&r" (trashed_x3),[x1] "=&r" (trashed_x1)
:[x6] "r" (&_a->y[_i]),[x7] "r" (&_a->x[_i]),[x9] "r" (&_a->z[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      cond = final_ok(final_cond(_out_0_x5_i,_out_1_x4_i,_out_1_x5_i));
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ForwardAMO Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (0:x5=1 /\\ 1:x5=1 /\\ 1:x4=1)",cond ? "" : "NOT ");
  fprintf(out,"Hash=2261ce19be75e939167106d8a038d17f\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ForwardAMO %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ForwardAMO %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ForwardAMO: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int ForwardAMO(int argc, char **argv, FILE *out) {
