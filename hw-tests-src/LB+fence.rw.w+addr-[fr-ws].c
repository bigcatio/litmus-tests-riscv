...
inline static int final_cond(int _out_0_x5,int _out_1_x5,int _out_1_x8,int _x) {
  switch (_out_0_x5) {
  case 2:
    switch (_out_1_x5) {
    case 1:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x5=%d; 1:x5=%d; 1:x8=%d; [x]=%d;\n",c,show ? '*' : ':',(int)o[out_0_x5_f],(int)o[out_1_x5_f],(int)o[out_1_x8_f],(int)o[x_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("LB+fence.rw.w+addr-[fr-ws], check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("LB+fence.rw.w+addr-[fr-ws], check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"LB+fence.rw.w+addr-[fr-ws]");
  check_globals(_a);
  int _th_id = _b->th_id;
...
:[x6] "r" (&_a->y[_i]),[x9] "r" (&_a->x[_i]),[x11] "r" ((int)(2))
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("LB+fence.rw.w+addr-[fr-ws], global x unstabilized") ;
      }
      cond = final_ok(final_cond(_out_0_x5_i,_out_1_x5_i,_out_1_x8_i,_x_i));
...
#else
#include "LB+fence.rw.w+addr-[fr-ws].h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/LB+fence.rw.w+addr-[fr-ws].litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV LB+fence.rw.w+addr-[fr-ws]");
  fprintf(out,"%s\n","\"Fence.rw.wdRW Rfe DpAddrdR FrLeave WsBack Rfe\"");
  fprintf(out,"%s\n","{");
  ...
  fprintf(out,"%s\n","             | sw x11,0(x9)  |             ;");
  fprintf(out,"%s\n","");
  fprintf(out,"%s\n","exists ([x]=2 /\\ 0:x5=2 /\\ 1:x5=1 /\\ 1:x8=0)");
  fprintf(out,"%s\n","Generated assembler");
  ass(out);
}
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LB+fence.rw.w+addr-[fr-ws] Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([x]=2 /\\ 0:x5=2 /\\ 1:x5=1 /\\ 1:x8=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=cb34cad6629d866e886e2b9b3b495fa8\n");
  fprintf(out,"Cycle=Rfe Fence.rw.wdRW Rfe DpAddrdR FrLeave WsBack\n");
  fprintf(out,"Relax LB+fence.rw.w+addr-[fr-ws] %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"Safe=Rfe Fence.rw.wdRW DpAddrdR FrLeave WsBack\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Rf Ws\n");
  fprintf(out,"Orig=Fence.rw.wdRW Rfe DpAddrdR FrLeave WsBack Rfe\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0, 1] [2] ; \n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation LB+fence.rw.w+addr-[fr-ws] %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time LB+fence.rw.w+addr-[fr-ws] %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LB+fence.rw.w+addr-[fr-ws]: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("LB+fence.rw.w+addr-[fr-ws], sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int LB_2B_fence_2E_rw_2E_w_2B_addr_2D__5B_fr_2D_ws_5D_(int argc, char **argv, FILE *out) {
