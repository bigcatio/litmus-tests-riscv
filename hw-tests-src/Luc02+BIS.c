...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("Luc02+BIS, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("Luc02+BIS, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"Luc02+BIS");
  check_globals(_a);
  int _th_id = _b->th_id;
...
:[x1] "r" ((int)(0)),[x6] "r" (&_a->y[_i]),[x8] "r" (&_a->x[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("Luc02+BIS, global x unstabilized") ;
      }
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/HAND/Luc02+BIS.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV Luc02+BIS");
  fprintf(out,"%s\n","{");
  fprintf(out,"%s\n"," 0:x6=x; 0:x8=y;");
  fprintf(out,"%s\n"," 1:x6=y; 1:x8=x; 1:x1=0;");
  fprintf(out,"%s\n","}");
  fprintf(out,"%s\n"," P0                        | P1                      ;");
  fprintf(out,"%s\n"," ori x5,x0,2               | amoor.w.aqrl x5,x1,(x6) ;");
  fprintf(out,"%s\n"," amoswap.w.aqrl x1,x5,(x6) | ori x7,x0,1             ;");
  fprintf(out,"%s\n"," ori x7,x0,1               | sw x7,0(x8)             ;");
  fprintf(out,"%s\n"," amoswap.w.aqrl x2,x7,(x8) | amoor.w.aqrl x9,x1,(x8) ;");
  fprintf(out,"%s\n","");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test Luc02+BIS Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (1:x5=0 /\\ 1:x9=2 /\\ [x]=2)",cond ? "" : "NOT ");
  fprintf(out,"Hash=a1347a3c0d3c2b5be622c25295271a96\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation Luc02+BIS %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time Luc02+BIS %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "Luc02+BIS: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int Luc02_2B_BIS(int argc, char **argv, FILE *out) {
