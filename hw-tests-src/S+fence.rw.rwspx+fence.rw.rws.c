...
inline static int final_cond(int _out_0_x8,int _out_0_x9,int _out_1_x5,int _x) {
  switch (_out_1_x5) {
  case 0:
    switch (_x) {
    case 1:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x8=%d; 0:x9=%d; 1:x5=%d; [x]=%d;\n",c,show ? '*' : ':',(int)o[out_0_x8_f],(int)o[out_0_x9_f],(int)o[out_1_x5_f],(int)o[x_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("S+fence.rw.rwspx+fence.rw.rws, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"S+fence.rw.rwspx+fence.rw.rws");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("S+fence.rw.rwspx+fence.rw.rws, global x unstabilized") ;
      }
      cond = final_ok(final_cond(_out_0_x8_i,_out_0_x9_i,_out_1_x5_i,_x_i));
...
#else
#include "S+fence.rw.rwspx+fence.rw.rws.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test S+fence.rw.rwspx+fence.rw.rws Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (not (0:x8=1 /\\ ([x]=3 /\\ (0:x9=0 /\\ (1:x5=2 \\/ 1:x5=1 \\/ 1:x5=0) \\/ 0:x9=1 /\\ (1:x5=0 \\/ 1:x5=1)) \\/ 1:x5=0 /\\ (0:x9=0 /\\ [x]=2 \\/ 0:x9=1 /\\ [x]=1)) \\/ 0:x8=3 /\\ (0:x9=0 /\\ [x]=2 /\\ (1:x5=0 \\/ 1:x5=1) \\/ 0:x9=1 /\\ [x]=3 /\\ (1:x5=1 \\/ 1:x5=0))))",cond ? "" : "NOT ");
  fprintf(out,"Hash=6556e507be1f2c9dd36f1913a5511408\n");
  fprintf(out,"Cycle=Wse Fence.rw.rwsWWPX RfeXP Fence.rw.rwsRW\n");
  fprintf(out,"Relax S+fence.rw.rwspx+fence.rw.rws %s %s\n",p_true > 0 ? "Ok" : "No","Fence.rw.rwsWWPX");
  fprintf(out,"Safe=Rfe Wse Fence.rw.rwsRW\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Ws\n");
  fprintf(out,"Orig=Fence.rw.rwsWWPX RfeXP Fence.rw.rwsRW Wse\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0] [1] ; \n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation S+fence.rw.rwspx+fence.rw.rws %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time S+fence.rw.rwspx+fence.rw.rws %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "S+fence.rw.rwspx+fence.rw.rws: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("S+fence.rw.rwspx+fence.rw.rws, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int S_2B_fence_2E_rw_2E_rwspx_2B_fence_2E_rw_2E_rws(int argc, char **argv, FILE *out) {
