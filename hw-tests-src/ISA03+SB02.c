...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA03+SB02, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA03+SB02, check_globals failed");
    if (rand_bit(&(_a->seed)) && lock[_i] != 0) fatal("ISA03+SB02, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA03+SB02");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA03+SB02 Allowed\n");
  ...
  fprintf(out,"Hash=df7504b1422e3c30d27a973873e338ba\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ISA03+SB02 %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA03+SB02 %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA03+SB02: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int ISA03_2B_SB02(int argc, char **argv, FILE *out) {
