...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x2=%d; 1:x4=%d; 1:x5=%d;\n",c,show ? '*' : ':',(int)o[out_1_x2_f],(int)o[out_1_x4_f],(int)o[out_1_x5_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("ISA-MP-DEP-SUCCESS-SWAP, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA-MP-DEP-SUCCESS-SWAP, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA-MP-DEP-SUCCESS-SWAP, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA-MP-DEP-SUCCESS-SWAP");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"amoswap.w %[x5],%[x4],(%[x6])\n"
"#END _litmus_P1\n"
:[x5] "=&r" (_a->out_1_x5[_i]),[x4] "=&r" (_a->out_1_x4[_i]),[x2] "=&r" (_a->out_1_x2[_i]),[x3] "=&r" (trashed_x3)
:[x6] "r" (&_a->x[_i]),[x8] "r" (&_a->y[_i]),[x10] "r" (&_a->z[_i])
:"cc","memory"
);
    }
  }
...
#else
#include "ISA-MP-DEP-SUCCESS-SWAP.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA-MP-DEP-SUCCESS-SWAP Allowed\n");
  ...
  fprintf(out,"Hash=d7c579555c774de19c7dcaf4d48b05f6\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ISA-MP-DEP-SUCCESS-SWAP %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA-MP-DEP-SUCCESS-SWAP %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA-MP-DEP-SUCCESS-SWAP: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("ISA-MP-DEP-SUCCESS-SWAP, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int ISA_2D_MP_2D_DEP_2D_SUCCESS_2D_SWAP(int argc, char **argv, FILE *out) {
