...
inline static int final_cond(int* _out_1_x10,int _y,void *x) {
  return (_out_1_x10 == x) && (_y == 2);
}
...
static int idx_addr(ctx_t *_a,int _i,void *v_addr) {
  ...
  else { fatal("ISA-S-DEP-ADDR-SUCCESS, ???"); return -1;}
}
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x10=%s; [y]=%d;\n",c,show ? '*' : ':',pretty_addr[o[out_1_x10_f]],(int)o[y_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("ISA-S-DEP-ADDR-SUCCESS, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA-S-DEP-ADDR-SUCCESS, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA-S-DEP-ADDR-SUCCESS, check_globals failed");
    if (rand_bit(&(_a->seed)) && p[_i] != &(_a->z[_i])) fatal("ISA-S-DEP-ADDR-SUCCESS, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA-S-DEP-ADDR-SUCCESS");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sw %[x12],0(%[x9])\n"
"#END _litmus_P1\n"
:[x10] "=&r" (_a->out_1_x10[_i]),[x12] "=&r" (trashed_x12),[x11] "=&r" (trashed_x11),[x6] "=&r" (trashed_x6)
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("ISA-S-DEP-ADDR-SUCCESS, global y unstabilized") ;
      }
      cond = final_ok(final_cond(_out_1_x10_i,_y_i,&(ctx.x[_i])));
...
#else
#include "ISA-S-DEP-ADDR-SUCCESS.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA-S-DEP-ADDR-SUCCESS Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (1:x10=x /\\ [y]=2)",cond ? "" : "NOT ");
  fprintf(out,"Hash=8a62adcdb838a848293dfc54a184263a\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ISA-S-DEP-ADDR-SUCCESS %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA-S-DEP-ADDR-SUCCESS %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA-S-DEP-ADDR-SUCCESS: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("ISA-S-DEP-ADDR-SUCCESS, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int ISA_2D_S_2D_DEP_2D_ADDR_2D_SUCCESS(int argc, char **argv, FILE *out) {
