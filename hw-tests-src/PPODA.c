...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("PPODA, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("PPODA, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("PPODA, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"PPODA");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test PPODA Allowed\n");
  ...
  fprintf(out,"Hash=81b29d130d7480e5feb62850c94c3c11\n");
  ...
  fprintf(out,"Observation PPODA %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time PPODA %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "PPODA: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int PPODA(int argc, char **argv, FILE *out) {
