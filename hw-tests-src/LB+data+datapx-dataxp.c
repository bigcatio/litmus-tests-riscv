...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("LB+data+datapx-dataxp, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("LB+data+datapx-dataxp, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("LB+data+datapx-dataxp, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"LB+data+datapx-dataxp");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sw %[x11],0(%[x12])\n"
"#END _litmus_P1\n"
:[x10] "=&r" (_a->out_1_x10[_i]),[x5] "=&r" (_a->out_1_x5[_i]),[x11] "=&r" (trashed_x11),[x9] "=&r" (trashed_x9),[x7] "=&r" (trashed_x7)
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_z_i != ctx.cpy_z[_p][_i]) fatal("LB+data+datapx-dataxp, global z unstabilized") ;
      }
...
#else
#include "LB+data+datapx-dataxp.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LB+data+datapx-dataxp Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([z]=1 /\\ 0:x5=1 /\\ 1:x10=0 /\\ 1:x5=1)",cond ? "" : "NOT ");
  fprintf(out,"Hash=aa02d2a2822ef1ea9735e875dc6b5e1e\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation LB+data+datapx-dataxp %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time LB+data+datapx-dataxp %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LB+data+datapx-dataxp: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("LB+data+datapx-dataxp, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int LB_2B_data_2B_datapx_2D_dataxp(int argc, char **argv, FILE *out) {
