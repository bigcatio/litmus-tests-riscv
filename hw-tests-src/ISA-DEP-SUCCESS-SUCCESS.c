...
inline static int final_cond(int _out_0_x10,int _out_0_x12,int _out_0_x14,int _out_1_x13,uint64_t _a,uint64_t _y) {
  switch (_a) {
  case 0:
    switch (_out_0_x12) {
    case 2:
...
static const int out_0_x10_f = 0 ;
static const int out_0_x12_f = 1 ;
static const int out_0_x14_f = 2 ;
static const int out_1_x13_f = 3 ;
static const int a_f = 4 ;
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x10=%d; 0:x12=%d; 0:x14=%d; 1:x13=%d; [a]=%"PRIu64"; [y]=%"PRIu64";\n",c,show ? '*' : ':',(int)o[out_0_x10_f],(int)o[out_0_x12_f],(int)o[out_0_x14_f],(int)o[out_1_x13_f],(uint64_t)o[a_f],(uint64_t)o[y_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("ISA-DEP-SUCCESS-SUCCESS, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA-DEP-SUCCESS-SUCCESS, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA-DEP-SUCCESS-SUCCESS, check_globals failed");
    if (rand_bit(&(_a->seed)) && a[_i] != 0) fatal("ISA-DEP-SUCCESS-SUCCESS, check_globals failed");
  }
...
static void stabilize_globals(int _id, ctx_t *_a) {
  ...
  uint64_t *a = _a->a;
  uint64_t **cpy_a = _a->cpy_a;
  uint64_t *y = _a->y;
  uint64_t **cpy_y = _a->cpy_y;
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA-DEP-SUCCESS-SUCCESS");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sd %[x14],0(%[x18])\n"
"#END _litmus_P0\n"
:[x14] "=&r" (_a->out_0_x14[_i]),[x12] "=&r" (_a->out_0_x12[_i]),[x10] "=&r" (_a->out_0_x10[_i]),[x13] "=&r" (trashed_x13),[x11] "=&r" (trashed_x11)
:[x8] "r" (&_a->x[_i]),[x9] "r" (&_a->y[_i]),[x18] "r" (&_a->z[_i]),[x19] "r" (&_a->a[_i])
:"cc","memory"
);
    }
  }
...
"sd %[x13],0(%[x8])\n"
"#END _litmus_P1\n"
:[x13] "=&r" (_a->out_1_x13[_i])
:[x8] "r" (&_a->x[_i]),[x18] "r" (&_a->z[_i])
:"cc","memory"
);
    }
  }
...
static void init(ctx_t *_a) {
  ...
  _a->out_0_x14 = malloc_check(size_of_test*sizeof(*(_a->out_0_x14)));
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      int _out_0_x14_i = ctx.out_0_x14[_i];
      int _out_1_x13_i = ctx.out_1_x13[_i];
      uint64_t _a_i = ctx.a[_i];
      uint64_t _y_i = ctx.y[_i];
      outcome_t o;
      int cond;

      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_a_i != ctx.cpy_a[_p][_i]) fatal("ISA-DEP-SUCCESS-SUCCESS, global a unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("ISA-DEP-SUCCESS-SUCCESS, global y unstabilized") ;
      }
      cond = final_ok(final_cond(_out_0_x10_i,_out_0_x12_i,_out_0_x14_i,_out_1_x13_i,_a_i,_y_i));
      o[out_0_x10_f] = _out_0_x10_i;
      o[out_0_x12_f] = _out_0_x12_i;
      o[out_0_x14_f] = _out_0_x14_i;
...
#else
#include "ISA-DEP-SUCCESS-SUCCESS.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA-DEP-SUCCESS-SUCCESS Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (0:x10=2 /\\ 0:x14=2 /\\ 1:x13=2 /\\ (0:x12=2 /\\ [a]=2 /\\ [y]=2 \\/ 0:x12=3 /\\ [a]=3 /\\ [y]=0) \\/ 0:x10=3 /\\ 0:x14=3 /\\ 1:x13=3 /\\ [a]=0 /\\ (0:x12=2 /\\ [y]=3 \\/ 0:x12=3 /\\ [y]=0))",cond ? "" : "NOT ");
  fprintf(out,"Hash=178e3a4e8de53618fbedac3bfea2c7dc\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ISA-DEP-SUCCESS-SUCCESS %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA-DEP-SUCCESS-SUCCESS %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA-DEP-SUCCESS-SUCCESS: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("ISA-DEP-SUCCESS-SUCCESS, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int ISA_2D_DEP_2D_SUCCESS_2D_SUCCESS(int argc, char **argv, FILE *out) {
