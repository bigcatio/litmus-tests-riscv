...
inline static int final_cond(int _out_0_x10) {
  switch (_out_0_x10) {
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x10=%d;\n",c,show ? '*' : ':',(int)o[out_0_x10_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA01, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA01");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sw %[x28],0(%[x8])\n"
"#END _litmus_P0\n"
:[x10] "=&r" (_a->out_0_x10[_i]),[x28] "=&r" (trashed_x28),[x7] "=&r" (trashed_x7),[x6] "=&r" (trashed_x6)
:[x8] "r" (&_a->x[_i])
:"cc","memory"
);
    }
...
"sw %[x30],0(%[x8])\n"
"#END _litmus_P1\n"
:[x30] "=&r" (trashed_x30),[x29] "=&r" (trashed_x29)
:[x8] "r" (&_a->x[_i])
:"cc","memory"
);
    }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      cond = final_ok(final_cond(_out_0_x10_i));
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA01 Required\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","forall (0:x10=2 \\/ 0:x10=4 \\/ 0:x10=5)",cond ? "" : "NOT ");
  fprintf(out,"Hash=482d365bfd3aa0e0367a2b9a4f381101\n");
  count_t cond_true = p_false;
  count_t cond_false = p_true;
  fprintf(out,"Observation ISA01 %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA01 %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA01: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int ISA01(int argc, char **argv, FILE *out) {
