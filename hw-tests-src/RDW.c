...
inline static int final_cond(uint64_t _out_1_x5,uint64_t _out_1_x8,uint64_t _out_1_x11,uint64_t _out_1_x13) {
  switch (_out_1_x5) {
  case 1:
    switch (_out_1_x8) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x5=%"PRIu64"; 1:x8=%"PRIu64"; 1:x11=%"PRIu64"; 1:x13=%"PRIu64";\n",c,show ? '*' : ':',(uint64_t)o[out_1_x5_f],(uint64_t)o[out_1_x8_f],(uint64_t)o[out_1_x11_f],(uint64_t)o[out_1_x13_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("RDW, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("RDW, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("RDW, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"RDW");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"ld %[x13],0(%[x15])\n"
"#END _litmus_P1\n"
:[x13] "=&r" (_a->out_1_x13[_i]),[x11] "=&r" (_a->out_1_x11[_i]),[x8] "=&r" (_a->out_1_x8[_i]),[x5] "=&r" (_a->out_1_x5[_i]),[x15] "=&r" (trashed_x15),[x12] "=&r" (trashed_x12),[x10] "=&r" (trashed_x10),[x7] "=&r" (trashed_x7)
:[x6] "r" (&_a->y[_i]),[x9] "r" (&_a->z[_i]),[x14] "r" (&_a->x[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      uint64_t _out_1_x5_i = ctx.out_1_x5[_i];
      uint64_t _out_1_x8_i = ctx.out_1_x8[_i];
      uint64_t _out_1_x11_i = ctx.out_1_x11[_i];
      uint64_t _out_1_x13_i = ctx.out_1_x13[_i];
      outcome_t o;
      int cond;

      cond = final_ok(final_cond(_out_1_x5_i,_out_1_x8_i,_out_1_x11_i,_out_1_x13_i));
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/HAND/RDW.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV RDW");
  ...
  fprintf(out,"%s\n"," uint64_t 1:x13=0; uint64_t 1:x11=0; uint64_t 1:x8=0; uint64_t 1:x5=0; 1:x6=y; 1:x9=z; 1:x14=x;");
  fprintf(out,"%s\n"," 2:x5=1; 2:x6=z;");
  fprintf(out,"%s\n","}");
  fprintf(out,"%s\n"," P0          | P1              | P2          ;");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (1:x5=1 /\\ 1:x8=0 /\\ 1:x11=1 /\\ 1:x13=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=72ea4d00dc6ea8c33d77e974e57fe411\n");
  fprintf(out,"Generator=diyone7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Fr Rf\n");
  fprintf(out,"Orig=Fence.rw.rwdWW Rfe DpAddrdR FrLeave RfBack DpAddrdR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[2, 0] [1] ; (1,2)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation RDW %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time RDW %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "RDW: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int RDW(int argc, char **argv, FILE *out) {
