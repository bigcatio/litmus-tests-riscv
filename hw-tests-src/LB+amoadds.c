...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("LB+amoadds, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("LB+amoadds, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"LB+amoadds");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"amoadd.w %[x11],%[x10],(%[x9])\n"
"#END _litmus_P0\n"
:[x11] "=&r" (trashed_x11),[x10] "=&r" (trashed_x10),[x7] "=&r" (trashed_x7),[x6] "=&r" (trashed_x6)
:[x5] "r" (&_a->x[_i]),[x9] "r" (&_a->y[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("LB+amoadds, global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("LB+amoadds, global y unstabilized") ;
      }
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LB+amoadds Required\n");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LB+amoadds: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int LB_2B_amoadds(int argc, char **argv, FILE *out) {
