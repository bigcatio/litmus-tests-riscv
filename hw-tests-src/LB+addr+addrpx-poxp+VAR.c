...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("LB+addr+addrpx-poxp+VAR, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("LB+addr+addrpx-poxp+VAR, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("LB+addr+addrpx-poxp+VAR, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"LB+addr+addrpx-poxp+VAR");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sw %[x12],0(%[x13])\n"
"#END _litmus_P1\n"
:[x11] "=&r" (_a->out_1_x11[_i]),[x5] "=&r" (_a->out_1_x5[_i]),[x12] "=&r" (trashed_x12),[x10] "=&r" (trashed_x10),[x9] "=&r" (trashed_x9),[x7] "=&r" (trashed_x7)
...
#else
#include "LB+addr+addrpx-poxp+VAR.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LB+addr+addrpx-poxp+VAR Allowed\n");
  ...
  fprintf(out,"Hash=3b31dc1dae279fea280f1553ee21c92d\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation LB+addr+addrpx-poxp+VAR %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time LB+addr+addrpx-poxp+VAR %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LB+addr+addrpx-poxp+VAR: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("LB+addr+addrpx-poxp+VAR, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int LB_2B_addr_2B_addrpx_2D_poxp_2B_VAR(int argc, char **argv, FILE *out) {
