...
inline static int final_cond(int _out_0_x5,int _out_0_x8,int _out_1_x5,int _out_1_x8,int _x,int _y,int _z) {
  switch (_out_0_x5) {
  case 0:
    switch (_out_0_x8) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x5=%d; 0:x8=%d; 1:x5=%d; 1:x8=%d; [x]=%d; [y]=%d; [z]=%d;\n",c,show ? '*' : ':',(int)o[out_0_x5_f],(int)o[out_0_x8_f],(int)o[out_1_x5_f],(int)o[out_1_x8_f],(int)o[x_f],(int)o[y_f],(int)o[z_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("LR-SC-diff-loc1, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("LR-SC-diff-loc1, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("LR-SC-diff-loc1, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"LR-SC-diff-loc1");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sc.w %[x8],%[x7],0(%[x9])\n"
"#END _litmus_P0\n"
:[x8] "=&r" (_a->out_0_x8[_i]),[x5] "=&r" (_a->out_0_x5[_i]),[x7] "=&r" (trashed_x7)
...
"sc.w %[x8],%[x7],0(%[x9])\n"
"#END _litmus_P1\n"
:[x8] "=&r" (_a->out_1_x8[_i]),[x5] "=&r" (_a->out_1_x5[_i]),[x7] "=&r" (trashed_x7)
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("LR-SC-diff-loc1, global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("LR-SC-diff-loc1, global y unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_z_i != ctx.cpy_z[_p][_i]) fatal("LR-SC-diff-loc1, global z unstabilized") ;
      }
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LR-SC-diff-loc1 Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([x]=0 /\\ [y]=1 /\\ [z]=1 /\\ 0:x5=0 /\\ 0:x8=0 /\\ 1:x5=0 /\\ 1:x8=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=85a84102c453b7aa8c2c813e2f03c06e\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation LR-SC-diff-loc1 %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time LR-SC-diff-loc1 %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LR-SC-diff-loc1: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("LR-SC-diff-loc1, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int LR_2D_SC_2D_diff_2D_loc1(int argc, char **argv, FILE *out) {
