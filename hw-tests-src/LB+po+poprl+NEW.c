...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("LB+po+poprl+NEW, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("LB+po+poprl+NEW, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"LB+po+poprl+NEW");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LB+po+poprl+NEW Allowed\n");
  ...
  fprintf(out,"Hash=2a4c34edab11e969eefaa958db2da2a6\n");
  fprintf(out,"Cycle=Rfe PodRWPRl RfeRlP PodRW\n");
  fprintf(out,"Relax LB+po+poprl+NEW %s %s\n",p_true > 0 ? "Ok" : "No","Rl");
  fprintf(out,"Safe=Rfe PodRW\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Rf\n");
  fprintf(out,"Orig=PodRW Rfe PodRWPRl RfeRlP\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1, 0] ; \n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation LB+po+poprl+NEW %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time LB+po+poprl+NEW %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LB+po+poprl+NEW: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("LB+po+poprl+NEW, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int LB_2B_po_2B_poprl_2B_NEW(int argc, char **argv, FILE *out) {
