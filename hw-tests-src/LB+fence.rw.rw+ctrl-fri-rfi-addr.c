...
inline static int final_cond(int _out_0_x5,int _out_1_x5,int _out_1_x7,int _out_1_x10) {
  switch (_out_0_x5) {
  case 1:
    switch (_out_1_x5) {
    case 1:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x5=%d; 1:x5=%d; 1:x7=%d; 1:x10=%d;\n",c,show ? '*' : ':',(int)o[out_0_x5_f],(int)o[out_1_x5_f],(int)o[out_1_x7_f],(int)o[out_1_x10_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("LB+fence.rw.rw+ctrl-fri-rfi-addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("LB+fence.rw.rw+ctrl-fri-rfi-addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("LB+fence.rw.rw+ctrl-fri-rfi-addr, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"LB+fence.rw.rw+ctrl-fri-rfi-addr");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      cond = final_ok(final_cond(_out_0_x5_i,_out_1_x5_i,_out_1_x7_i,_out_1_x10_i));
...
#else
#include "LB+fence.rw.rw+ctrl-fri-rfi-addr.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LB+fence.rw.rw+ctrl-fri-rfi-addr Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (0:x5=1 /\\ 1:x5=1 /\\ 1:x7=0 /\\ 1:x10=1)",cond ? "" : "NOT ");
  fprintf(out,"Hash=cfe2f04d649eb01202e2b6d9f16f6b64\n");
  fprintf(out,"Cycle=Rfi DpAddrdW Rfe Fence.rw.rwdRW Rfe DpCtrldR Fri\n");
  fprintf(out,"Relax LB+fence.rw.rw+ctrl-fri-rfi-addr %s %s\n",p_true > 0 ? "Ok" : "No","[Fri,Rfi]");
  fprintf(out,"Safe=Rfe Fence.rw.rwdRW DpAddrdW DpCtrldR\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Rf\n");
  fprintf(out,"Orig=Fence.rw.rwdRW Rfe DpCtrldR Fri Rfi DpAddrdW Rfe\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1, 0] ; \n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation LB+fence.rw.rw+ctrl-fri-rfi-addr %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time LB+fence.rw.rw+ctrl-fri-rfi-addr %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LB+fence.rw.rw+ctrl-fri-rfi-addr: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("LB+fence.rw.rw+ctrl-fri-rfi-addr, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int LB_2B_fence_2E_rw_2E_rw_2B_ctrl_2D_fri_2D_rfi_2D_addr(int argc, char **argv, FILE *out) {
