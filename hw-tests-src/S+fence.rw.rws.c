...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("S+fence.rw.rws, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("S+fence.rw.rws, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"S+fence.rw.rws");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("S+fence.rw.rws, global x unstabilized") ;
      }
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test S+fence.rw.rws Allowed\n");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "S+fence.rw.rws: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int S_2B_fence_2E_rw_2E_rws(int argc, char **argv, FILE *out) {
