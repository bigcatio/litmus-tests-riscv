...
inline static int final_cond(int _out_1_x7,int _out_1_x10,int _x,int _y) {
  switch (_out_1_x7) {
  case 2:
    switch (_out_1_x10) {
    case 1:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x7=%d; 1:x10=%d; [x]=%d; [y]=%d;\n",c,show ? '*' : ':',(int)o[out_1_x7_f],(int)o[out_1_x10_f],(int)o[x_f],(int)o[y_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("R+fence.rw.rw+rfi-ctrlfencei-rfi, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("R+fence.rw.rw+rfi-ctrlfencei-rfi, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"R+fence.rw.rw+rfi-ctrlfencei-rfi");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("R+fence.rw.rw+rfi-ctrlfencei-rfi, global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("R+fence.rw.rw+rfi-ctrlfencei-rfi, global y unstabilized") ;
      }
      cond = final_ok(final_cond(_out_1_x7_i,_out_1_x10_i,_x_i,_y_i));
...
#else
#include "R+fence.rw.rw+rfi-ctrlfencei-rfi.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test R+fence.rw.rw+rfi-ctrlfencei-rfi Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([x]=2 /\\ [y]=2 /\\ 1:x7=2 /\\ 1:x10=1)",cond ? "" : "NOT ");
  fprintf(out,"Hash=22c48328f3f8fd85fc272d13af2b530d\n");
  fprintf(out,"Cycle=Rfi Fre Fence.rw.rwdWW Wse Rfi DpCtrlFenceIdW\n");
  fprintf(out,"Relax R+fence.rw.rw+rfi-ctrlfencei-rfi %s %s\n",p_true > 0 ? "Ok" : "No","Rfi");
  fprintf(out,"Safe=Fre Wse Fence.rw.rwdWW DpCtrlFenceIdW\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Ws Fr\n");
  fprintf(out,"Orig=Fence.rw.rwdWW Wse Rfi DpCtrlFenceIdW Rfi Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0] [1] ; (1,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation R+fence.rw.rw+rfi-ctrlfencei-rfi %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time R+fence.rw.rw+rfi-ctrlfencei-rfi %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "R+fence.rw.rw+rfi-ctrlfencei-rfi: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("R+fence.rw.rw+rfi-ctrlfencei-rfi, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int R_2B_fence_2E_rw_2E_rw_2B_rfi_2D_ctrlfencei_2D_rfi(int argc, char **argv, FILE *out) {
