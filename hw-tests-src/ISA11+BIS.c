...
inline static int filter_cond(int _out_0_x29) {
  switch (_out_0_x29) {
  case 0:
    return 1;
  default:
...
inline static int final_cond(int _out_0_x10,uint64_t _x,uint64_t _y) {
  switch (_out_0_x10) {
  case 2:
    switch (_x) {
    case 3:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x10=%d; [x]=%"PRIu64"; [y]=%"PRIu64";\n",c,show ? '*' : ':',(int)o[out_0_x10_f],(uint64_t)o[x_f],(uint64_t)o[y_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA11+BIS, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA11+BIS, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA11+BIS");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sd %[x7],0(%[x9])\n"
"#END _litmus_P0\n"
:[x29] "=&r" (_a->out_0_x29[_i]),[x10] "=&r" (_a->out_0_x10[_i]),[x28] "=&r" (trashed_x28),[x7] "=&r" (trashed_x7),[x6] "=&r" (trashed_x6)
...
"sd %[x28],0(%[x9])\n"
"#_litmus_P1_3\n\t"
"amoswap.d.rl x0,%[x29],(%[x8])\n"
"#END _litmus_P1\n"
:[x29] "=&r" (trashed_x29),[x28] "=&r" (trashed_x28)
:[x8] "r" (&_a->x[_i]),[x9] "r" (&_a->y[_i])
:"cc","memory"
);
    }
...
static void init(ctx_t *_a) {
  ...
  _a->out_0_x29 = malloc_check(size_of_test*sizeof(*(_a->out_0_x29)));
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      int _out_0_x29_i = ctx.out_0_x29[_i];
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("ISA11+BIS, global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("ISA11+BIS, global y unstabilized") ;
      }
      if (filter_cond(_out_0_x29_i)) {
        cond = final_ok(final_cond(_out_0_x10_i,_x_i,_y_i));
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA11+BIS Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (0:x10=2 /\\ [x]=3 /\\ [y]=2)",cond ? "" : "NOT ");
  fprintf(out,"Hash=96f7b762dab5821e81eae5c0758785fe\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ISA11+BIS %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA11+BIS %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA11+BIS: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int ISA11_2B_BIS(int argc, char **argv, FILE *out) {
