...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("ISA10+TER, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA10+TER, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA10+TER, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA10+TER");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x14],0(%[x16])\n"
"#END _litmus_P1\n"
:[x14] "=&r" (_a->out_1_x14[_i]),[x12] "=&r" (_a->out_1_x12[_i]),[x8] "=&r" (_a->out_1_x8[_i]),[x5] "=&r" (_a->out_1_x5[_i]),[x16] "=&r" (trashed_x16),[x13] "=&r" (trashed_x13),[x11] "=&r" (trashed_x11),[x10] "=&r" (trashed_x10),[x7] "=&r" (trashed_x7)
:[x6] "r" (&_a->y[_i]),[x9] "r" (&_a->z[_i]),[x15] "r" (&_a->x[_i])
:"cc","memory"
);
    }
  }
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA10+TER Allowed\n");
  ...
  fprintf(out,"Hash=be9ecf0e832eafbc95ba44eb82738496\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ISA10+TER %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA10+TER %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA10+TER: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int ISA10_2B_TER(int argc, char **argv, FILE *out) {
