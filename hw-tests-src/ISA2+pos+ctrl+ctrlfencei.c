...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA2+pos+ctrl+ctrlfencei, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA2+pos+ctrl+ctrlfencei, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA2+pos+ctrl+ctrlfencei");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("ISA2+pos+ctrl+ctrlfencei, global x unstabilized") ;
      }
...
#else
#include "ISA2+pos+ctrl+ctrlfencei.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/ISA2+pos+ctrl+ctrlfencei.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV ISA2+pos+ctrl+ctrlfencei");
  fprintf(out,"%s\n","\"PosWW Rfe DpCtrldW Rfe DpCtrlFenceIdR Fre\"");
  fprintf(out,"%s\n","{");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA2+pos+ctrl+ctrlfencei Allowed\n");
  ...
  fprintf(out,"Hash=4486b6e86b700914c393bc4e1a11536c\n");
  fprintf(out,"Cycle=Rfe DpCtrldW Rfe DpCtrlFenceIdR Fre PosWW\n");
  fprintf(out,"Relax ISA2+pos+ctrl+ctrlfencei %s %s\n",p_true > 0 ? "Ok" : "No","");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA2+pos+ctrl+ctrlfencei: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("ISA2+pos+ctrl+ctrlfencei, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int ISA2_2B_pos_2B_ctrl_2B_ctrlfencei(int argc, char **argv, FILE *out) {
