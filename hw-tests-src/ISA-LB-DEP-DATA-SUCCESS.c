...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x10=%d; 1:x10=%d; 1:x11=%d;\n",c,show ? '*' : ':',(int)o[out_0_x10_f],(int)o[out_1_x10_f],(int)o[out_1_x11_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("ISA-LB-DEP-DATA-SUCCESS, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA-LB-DEP-DATA-SUCCESS, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA-LB-DEP-DATA-SUCCESS, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA-LB-DEP-DATA-SUCCESS");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sw %[x7],0(%[x9])\n"
"#END _litmus_P1\n"
:[x11] "=&r" (_a->out_1_x11[_i]),[x10] "=&r" (_a->out_1_x10[_i]),[x7] "=&r" (trashed_x7)
...
#else
#include "ISA-LB-DEP-DATA-SUCCESS.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA-LB-DEP-DATA-SUCCESS Forbidden\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","~exists (1:x10=1 /\\ 0:x10=1)",cond ? "" : "NOT ");
  fprintf(out,"Hash=ff1fd165728d8547e728bbb1114acffd\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation ISA-LB-DEP-DATA-SUCCESS %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA-LB-DEP-DATA-SUCCESS %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA-LB-DEP-DATA-SUCCESS: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("ISA-LB-DEP-DATA-SUCCESS, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int ISA_2D_LB_2D_DEP_2D_DATA_2D_SUCCESS(int argc, char **argv, FILE *out) {
