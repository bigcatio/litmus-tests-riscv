...
inline static int final_cond(int _out_0_x8,int _y) {
  switch (_out_0_x8) {
  case 1:
    switch (_y) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x8=%d; [y]=%d;\n",c,show ? '*' : ':',(int)o[out_0_x8_f],(int)o[y_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("SC-FAIL, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("SC-FAIL, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"SC-FAIL");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sc.w %[x8],%[x6],0(%[x3])\n"
"#END _litmus_P0\n"
:[x8] "=&r" (_a->out_0_x8[_i]),[x7] "=&r" (trashed_x7),[x6] "=&r" (trashed_x6)
:[x2] "r" (&_a->x[_i]),[x3] "r" (&_a->y[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("SC-FAIL, global y unstabilized") ;
      }
      cond = final_ok(final_cond(_out_0_x8_i,_y_i));
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test SC-FAIL Required\n");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "SC-FAIL: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int SC_2D_FAIL(int argc, char **argv, FILE *out) {
