...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("LB+ctrl+ctrl-[rf-fr], check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("LB+ctrl+ctrl-[rf-fr], check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"LB+ctrl+ctrl-[rf-fr]");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("LB+ctrl+ctrl-[rf-fr], global x unstabilized") ;
      }
...
#else
#include "LB+ctrl+ctrl-[rf-fr].h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/LB+ctrl+ctrl-[rf-fr].litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV LB+ctrl+ctrl-[rf-fr]");
  fprintf(out,"%s\n","\"DpCtrldW Rfe DpCtrldW RfLeave FrBack Rfe\"");
  fprintf(out,"%s\n","{");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LB+ctrl+ctrl-[rf-fr] Allowed\n");
  ...
  fprintf(out,"Hash=6ddf44f22af186a978f8e037c2e7522c\n");
  fprintf(out,"Cycle=Rfe DpCtrldW Rfe DpCtrldW RfLeave FrBack\n");
  fprintf(out,"Relax LB+ctrl+ctrl-[rf-fr] %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"Safe=Rfe DpCtrldW RfLeave FrBack\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Rf Fr\n");
  fprintf(out,"Orig=DpCtrldW Rfe DpCtrldW RfLeave FrBack Rfe\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0, 1] [2] ; (2,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation LB+ctrl+ctrl-[rf-fr] %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time LB+ctrl+ctrl-[rf-fr] %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LB+ctrl+ctrl-[rf-fr]: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("LB+ctrl+ctrl-[rf-fr], sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int LB_2B_ctrl_2B_ctrl_2D__5B_rf_2D_fr_5D_(int argc, char **argv, FILE *out) {
