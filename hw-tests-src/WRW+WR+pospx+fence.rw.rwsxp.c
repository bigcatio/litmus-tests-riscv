...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("WRW+WR+pospx+fence.rw.rwsxp, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"WRW+WR+pospx+fence.rw.rwsxp");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("WRW+WR+pospx+fence.rw.rwsxp, global x unstabilized") ;
      }
...
#else
#include "WRW+WR+pospx+fence.rw.rwsxp.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/ATOMICS/CO/WRW+WR+pospx+fence.rw.rwsxp.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV WRW+WR+pospx+fence.rw.rwsxp");
  fprintf(out,"%s\n","\"Rfe PosRWPX WseXX Fence.rw.rwsWRXP Fre\"");
  fprintf(out,"%s\n","{");
  fprintf(out,"%s\n"," 0:x5=1; 0:x6=x;");
  fprintf(out,"%s\n"," 1:x6=x; 1:x7=2;");
  fprintf(out,"%s\n"," 2:x5=x; 2:x6=3;");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test WRW+WR+pospx+fence.rw.rwsxp Allowed\n");
  ...
  fprintf(out,"Hash=990d320dfc2c573b14622bf646ead5e1\n");
  fprintf(out,"Cycle=Rfe PosRWPX WseXX Fence.rw.rwsWRXP Fre\n");
  fprintf(out,"Relax WRW+WR+pospx+fence.rw.rwsxp %s %s\n",p_true > 0 ? "Ok" : "No","WseXX");
  fprintf(out,"Safe=Rfe Fre PosRW Fence.rw.rwsWR\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Ws Fr\n");
  fprintf(out,"Orig=Rfe PosRWPX WseXX Fence.rw.rwsWRXP Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0, 1] [2] ; (2,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation WRW+WR+pospx+fence.rw.rwsxp %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time WRW+WR+pospx+fence.rw.rwsxp %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "WRW+WR+pospx+fence.rw.rwsxp: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("WRW+WR+pospx+fence.rw.rwsxp, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int WRW_2B_WR_2B_pospx_2B_fence_2E_rw_2E_rwsxp(int argc, char **argv, FILE *out) {
