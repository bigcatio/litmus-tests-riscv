...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("SB+wsi-rfi-ctrls, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("SB+wsi-rfi-ctrls, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"SB+wsi-rfi-ctrls");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("SB+wsi-rfi-ctrls, global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("SB+wsi-rfi-ctrls, global y unstabilized") ;
      }
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test SB+wsi-rfi-ctrls Allowed\n");
  ...
  fprintf(out,"Hash=affd25605daaaa79767f1d32ed97f949\n");
  fprintf(out,"Cycle=Rfi DpCtrldR Fre Wsi Rfi DpCtrldR Fre Wsi\n");
  fprintf(out,"Relax SB+wsi-rfi-ctrls %s %s\n",p_true > 0 ? "Ok" : "No","[Wsi,Rfi]");
  fprintf(out,"Safe=Fre DpCtrldR\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Fr Fr\n");
  fprintf(out,"Orig=Wsi Rfi DpCtrldR Fre Wsi Rfi DpCtrldR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1] [0] ; (0,1) (1,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation SB+wsi-rfi-ctrls %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time SB+wsi-rfi-ctrls %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "SB+wsi-rfi-ctrls: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("SB+wsi-rfi-ctrls, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int SB_2B_wsi_2D_rfi_2D_ctrls(int argc, char **argv, FILE *out) {
