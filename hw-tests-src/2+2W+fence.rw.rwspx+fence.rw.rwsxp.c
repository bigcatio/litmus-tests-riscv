...
inline static int final_cond(int _out_0_x8,int _out_0_x9,int _out_1_x7,int _out_1_x8,int _x) {
  switch (_out_0_x8) {
  case 1:
    switch (_out_1_x7) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x8=%d; 0:x9=%d; 1:x7=%d; 1:x8=%d; [x]=%d;\n",c,show ? '*' : ':',(int)o[out_0_x8_f],(int)o[out_0_x9_f],(int)o[out_1_x7_f],(int)o[out_1_x8_f],(int)o[x_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("2+2W+fence.rw.rwspx+fence.rw.rwsxp, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"2+2W+fence.rw.rwspx+fence.rw.rwsxp");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sw %[x9],0(%[x5])\n"
"#END _litmus_P1\n"
:[x8] "=&r" (_a->out_1_x8[_i]),[x7] "=&r" (_a->out_1_x7[_i])
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("2+2W+fence.rw.rwspx+fence.rw.rwsxp, global x unstabilized") ;
      }
      cond = final_ok(final_cond(_out_0_x8_i,_out_0_x9_i,_out_1_x7_i,_out_1_x8_i,_x_i));
...
#else
#include "2+2W+fence.rw.rwspx+fence.rw.rwsxp.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test 2+2W+fence.rw.rwspx+fence.rw.rwsxp Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (not ([x]=4 /\\ (0:x8=1 /\\ (0:x9=0 /\\ (1:x8=1 /\\ (1:x7=0 \\/ 1:x7=1 \\/ 1:x7=2) \\/ 1:x8=0 /\\ (1:x7=2 \\/ 1:x7=0)) \\/ 0:x9=1 /\\ (1:x7=0 /\\ (1:x8=1 \\/ 1:x8=0) \\/ 1:x7=1 /\\ (1:x8=0 \\/ 1:x8=1))) \\/ 0:x9=1 /\\ (0:x8=4 /\\ (1:x7=0 /\\ (1:x8=1 \\/ 1:x8=0) \\/ 1:x7=1 /\\ (1:x8=0 \\/ 1:x8=1)) \\/ 0:x8=3 /\\ 1:x7=1 /\\ 1:x8=0) \\/ 0:x8=3 /\\ 0:x9=0 /\\ 1:x7=1 /\\ 1:x8=0) \\/ 0:x9=0 /\\ [x]=2 /\\ (0:x8=4 /\\ (1:x7=0 /\\ (1:x8=1 \\/ 1:x8=0) \\/ 1:x7=1 /\\ (1:x8=0 \\/ 1:x8=1)) \\/ 0:x8=1 /\\ 1:x7=0 /\\ (1:x8=0 \\/ 1:x8=1)) \\/ 0:x8=1 /\\ 0:x9=1 /\\ 1:x7=0 /\\ [x]=1 /\\ (1:x8=0 \\/ 1:x8=1)))",cond ? "" : "NOT ");
  fprintf(out,"Hash=5b778f5af0961ce54d2ea79417b7ca27\n");
  fprintf(out,"Cycle=Wse Fence.rw.rwsWWPX WseXX Fence.rw.rwsWWXP\n");
  fprintf(out,"Relax 2+2W+fence.rw.rwspx+fence.rw.rwsxp %s %s\n",p_true > 0 ? "Ok" : "No","WseXX");
  fprintf(out,"Safe=Wse Fence.rw.rwsWW\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Ws Ws\n");
  fprintf(out,"Orig=Fence.rw.rwsWWPX WseXX Fence.rw.rwsWWXP Wse\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1] [0] ; \n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation 2+2W+fence.rw.rwspx+fence.rw.rwsxp %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time 2+2W+fence.rw.rwspx+fence.rw.rwsxp %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "2+2W+fence.rw.rwspx+fence.rw.rwsxp: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("2+2W+fence.rw.rwspx+fence.rw.rwsxp, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int _X2_2B_2W_2B_fence_2E_rw_2E_rwspx_2B_fence_2E_rw_2E_rwsxp(int argc, char **argv, FILE *out) {
