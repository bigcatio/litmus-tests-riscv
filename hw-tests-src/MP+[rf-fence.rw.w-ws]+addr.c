...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("MP+[rf-fence.rw.w-ws]+addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("MP+[rf-fence.rw.w-ws]+addr, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"MP+[rf-fence.rw.w-ws]+addr");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("MP+[rf-fence.rw.w-ws]+addr, global y unstabilized") ;
      }
...
#else
#include "MP+[rf-fence.rw.w-ws]+addr.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/MP+[rf-fence.rw.w-ws]+addr.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV MP+[rf-fence.rw.w-ws]+addr");
  fprintf(out,"%s\n","\"RfLeave Fence.rw.wdRW WsBack Rfe DpAddrdR Fre\"");
  fprintf(out,"%s\n","{");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test MP+[rf-fence.rw.w-ws]+addr Allowed\n");
  ...
  fprintf(out,"Hash=17f576c859f34511a03db269d5aa9ec4\n");
  fprintf(out,"Cycle=Rfe DpAddrdR Fre RfLeave Fence.rw.wdRW WsBack\n");
  fprintf(out,"Relax MP+[rf-fence.rw.w-ws]+addr %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"Safe=Rfe Fre Fence.rw.wdRW DpAddrdR RfLeave WsBack\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Ws Fr\n");
  fprintf(out,"Orig=RfLeave Fence.rw.wdRW WsBack Rfe DpAddrdR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0, 1] [2] ; (2,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation MP+[rf-fence.rw.w-ws]+addr %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time MP+[rf-fence.rw.w-ws]+addr %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "MP+[rf-fence.rw.w-ws]+addr: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("MP+[rf-fence.rw.w-ws]+addr, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int MP_2B__5B_rf_2D_fence_2E_rw_2E_w_2D_ws_5D__2B_addr(int argc, char **argv, FILE *out) {
