...
inline static int final_cond(int _out_0_x12,int _out_0_x15,int _out_1_x10) {
  switch (_out_0_x12) {
  case 0:
    switch (_out_0_x15) {
    case 0:
...
static const int out_0_x12_f = 0 ;
static const int out_0_x15_f = 1 ;
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x12=%d; 0:x15=%d; 1:x10=%d;\n",c,show ? '*' : ':',(int)o[out_0_x12_f],(int)o[out_0_x15_f],(int)o[out_1_x10_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("OpVsHerd01, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("OpVsHerd01, check_globals failed");
    if (rand_bit(&(_a->seed)) && b[_i] != 0) fatal("OpVsHerd01, check_globals failed");
    if (rand_bit(&(_a->seed)) && a[_i] != 0) fatal("OpVsHerd01, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"OpVsHerd01");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x15],0(%[x9])\n"
"#END _litmus_P0\n"
:[x15] "=&r" (_a->out_0_x15[_i]),[x12] "=&r" (_a->out_0_x12[_i]),[x18] "=&r" (trashed_x18),[x14] "=&r" (trashed_x14),[x13] "=&r" (trashed_x13),[x10] "=&r" (trashed_x10),[x6] "=&r" (trashed_x6)
:[x8] "r" (&_a->x[_i]),[x9] "r" (&_a->y[_i]),"[x18]" (&_a->a[_i]),[x19] "r" (&_a->b[_i])
:"cc","memory"
);
    }
  }
...
"lw %[x10],0(%[x8])\n"
"#END _litmus_P1\n"
:[x10] "=&r" (_a->out_1_x10[_i]),[x6] "=&r" (trashed_x6)
...
static void init(ctx_t *_a) {
  ...
  _a->out_0_x15 = malloc_check(size_of_test*sizeof(*(_a->out_0_x15)));
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      int _out_0_x15_i = ctx.out_0_x15[_i];
      int _out_1_x10_i = ctx.out_1_x10[_i];
      outcome_t o;
      int cond;

      cond = final_ok(final_cond(_out_0_x12_i,_out_0_x15_i,_out_1_x10_i));
      o[out_0_x12_f] = _out_0_x12_i;
      o[out_0_x15_f] = _out_0_x15_i;
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test OpVsHerd01 Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (0:x15=0 /\\ 1:x10=0 /\\ 0:x12=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=92fc9a005a8e3b5c216174e77c59f272\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation OpVsHerd01 %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time OpVsHerd01 %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "OpVsHerd01: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int OpVsHerd01(int argc, char **argv, FILE *out) {
