...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("WRC+fence.rw.rws+pos, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"WRC+fence.rw.rws+pos");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("WRC+fence.rw.rws+pos, global x unstabilized") ;
      }
...
#else
#include "WRC+fence.rw.rws+pos.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/CO/WRC+fence.rw.rws+pos.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test WRC+fence.rw.rws+pos Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (not (1:x5=0 /\\ (2:x5=0 /\\ ([x]=1 /\\ (2:x7=2 \\/ 2:x7=1 \\/ 2:x7=0) \\/ [x]=2 /\\ (2:x7=0 \\/ 2:x7=1 \\/ 2:x7=2)) \\/ 2:x5=1 /\\ (2:x7=1 /\\ ([x]=1 \\/ [x]=2) \\/ 2:x7=2 /\\ [x]=2) \\/ 2:x5=2 /\\ (2:x7=2 /\\ ([x]=2 \\/ [x]=1) \\/ 2:x7=1 /\\ [x]=1)) \\/ 1:x5=1 /\\ [x]=2 /\\ (2:x5=0 /\\ (2:x7=0 \\/ 2:x7=1 \\/ 2:x7=2) \\/ 2:x5=1 /\\ (2:x7=2 \\/ 2:x7=1) \\/ 2:x5=2 /\\ 2:x7=2)))",cond ? "" : "NOT ");
  fprintf(out,"Hash=4ad807a6a19209ff42ea55640ad212b4\n");
  fprintf(out,"Cycle=Rfe PosRR Fre Rfe Fence.rw.rwsRW\n");
  fprintf(out,"Relax WRC+fence.rw.rws+pos %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"Safe=Rfe Fre PosRR Fence.rw.rwsRW\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Rf Fr\n");
  fprintf(out,"Orig=Rfe Fence.rw.rwsRW Rfe PosRR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0, 1] [2] ; (2,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation WRC+fence.rw.rws+pos %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time WRC+fence.rw.rws+pos %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "WRC+fence.rw.rws+pos: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("WRC+fence.rw.rws+pos, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int WRC_2B_fence_2E_rw_2E_rws_2B_pos(int argc, char **argv, FILE *out) {
