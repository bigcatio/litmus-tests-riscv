...
inline static int final_cond(uint64_t _out_0_x7,uint64_t _out_0_x9,uint64_t _out_1_x7,uint64_t _out_1_x9) {
  switch (_out_0_x7) {
  case 1:
    switch (_out_0_x9) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x7=%"PRIu64"; 0:x9=%"PRIu64"; 1:x7=%"PRIu64"; 1:x9=%"PRIu64";\n",c,show ? '*' : ':',(uint64_t)o[out_0_x7_f],(uint64_t)o[out_0_x9_f],(uint64_t)o[out_1_x7_f],(uint64_t)o[out_1_x9_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("SB+rfi-addrs, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("SB+rfi-addrs, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"SB+rfi-addrs");
  check_globals(_a);
  int _th_id = _b->th_id;
...
:[x5] "r" ((int)(1)),[x6] "r" (&_a->y[_i]),[x10] "r" (&_a->x[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      uint64_t _out_0_x7_i = ctx.out_0_x7[_i];
      uint64_t _out_0_x9_i = ctx.out_0_x9[_i];
      uint64_t _out_1_x7_i = ctx.out_1_x7[_i];
      uint64_t _out_1_x9_i = ctx.out_1_x9[_i];
      outcome_t o;
      int cond;

      cond = final_ok(final_cond(_out_0_x7_i,_out_0_x9_i,_out_1_x7_i,_out_1_x9_i));
...
static void prelude(FILE *out) {
  ...
  fprintf(out,"%s\n"," uint64_t y=0;");
  fprintf(out,"%s\n"," uint64_t x=0;");
  fprintf(out,"%s\n"," uint64_t 0:x9=0; uint64_t 0:x7=0; 0:x5=1; 0:x6=x; 0:x10=y;");
  fprintf(out,"%s\n"," uint64_t 1:x9=0; uint64_t 1:x7=0; 1:x5=1; 1:x6=y; 1:x10=x;");
  fprintf(out,"%s\n","}");
  fprintf(out,"%s\n"," P0             | P1             ;");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test SB+rfi-addrs Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (0:x7=1 /\\ 0:x9=0 /\\ 1:x7=1 /\\ 1:x9=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=fa7cab6fa58df9d2d25066f900c3474e\n");
  fprintf(out,"Generator=diyone7 (version 7.52)\n");
  fprintf(out,"Com=Fr Fr\n");
  fprintf(out,"Orig=Rfi DpAddrdR Fre Rfi DpAddrdR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1] [0] ; (0,1) (1,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation SB+rfi-addrs %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time SB+rfi-addrs %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "SB+rfi-addrs: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int SB_2B_rfi_2D_addrs(int argc, char **argv, FILE *out) {
