...
inline static int final_cond(int _out_0_x6,int _x) {
  switch (_out_0_x6) {
  case 0:
    switch (_x) {
    case 1:
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("RStar-WStar+W, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"RStar-WStar+W");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sc.w %[x8],%[x7],0(%[x5])\n"
"#END _litmus_P0\n"
:[x8] "=&r" (_a->out_0_x8[_i]),[x6] "=&r" (_a->out_0_x6[_i]),[x7] "=&r" (trashed_x7)
:[x5] "r" (&_a->x[_i])
:"cc","memory"
);
    }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("RStar-WStar+W, global x unstabilized") ;
      }
      cond = final_ok(final_cond(_out_0_x6_i,_x_i));
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test RStar-WStar+W Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (0:x6=0 /\\ [x]=1)",cond ? "" : "NOT ");
  fprintf(out,"Hash=3db34656fd1bc0442d3c0674e1e4d6cd\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation RStar-WStar+W %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time RStar-WStar+W %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "RStar-WStar+W: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int RStar_2D_WStar_2B_W(int argc, char **argv, FILE *out) {
