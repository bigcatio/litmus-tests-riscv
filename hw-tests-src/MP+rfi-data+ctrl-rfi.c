...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("MP+rfi-data+ctrl-rfi, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("MP+rfi-data+ctrl-rfi, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"MP+rfi-data+ctrl-rfi");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("MP+rfi-data+ctrl-rfi, global x unstabilized") ;
      }
...
#else
#include "MP+rfi-data+ctrl-rfi.h"
#endif
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test MP+rfi-data+ctrl-rfi Allowed\n");
  ...
  fprintf(out,"Hash=6f3f028f0474610002f8d140722f0e49\n");
  fprintf(out,"Cycle=Rfi Fre Rfi DpDatadW Rfe DpCtrldW\n");
  fprintf(out,"Relax MP+rfi-data+ctrl-rfi %s %s\n",p_true > 0 ? "Ok" : "No","Rfi");
  fprintf(out,"Safe=Rfe Fre DpDatadW DpCtrldW\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Fr\n");
  fprintf(out,"Orig=Rfi DpDatadW Rfe DpCtrldW Rfi Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0] [1] ; (1,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation MP+rfi-data+ctrl-rfi %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time MP+rfi-data+ctrl-rfi %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "MP+rfi-data+ctrl-rfi: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("MP+rfi-data+ctrl-rfi, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int MP_2B_rfi_2D_data_2B_ctrl_2D_rfi(int argc, char **argv, FILE *out) {
