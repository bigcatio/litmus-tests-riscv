...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>0:x9=%d; 1:x7=%d;\n",c,show ? '*' : ':',(int)o[out_0_x9_f],(int)o[out_1_x7_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("C-Will02, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("C-Will02, check_globals failed");
    if (rand_bit(&(_a->seed)) && ok[_i] != 1) fatal("C-Will02, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"C-Will02");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"ori %[x8],x0,1\n"
"#_litmus_P0_3\n\t"
"amoadd.w.rl %[x9],%[x5],(%[x7])\n"
"#_litmus_P0_4\n\t"
"fence rw,rw\n"
"#END _litmus_P0\n"
:[x9] "=&r" (_a->out_0_x9[_i]),[x8] "=&r" (trashed_x8),[x5] "=&r" (trashed_x5)
:[x6] "r" (&_a->x[_i]),[x7] "r" (&_a->y[_i])
:"cc","memory"
);
    }
  }
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test C-Will02 Allowed\n");
  ...
  fprintf(out,"Hash=527087dbf8966bcd0048dbd64c00ee9c\n");
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation C-Will02 %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time C-Will02 %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "C-Will02: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int C_2D_Will02(int argc, char **argv, FILE *out) {
