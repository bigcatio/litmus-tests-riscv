...
inline static int final_cond(int _out_1_x5,int _out_1_x8,int _out_3_x5,int _out_3_x8) {
  switch (_out_1_x5) {
  case 1:
    switch (_out_1_x8) {
    case 0:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x5=%d; 1:x8=%d; 3:x5=%d; 3:x8=%d;\n",c,show ? '*' : ':',(int)o[out_1_x5_f],(int)o[out_1_x8_f],(int)o[out_3_x5_f],(int)o[out_3_x8_f]);
}
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"IRIW+addrs");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      cond = final_ok(final_cond(_out_1_x5_i,_out_1_x8_i,_out_3_x5_i,_out_3_x8_i));
...
#else
#include "IRIW+addrs.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/IRIW+addrs.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV IRIW+addrs");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (1:x5=1 /\\ 1:x8=0 /\\ 3:x5=1 /\\ 3:x8=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=1e12f27fd5670c0fee59020cab1f4c53\n");
  fprintf(out,"Cycle=Rfe DpAddrdR Fre Rfe DpAddrdR Fre\n");
  fprintf(out,"Relax IRIW+addrs %s %s\n",p_true > 0 ? "Ok" : "No","");
...
int IRIW_2B_addrs(int argc, char **argv, FILE *out) {
