...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("3.LB+fence.rw.w+ctrl+addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("3.LB+fence.rw.w+ctrl+addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("3.LB+fence.rw.w+ctrl+addr, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"3.LB+fence.rw.w+ctrl+addr");
  check_globals(_a);
  int _th_id = _b->th_id;
...
#else
#include "3.LB+fence.rw.w+ctrl+addr.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/3.LB+fence.rw.w+ctrl+addr.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV 3.LB+fence.rw.w+ctrl+addr");
  fprintf(out,"%s\n","\"Fence.rw.wdRW Rfe DpCtrldW Rfe DpAddrdW Rfe\"");
  fprintf(out,"%s\n","{");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test 3.LB+fence.rw.w+ctrl+addr Allowed\n");
  ...
  fprintf(out,"Hash=0680f1efbbad3aa1ed8c3949ec267229\n");
  fprintf(out,"Cycle=Rfe Fence.rw.wdRW Rfe DpCtrldW Rfe DpAddrdW\n");
  fprintf(out,"Relax 3.LB+fence.rw.w+ctrl+addr %s %s\n",p_true > 0 ? "Ok" : "No","");
  ...
  fprintf(out,"Orig=Fence.rw.wdRW Rfe DpCtrldW Rfe DpAddrdW Rfe\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1, 2] [0] ; \n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation 3.LB+fence.rw.w+ctrl+addr %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time 3.LB+fence.rw.w+ctrl+addr %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "3.LB+fence.rw.w+ctrl+addr: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("3.LB+fence.rw.w+ctrl+addr, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int _X3_2E_LB_2B_fence_2E_rw_2E_w_2B_ctrl_2B_addr(int argc, char **argv, FILE *out) {
