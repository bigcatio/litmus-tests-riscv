...
inline static int final_cond(int _out_1_x7,int _y) {
  switch (_out_1_x7) {
  case 0:
    switch (_y) {
    case 2:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x7=%d; [y]=%d;\n",c,show ? '*' : ':',(int)o[out_1_x7_f],(int)o[y_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("R+poarar+poarp+NEW, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("R+poarar+poarp+NEW, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"R+poarar+poarp+NEW");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("R+poarar+poarp+NEW, global y unstabilized") ;
      }
      cond = final_ok(final_cond(_out_1_x7_i,_y_i));
...
#else
#include "R+poarar+poarp+NEW.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/AMO_X0_2_THREAD/R+poarar+poarp+NEW.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV R+poarar+poarp+NEW");
  fprintf(out,"%s\n","\"PodWWARAR WseARAR PodWRARP FrePAR\"");
  fprintf(out,"%s\n","{");
  ...
  fprintf(out,"%s\n"," amoswap.w.aqrl x0,x5,(x7) | lw x7,0(x8)               ;");
  fprintf(out,"%s\n","");
  fprintf(out,"%s\n","exists ([y]=2 /\\ 1:x7=0)");
  fprintf(out,"%s\n","Generated assembler");
  ass(out);
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test R+poarar+poarp+NEW Allowed\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists ([y]=2 /\\ 1:x7=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=b89a1e4235a6d424ac63bfaf173564fa\n");
  fprintf(out,"Cycle=FrePAR PodWWARAR WseARAR PodWRARP\n");
  fprintf(out,"Relax R+poarar+poarp+NEW %s %s\n",p_true > 0 ? "Ok" : "No","AR");
  fprintf(out,"Safe=Fre Wse PodWW PodWR\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Ws Fr\n");
  fprintf(out,"Orig=PodWWARAR WseARAR PodWRARP FrePAR\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[0] [1] ; (1,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation R+poarar+poarp+NEW %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time R+poarar+poarp+NEW %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "R+poarar+poarp+NEW: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("R+poarar+poarp+NEW, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int R_2B_poarar_2B_poarp_2B_NEW(int argc, char **argv, FILE *out) {
