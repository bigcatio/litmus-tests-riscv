...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("SB+rfi-fence.r.rs, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("SB+rfi-fence.r.rs, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"SB+rfi-fence.r.rs");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x8],0(%[x9])\n"
"#END _litmus_P0\n"
:[x8] "=&r" (_a->out_0_x8[_i]),[x7] "=&r" (_a->out_0_x7[_i]),[x5] "=&r" (trashed_x5)
...
"lw %[x8],0(%[x9])\n"
"#END _litmus_P1\n"
:[x8] "=&r" (_a->out_1_x8[_i]),[x7] "=&r" (_a->out_1_x7[_i]),[x5] "=&r" (trashed_x5)
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test SB+rfi-fence.r.rs Allowed\n");
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "SB+rfi-fence.r.rs: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("SB+rfi-fence.r.rs, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int SB_2B_rfi_2D_fence_2E_r_2E_rs(int argc, char **argv, FILE *out) {
