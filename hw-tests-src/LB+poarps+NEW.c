...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("LB+poarps+NEW, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("LB+poarps+NEW, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"LB+poarps+NEW");
  check_globals(_a);
  int _th_id = _b->th_id;
...
#else
#include "LB+poarps+NEW.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/AMO_X0_2_THREAD/LB+poarps+NEW.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV LB+poarps+NEW");
  fprintf(out,"%s\n","\"PodRWARP RfePAR PodRWARP RfePAR\"");
  fprintf(out,"%s\n","{");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LB+poarps+NEW Allowed\n");
  ...
  fprintf(out,"Hash=341400caaf87254b84a07bbb67b32be9\n");
  fprintf(out,"Cycle=RfePAR PodRWARP RfePAR PodRWARP\n");
  fprintf(out,"Relax LB+poarps+NEW %s %s\n",p_true > 0 ? "Ok" : "No","AR");
  fprintf(out,"Safe=Rfe PodRW\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Rf\n");
  fprintf(out,"Orig=PodRWARP RfePAR PodRWARP RfePAR\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1, 0] ; \n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation LB+poarps+NEW %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time LB+poarps+NEW %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LB+poarps+NEW: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int LB_2B_poarps_2B_NEW(int argc, char **argv, FILE *out) {
