...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("Z6.1+fence.w.w+fence.rw.rw+addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("Z6.1+fence.w.w+fence.rw.rw+addr, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("Z6.1+fence.w.w+fence.rw.rw+addr, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"Z6.1+fence.w.w+fence.rw.rw+addr");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_x_i != ctx.cpy_x[_p][_i]) fatal("Z6.1+fence.w.w+fence.rw.rw+addr, global x unstabilized") ;
      }
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_y_i != ctx.cpy_y[_p][_i]) fatal("Z6.1+fence.w.w+fence.rw.rw+addr, global y unstabilized") ;
      }
...
#else
#include "Z6.1+fence.w.w+fence.rw.rw+addr.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/SAFE/Z6.1+fence.w.w+fence.rw.rw+addr.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV Z6.1+fence.w.w+fence.rw.rw+addr");
  fprintf(out,"%s\n","\"Fence.w.wdWW Wse Fence.rw.rwdWW Rfe DpAddrdW Wse\"");
  fprintf(out,"%s\n","{");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test Z6.1+fence.w.w+fence.rw.rw+addr Allowed\n");
  ...
  fprintf(out,"Hash=1dc87b1cf16584b43f9dc2bd51adcaae\n");
  fprintf(out,"Cycle=Rfe DpAddrdW Wse Fence.w.wdWW Wse Fence.rw.rwdWW\n");
  fprintf(out,"Relax Z6.1+fence.w.w+fence.rw.rw+addr %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"Safe=Rfe Wse Fence.w.wdWW Fence.rw.rwdWW DpAddrdW\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Ws Rf Ws\n");
  fprintf(out,"Orig=Fence.w.wdWW Wse Fence.rw.rwdWW Rfe DpAddrdW Wse\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1, 2] [0] ; \n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation Z6.1+fence.w.w+fence.rw.rw+addr %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time Z6.1+fence.w.w+fence.rw.rw+addr %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "Z6.1+fence.w.w+fence.rw.rw+addr: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("Z6.1+fence.w.w+fence.rw.rw+addr, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int Z6_2E_1_2B_fence_2E_w_2E_w_2B_fence_2E_rw_2E_rw_2B_addr(int argc, char **argv, FILE *out) {
