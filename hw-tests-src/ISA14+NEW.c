...
inline static int final_cond(int _out_1_x3,int _out_1_x5,int _out_1_x9) {
  switch (_out_1_x3) {
  case 0:
    switch (_out_1_x5) {
    case 1:
...
static void do_dump_outcome(FILE *fhist, intmax_t *o, count_t c, int show) {
  fprintf(fhist,"%-6"PCTR"%c>1:x3=%d; 1:x5=%d; 1:x9=%d;\n",c,show ? '*' : ':',(int)o[out_1_x3_f],(int)o[out_1_x5_f],(int)o[out_1_x9_f]);
}
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && z[_i] != 0) fatal("ISA14+NEW, check_globals failed");
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("ISA14+NEW, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("ISA14+NEW, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA14+NEW");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"lw %[x3],0(%[x1])\n"
"#END _litmus_P1\n"
:[x9] "=&r" (_a->out_1_x9[_i]),[x5] "=&r" (_a->out_1_x5[_i]),[x3] "=&r" (_a->out_1_x3[_i]),[x2] "=&r" (trashed_x2),[x1] "=&r" (trashed_x1)
:"[x1]" (&_a->x[_i]),[x6] "r" (&_a->y[_i]),[x8] "r" (&_a->z[_i])
:"cc","memory"
);
    }
  }
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      cond = final_ok(final_cond(_out_1_x3_i,_out_1_x5_i,_out_1_x9_i));
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA14+NEW Forbidden\n");
  ...
  fprintf(out,"Condition %s is %svalidated\n","~exists (1:x5=1 /\\ 1:x9=1 /\\ 1:x3=0)",cond ? "" : "NOT ");
  fprintf(out,"Hash=4c7efe43f6cda6c98014b51ad71a6bca\n");
  ...
  fprintf(out,"Observation ISA14+NEW %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA14+NEW %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA14+NEW: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int ISA14_2B_NEW(int argc, char **argv, FILE *out) {
