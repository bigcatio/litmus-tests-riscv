...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("LB+popars+NEW, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("LB+popars+NEW, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"LB+popars+NEW");
  check_globals(_a);
  int _th_id = _b->th_id;
...
#else
#include "LB+popars+NEW.h"
#endif

static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for tests/non-mixed-size/AMO_X0_2_THREAD/LB+popars+NEW.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV LB+popars+NEW");
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test LB+popars+NEW Allowed\n");
  ...
  fprintf(out,"Hash=8c752df55da653eaef6ca808e024eed3\n");
  fprintf(out,"Cycle=PodRWPAR RfeARP PodRWPAR RfeARP\n");
  fprintf(out,"Relax LB+popars+NEW %s %s\n",p_true > 0 ? "Ok" : "No","AR");
  fprintf(out,"Safe=Rfe PodRW\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Rf Rf\n");
  fprintf(out,"Orig=PodRWPAR RfeARP PodRWPAR RfeARP\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1, 0] ; \n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation LB+popars+NEW %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time LB+popars+NEW %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "LB+popars+NEW: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int LB_2B_popars_2B_NEW(int argc, char **argv, FILE *out) {
