...
inline static int filter_cond(int _out_0_x5,int _out_1_x5) {
  switch (_out_0_x5) {
  case 0:
    switch (_out_1_x5) {
    case 0:
...
inline static int final_cond(int _a) {
  switch (_a) {
  case 2:
    return 1;
  default:
...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && lock[_i] != 0) fatal("ISA03+SIMPLE, check_globals failed");
    if (rand_bit(&(_a->seed)) && a[_i] != 0) fatal("ISA03+SIMPLE, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"ISA03+SIMPLE");
  check_globals(_a);
  int _th_id = _b->th_id;
...
"sw %[x6],0(%[x15])\n"
"#_litmus_P0_5\n\t"
"amoswap.w.rl x0,x0,(%[x10])\n"
"#END _litmus_P0\n"
:[x5] "=&r" (_a->out_0_x5[_i]),[x6] "=&r" (trashed_x6)
:[x10] "r" (&_a->lock[_i]),[x15] "r" (&_a->a[_i])
:"cc","memory"
);
    }
  }
...
"amoswap.w.rl x0,x0,(%[x10])\n"
"#END _litmus_P1\n"
:[x5] "=&r" (_a->out_1_x5[_i]),[x6] "=&r" (trashed_x6)
:[x10] "r" (&_a->lock[_i]),[x15] "r" (&_a->a[_i])
:"cc","memory"
);
    }
...
static void init(ctx_t *_a) {
  ...
  _a->lock = malloc_check(size_of_test*sizeof(*(_a->lock)));
...
static void *zyva(void *_va) {
  ...
  for (int n_run = 0 ; n_run < _b->max_run ; n_run++) {
    ...
    for (int _i = _b->size_of_test-1 ; _i >= 0 ; _i--) {
      ...
      for (int _p = N-1 ; _p >= 0 ; _p--) {
        if (_a_i != ctx.cpy_a[_p][_i]) fatal("ISA03+SIMPLE, global a unstabilized") ;
      }
      if (filter_cond(_out_0_x5_i,_out_1_x5_i)) {
        cond = final_ok(final_cond(_a_i));
        o[a_f] = _a_i;
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test ISA03+SIMPLE Required\n");
  ...
  fprintf(out,"Hash=277646d28ce797552d4c6351515fb0f7\n");
  count_t cond_true = p_false;
  count_t cond_false = p_true;
  fprintf(out,"Observation ISA03+SIMPLE %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time ISA03+SIMPLE %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "ISA03+SIMPLE: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
...
int ISA03_2B_SIMPLE(int argc, char **argv, FILE *out) {
