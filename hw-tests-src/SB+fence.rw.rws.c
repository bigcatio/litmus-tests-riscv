...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("SB+fence.rw.rws, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("SB+fence.rw.rws, check_globals failed");
  }
...
static void *P0(void *_vb) {
  ...
  force_one_affinity(_ecpu,AVAIL,_a->_p->verbose,"SB+fence.rw.rws");
  check_globals(_a);
  int _th_id = _b->th_id;
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  fprintf(out,"Test SB+fence.rw.rws Allowed\n");
  ...
  fprintf(out,"Hash=aa9bc0b78dcbf8fc7bfb4591f327e832\n");
  fprintf(out,"Cycle=Fre Fence.rw.rwdWR Fre Fence.rw.rwdWR\n");
  fprintf(out,"Relax SB+fence.rw.rws %s %s\n",p_true > 0 ? "Ok" : "No","");
  fprintf(out,"Safe=Fre Fence.rw.rwdWR\n");
  fprintf(out,"Generator=diy7 (version 7.51+4(dev))\n");
  fprintf(out,"Com=Fr Fr\n");
  fprintf(out,"Orig=Fence.rw.rwdWR Fre Fence.rw.rwdWR Fre\n");
  if (cmd->aff_mode == aff_custom) {
    fprintf(out,"Affinity=[1] [0] ; (0,1) (1,0)\n");
  }
  count_t cond_true = p_true;
  count_t cond_false = p_false;
  fprintf(out,"Observation SB+fence.rw.rws %s %" PCTR " %" PCTR "\n",!cond_true ? "Never" : !cond_false ? "Always" : "Sometimes",cond_true,cond_false);
  fprintf(out,"Time SB+fence.rw.rws %.2f\n",total / 1000000.0);
  fflush(out);
}
...
/* Show parameters to user */
  if (prm.verbose) {
    log_error( "SB+fence.rw.rws: n=%i, r=%i, s=%i",n_exe,prm.max_run,prm.size_of_test);
  ...
  for (int k=0 ; k < n_th ; k++) {
    ...
    if (sum_outs(hk->outcomes) != n_outs || hk->n_pos + hk->n_neg != n_outs) {
      fatal("SB+fence.rw.rws, sum_hist");
    }
    merge_hists(hist,hk);
    free_hist(hk);
  }
...
int SB_2B_fence_2E_rw_2E_rws(int argc, char **argv, FILE *out) {
